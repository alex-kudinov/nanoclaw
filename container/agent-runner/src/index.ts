...
function shouldClose(): boolean {
  if (IPC_INPUT_CLOSE_TARGETED && fs.existsSync(IPC_INPUT_CLOSE_TARGETED)) {
    try {
      fs.unlinkSync(IPC_INPUT_CLOSE_TARGETED);
    } catch {
      /* ignore */
    }
    return true;
...
function writeAckFile(messageId: string): void {
  try {
    ...
    log(
      `Failed to write ack file for ${messageId}: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
...
interface IpcTurnFile extends IpcTurnCandidate {
  filePath: string;
  messageId?: string;
}

function drainIpcInput(mergeIntoExisting: boolean): PendingTurn | undefined {
  try {
    ...
    for (const file of files) {
      ...
      candidates.push({
        ...
        text: data.text,
        isolated: Object.prototype.hasOwnProperty.call(data, 'run_id'),
        runId:
    ...
    const selected = selectNextIpcTurn(candidates, mergeIntoExisting);
    for (const candidate of selected) {
      try {
        fs.unlinkSync(candidate.filePath);
        // Ack fires on READ (before Claude responds). Files after an isolated
        // turn remain on disk and unacked until that turn completes.
        if (candidate.messageId) writeAckFile(candidate.messageId);
      } catch (err) {
        log(
          `Failed to consume input file ${candidate.filePath}: ${err instanceof Error ? err.message : String(err)}`,
        );
        return undefined;
      }
    }
    ...
    return {
      prompt: selected.map((candidate) => candidate.text).join('\n'),
...
interface RunAgentResult {
  ...
  /** Exact logical turn to replay after credential rotation. */
...
async function runAgent(
  ...
  const baseArgs: string[] = [
    ...
    '--model',
    resolveModel(containerInput.model),
    '--mcp-config',
    mcpConfigPath,
    '--allowedTools',
  ...
  if (!pendingTurn.runId) {
    ...
    if (drained) {
      log('Draining pending legacy IPC messages into initial prompt');
      pendingTurn.prompt += '\n' + drained.prompt;
    }
  }
  ...
  try {
    while (true) {
      ...
      if (pendingTurn === undefined) {
        ...
        if (decision.turn) {
          ...
          if (Date.now() - lastActivityMs > WRAPPER_IDLE_TIMEOUT_MS) {
            log(
              `Wrapper idle timeout (${WRAPPER_IDLE_TIMEOUT_MS}ms) — exiting agent loop`,
            );
            break;
          }
      ...
      const runOnce = async (
        ...
        log(
          `Turn ${turnCount}: spawning claude (${args.length} args, prompt ${prompt.length} chars${useSessionId ? ', resume=' + useSessionId : ''})`,
        );
        ...
        try {
          ...
          log(
            `Failed to write prompt to claude stdin: ${err instanceof Error ? err.message : String(err)}`,
          );
        }
        ...
        log(
          `Turn ${turnCount}: claude exited code=${exitCode} stdout_len=${stdout.length}`,
        );
        ...
        if (exitCode !== 0 && !result && stdout.trim()) {
          log(
            `Turn ${turnCount}: stdout snippet (first 500): ${stdout.slice(0, 500)}`,
          );
        }
      ...
      if (attempt.result && typeof attempt.result.session_id === 'string') {
        if (!sessionId)
          log(`Session initialized: ${attempt.result.session_id}`);
      ...
      if (
        attempt.result &&
        typeof attempt.result.usage === 'object' &&
      ...
      const textResult =
        attempt.result && typeof attempt.result.result === 'string'
      ...
      const limitFromStderr =
        attempt.exitCode !== 0 && detectRateLimit(attempt.stderrTail);
      if (limitFromResult || limitFromStderr) {
        const rateLimitMessage = limitFromResult
          ? (textResult ?? undefined)
          : undefined;
        log(
          `Rate-limit detected on turn ${turnCount} — returning to outer rotation logic`,
        );
        clearInterval(heartbeatInterval);
        return { rateLimited: true, rateLimitMessage, failedTurn: turn };
      }
      ...
      if (attempt.exitCode !== 0 || isErrorResult) {
        ...
        if (authText) {
          log(
            `Credential failure detected on turn ${turnCount} — returning to rotation/fallback`,
          );
          clearInterval(heartbeatInterval);
          return {
            rateLimited: false,
...
async function main(): Promise<void> {
  ...
  if (businessDbUrl) {
    try {
      ...
      log(
        `business DB URL parse: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
  ...
  if (!cliEnv.CLAUDE_CODE_OAUTH_TOKEN && !cliEnv.ANTHROPIC_API_KEY_FALLBACK) {
    writeOutput({
      ...
      result: null,
      error:
        'No Claude credential: neither an OAuth token nor an API-key fallback was provided',
    });
    process.exit(1);
  }
  ...
  if (!cliEnv.CLAUDE_CODE_OAUTH_TOKEN && apiKeyFallback) {
    log(
      'No live OAuth token provided — using Anthropic API key (metered billing)',
    );
  ...
  try {
    ...
    if (cliEnv.CLAUDE_CODE_OAUTH_TOKEN) {
      tokenEvents.push(
        tokenFailed(result)
          ? {
              name: nameOf(cliEnv.CLAUDE_CODE_OAUTH_TOKEN),
    ...
    if (tokenFailed(result)) {
      ...
      if (apiKeyFallback && !cliEnv.ANTHROPIC_API_KEY) {
        log(
          'All OAuth tokens exhausted — falling back to Anthropic API key (metered billing)',
        );
        cliEnv.ANTHROPIC_API_KEY = apiKeyFallback;
        ...
        lastMessage =
          keyRetry.rateLimitMessage ?? keyRetry.authFailMessage ?? lastMessage;
