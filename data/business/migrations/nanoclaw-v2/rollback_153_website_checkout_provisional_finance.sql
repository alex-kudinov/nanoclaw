...
DO $$ BEGIN
  IF EXISTS(SELECT 1 FROM business_v2.student_enrollment_orders WHERE source_channel='website_checkout' OR financial_classification='provider_accepted_provisional')
    OR EXISTS(SELECT 1 FROM business_v2.student_financial_obligations WHERE state='accepted_pending_receipt') THEN
    RAISE EXCEPTION 'rollback153 refused: website checkout finance state exists';
  END IF;
END; $$;
...
ALTER TABLE business_v2.student_enrollment_orders
  ADD CONSTRAINT student_enrollment_orders_source_channel_check CHECK (source_channel IN (
    'website_stripe_checkout','manual_stripe_payment','plutio_invoice_or_contract',
    'check_ach_or_wire','sponsored_cohort','scholarship',
    'complimentary_owner_grant','migration_or_correction'
  )),
