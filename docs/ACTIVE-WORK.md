...
same first-sale rejection recurred for AI for Coaches, ACC, MCS practicum and
ACTC. Security and payment identity stay fail-closed; content becomes named
issues. Payment Log and PostgreSQL are always written; roster placement the
host cannot make is held as an open row in the private payments spreadsheet's
`Bookkeeper Exceptions` tab and posted as "Needs your decision". Contador
applies the owner's answer with `tools/contador/resolve-commerce-exception.cjs`,
...
regressions now close both hops. Email-critical 809/809 and independent runner
...
release also dropped the separately reviewed Italian attestation path during a
divergent lineage switch. This integration restores that exact reviewed path and
extends only its closed enum with `pl`, preserving exact-root binding,
request-hash/idempotency checks, TTL, English course identity, English feedback
language, and fail-closed omission. No new route, schema, worker, queue,
scheduler, durable student state, broad locale policy, translation-primary
evidence, or certificate action. Necessity review returned `KEEP`; five focused
files / 165 tests, typecheck, continuity, runtime doctor, email replay, the
...
- 2026-09-11T19:05Z final isolated TEST acceptance: reviewed source fixes and
  ...
  preserves populated legacy rows/guards; the runner and edge share all 11 card
  lifecycle events. Exact retained v1 attribution HTTP replay now survives the
  v2 map and newer document policy without weakening current pins for new
  admissions. Webhook-first and matching Session-result method evidence
  converge without rewriting provenance; retry/pre-submit remains exact-Session
  bound and late-positive fail-closed.
- Review acceptance path: combined corrections R1
  `e765b90a-8dad-4ad9-a471-c26bea17037f` used 3 Sonnet/high calls, 6 input,
  45,338 cache-create, 41,267 cache-read and 17,870 output tokens, maximum
  context 46,878, and found the cross-attempt DDL gap. Corrected R2
  `394c52d0-431f-44ba-ab81-bf03bb4eb6f4` used 3 calls, 6 input, 39,389
  cache-create, 28,317 cache-read and 24,900 output, maximum context 40,929,
  and returned `NO MATERIAL FINDINGS`. Native-defect review
  `139235e8-8b73-467b-9ebf-8c080c95d2e0` used 3 calls, 6 input, 34,033
  cache-create, 21,061 cache-read and 17,101 output, maximum context 34,035;
  its conditional concurrency concern was closed by existing exact PostgreSQL
  uniqueness constraints, leaving no verified material finding.
- Final fresh native TEST used the official 3DS simulator and produced exactly
  one verified Session result, one HMAC `AUTHORISATION=true` event, one `card`
  binding with exact webhook operation/sequence provenance, September 11 Terms,
  the English MCS card v2 attribution map, one canonical active enrollment with
  included component, and one verified Heartbeat projection. Exact owner CLI
  readback confirms the QA user in the target group with 13 total group
  memberships. One subsequent status reload left every newest-attempt business
  row singular. Two earlier global result conflicts remain immutable.
- Payment/checkout verification after the two test-harness corrections is 433
  passed, 8 skipped, 0 failed; typecheck and diff checks pass. The last broader
  suite was not globally green because of established unrelated baselines and
  parallel-only failures, recorded separately rather than attributed to this
  payment path.
- This is deployed TEST evidence, not LIVE activation. It proves authenticated
  TEST result/event/method/enrollment/group membership only; it does not prove
  settlement, customer receipt/welcome email, learner login/course activity,
  certification or production readiness. LIVE remains default-off and was not
  configured, migrated, charged or deployed.

- 2026-09-11T14:40Z: the full private website-checkout service slice is
  source-ready and default-off. One TEST/LIVE composition now owns the existing
  signed routes; LIVE adds loopback-only managed HTTP, explicit private config,
  exact 148/149-159 schema readiness, shallow health, graceful shutdown and a
  restart-safe fulfillment scanner independent of browser/status callbacks.
  The original four-locale TEST artifact/hash is unchanged; the English-only
  revision-2 production candidate remains truthfully inert until a separate
  exact activation receipt. General Heartbeat delivery re-reads canonical
  learner/card authority before every registered provider operation, preserves
  existing identities/groups, adds only the verified English group and records
  post-access adverse review without revoke/certificate action. Tentative local
  migration157 adds exact card-only signed-webhook method evidence, preserving
  Session-result evidence and refusing populated rollback; migration158's
  independently reviewed Gmail notice owner is now wired behind its own
  activation receipt. The scanner keyset-starvation finding from Sonnet/high
  access R1 was fixed; narrow corrections R2 returned no material findings.
  Pinned typecheck/build and focused integration pass; full suite 4220 pass/32
  skip with only the three established unrelated failures. Native anonymous
  TEST reached real 3DS and HTTP200 return but used the DECLINED holder and is
  durably refused/conflict-held, not positive method/payment/enrollment proof.
  No LIVE config/DB/provider/payment/invite/email/deploy action, commit or push
  occurred. A later separately authorized logged-out USD299 official-Visa TEST
  case completed native3DS/return and produced exactly one verified Session
  result plus one card/session_result binding while preserving the earlier
  DECLINED conflict. With zero webhook events/admissions, state remains
  `confirming_payment`/`payment_evidence_pending`; webhook, enrollment, access,
  notice and learner-login outcomes remain gated alongside exact production
  configuration, migration apply/readback, activation and deploy.

- 2026-09-11T14:34Z: the concrete receipt/welcome owner is source-ready for
  managed-service wiring. Local migration 158 provides admin-only immutable
  notice jobs and append-only receipts with empty rollback/reapply and populated
  refusal. The Gmail adapter re-reads exact LIVE card/money/enrollment/Party/
  current Heartbeat authority before each possible send, separates self from
  gift payer/learner copy, pins account/Send-As, and reconciles unknown ACKs
  without blind resend. Focused 12/12, related 84/84, typecheck/build/continuity
  and full 4,220 pass/32 skip with only the three established unrelated
  capacity/CNPC/Trafft failures. Sonnet/high R1 found one aggregate partial-
  receipt reporting inconsistency; corrected so only the complete notice set
  gets a service receipt, and R2 returned no material findings. No email,
  non-disposable DB/config/provider call, migration apply, service activation,
  commit, push or deploy occurred. The full-service owner has the exact factory
  and constructor contract; actual wiring/config/readback remain root-owned.

- 2026-09-11T14:36Z: customer notice ownership is assigned to a bounded
  Gmail-backed adapter and new admin-only local migration 158, ordered after
  the service owner's tentative webhook-method migration 157. The adapter
  will create deterministic self confirmation or gift payer/learner jobs only
  from verified card plus canonical enrollment/access authority, re-read party
  identity before every send, use the host Gmail safety boundary and exact
  readback, and hold uncertain acceptance without blind resend. It will not use
  the Plutio-specific outbox, widen the four-target student projection outbox,
  duplicate Heartbeat's native invitation, read live config/auth, send email,
  apply schema, or activate production. Migration 158 is a tentative local
  allocation only; no global reservation, commit or push is implied.

- 2026-09-11T14:10Z: the next bounded production-readiness slice is in
  implementation: one explicit TEST/LIVE full website-checkout composition,
  loopback-only managed LIVE listener with an explicitly configured port and
  external trusted TLS termination, exact existing-schema readiness checks,
  inert English-only production publication candidate plus a separate
  versioned activation receipt, and default-off production Heartbeat delivery
  through the existing projection outbox. Existing TEST runner/wrappers and the
  four-locale staged artifact remain unchanged. Receipt/welcome ownership is
  being reconciled against current canonical obligations before any new sender
  is proposed. No config/credential read, database/provider connection,
  migration, payment, invitation, registered write operation, commit, push,
  deploy or activation is authorized in this source-only slice.

- 2026-09-11T13:13Z: the bounded environment-explicit Adyen core and
  disabled-by-default English Foundations/card LIVE factory are source-ready in
  the dirty task worktree. TEST exports and executable TEST composition remain
  compatible. LIVE uses only the prefix-constructed Adyen v72 hostname,
  canonical HTTPS return origin, pinned quote authority, literal LIVE caller,
  environment-labeled credentials/HMAC, exact merchant/store scope and a
  disjoint reference namespace. New attempts require enabled+recovery and
  default off; recovery-only keeps status/returns/original-provider recovery
  and owned webhook intake. All 11 existing card reducer codes use the canonical
  event store; owned unsupported events are hash-only. Focused134/134,
  typecheck/build pass; final full4183 pass/32 skip plus three unchanged known
  failures and one parallel disposable timeout that immediately passed2/2 in
  isolation. Sonnet/high bounded R1 had two confirmation-only questions both
  independently disproved; Codex added HTTPS/DNS/quote-authority hardening and
  narrow R2 returned no material findings. No listener/public BFF, config/env
  read, migration, provider call, production DB, fulfillment lowering, commit,
  push, deploy or activation occurred. Managed-host wiring, public WordPress,
  activated publication and production delivery remain separate gaps.

- 2026-09-11T12:47Z: production-readiness continuation claims only the
  environment-explicit provider core and disabled-by-default LIVE composition
  for English Foundations/card. The implementation will preserve the existing
  TEST factory and contracts while separating caller, payment scope, endpoint,
  merchant/store, credential environment, webhook HMAC environment and
  correlation namespace. LIVE endpoints are constructed only from a validated
  Adyen endpoint prefix; all supported card reducer events enter the canonical
  event store and verified owned unsupported events remain hash-only
  exceptions. No public BFF/frontend, listener/service activation, migration,
  environment/credential read, provider request, production database access,
  payment, commit, push or deploy is in scope.

- 2026-09-11T02:57Z: complete English/card TEST service source is validating
  from base `1a1f3b8d`. One listener-ready handler composes all existing signed
  private routes; same `/enrollment-admissions` replay persists encrypted exact
  attribution, moves canonical enrollment held->accepted->duplicate, and returns
  the reviewed promotion verifier only after durable card admission. The native
  HMAC event store remains injected; no default listener/runtime/environment.
  Actual disposable PostgreSQL/HTTP proof is2/2, no-promo/regional null2/2,
  affected63/63, full4142 pass/32 skip/same3 baselines; typecheck/build pass. Exact
  WordPress fixture file `846f3b4d...663` decodes to matching snapshot
  `afff1fa4...317` and binding `23da9601...690`. Sonnet/high R1 found a missing
  promotion caller pin; corrected join/filter uses `checkout_caller`, and narrow
  R2 returned no material findings. Migration156 is still local-only tentative;
  no commit/push/apply, real provider/payment, projection/access/certificate,
  external attribution, communication or business mutation occurred. A real
  coupon fixture corrected unlimited null-redemption handling after R2; explicit
  promotion-policy configuration and null/non-null tests are now in validation.

- 2026-09-11T02:21Z: owner prioritized a complete protected English Foundations
  card TEST purchase over more standalone kernels. Backend sole writer starts at
  `1a1f3b8d`. Proposed executable composition is one TEST-only HTTP router over
  signed session/return/status, identity and same-route checkout-admission plus
  canonical enrollment, with injected provider-event intake. Tandemweb confirmed
  exact signed request/response, attribution snapshot/binding and promotion
  receipt contracts. No new request path or nonce migration is needed. Host
  attribution requires an immutable migration156 record; all fetched refs have
  max migration155 and root migration lease is pending. No source/runtime/schema/
  provider/business mutation has started beyond this continuity update.
...
### NC-20260911-001 — Bounded terminal card retry

- Implemented source-only local migration 159, exact encrypted Session return
  binding, strict sequence-1/2/3 merchant-reference correlation, and signed
  private successor endpoint. Only exact authenticated stored-Session
  `refused|canceled|expired` proof can authorize a deliberate successor.
- One immutable PaymentAttempt and its original attribution/identity/price/
  promotion binding survive the retry. Latest Session method/event lineage is
  independently required before enrollment. Quote expiry returns
  `reconfirmation_required` with zero provider call.
- Late positive Session or webhook evidence for a terminal predecessor becomes
  an owned exception before method binding and is consulted by enrollment,
  Heartbeat access, and customer-notice authority. No automatic revoke or
  certificate action exists.
- Disposable full-service proof covers terminal refusal, sequence-2 success,
  canonical enrollment/access and one stable notice-owner key; separate matrix
  covers the sequence-3 cap, replay after enrollment with zero provider call,
  stale-tab and webhook late-positive diversion, and expired-quote hold.
- Verification: focused retry/integration 172/172 plus fulfillment/notice 36/36,
  typecheck/build/format/diff pass. Full suite 4,246 pass/32 skip; the remaining
...
- A separate TEST attribution-profile boundary preserves the legacy manifest
  ...
  manifest; a disposable v2-pinned service replays accepted v1 fulfillment
  without rewriting attribution. Focused 21/21 and typecheck pass; independent
  review is paused pending root's review-authority decision.
- Root then found that migration159's original backfills would be rejected by
  populated legacy immutable triggers. The correction disables only the three
  named result/reference/event triggers inside its transaction, backfills only
  additive lineage columns, restores/verifies guards before commit, and
  preserves every prior business/cipher/state/version value. A populated
  verified+conflict+event upgrade and rollback-refusal proof passes 2/2;
  independent review remains pending.
- The signed exact-binding pre-submit check now narrows the known-before-submit
  gap without accepting card data or calling the provider. Original and valid
  successor Sessions allow; stale, late/known positive, pending/unknown,
  expired and config-off states block. Focused 28/28 pass. The signed nonce
  audit still writes replay bookkeeping, while the business-state decision is
  locked-read-only. The issued-provider-capability/check-to-submit race remains
  explicit; independent review is pending root's next approved packet.
- No actual provider, non-disposable database, runtime/config, payment,
  enrollment, access, notice, migration apply, commit, push, or deploy action
  occurred. WordPress/BFF integration and all LIVE gates remain root-owned.
...
- Astra's pre-release check found that `mcs-full` plus only an unlisted price
  still qualified through the offer signal. The generic profile now requires
  zero supplied price IDs for offer-key qualification. The exact regression
...
- Validation: 54 focused tests; email-critical 800 plus runner 45; typecheck;
  full root 3,635 passed / 32 skipped with the two unchanged CNPC wrapper and
  time-sensitive Trafft baseline failures. Final 15 native-card turns pass;
  Sonnet R1 rubric finding corrected and R2 reports NO MATERIAL FINDINGS.
...
- Next: verify merged live lineage 138e43ab, release and verify non-sending live behavior.
...
- Exact production case 63 repaired: CSS row 6, Sales placeholder cleared,
  Payment Log row 469 and single PostgreSQL payment read back; no communication.
- Stable offer and provider identities come from the existing entitlement
  catalog. New bindings own only namespace and legacy roster projection.
- First migrated population: inaugural/regular supervision. Other products
  retain their existing routes and are enumerated as coverage gaps.
- Verification: 85 focused tests and typecheck pass; initial full suite 3,630
  passed, 32 skipped and two known unrelated CNPC/Trafft baseline failures.
- Independent review, release and final program reconciliation pending.

### NC-20260906-004 — Sales fact consistency

- Trigger: a 2026-09-06 Sales card used the current March/July 2027
  supervision dates and regular tuition in its internal analysis, then told the
  prospective student that 2027 dates and pricing were unannounced. The host
  accepted the contradiction; an operator correction was required.
- Authority: Google Calendar/calendar-debug remains class-schedule authority;
  operational Sales `SCHEDULE.md` is its daily generated projection; immutable
  program catalogs remain stable fact/price authority beneath provider evidence
  and accepted owner decisions.
- Implementation: `src/sales-fact-consistency.ts` retains per-program schedule
  evidence, scopes denial/date claims by program, fails closed on missing/stale
  projections and ambiguous multi-program claims, and checks the Supervision
  catalog for published tuition. IPC, Slack, approval arming, host rescue, and
  final Gmail execution all stop a factual conflict.
- Review: Claude Sonnet/high R1 found cross-program evidence pooling; the
  correction keeps dates attributed per program and adds three direct
  regressions. Narrow R2 returned no material findings.
- Boundary: no customer email, approval, Slack production canary, calendar,
  knowledge, price, provider, or database write. Deployment is authorized as
  the normal completion step, but activation waits for clean commit/release,
  production drain, and exact live-base reconciliation.
- Deployment: reviewed source commit `663b63be2035a19a628121daed56ba4c915fc33c`
  is live on `mini-claw.local`; source tree `a610ab8e`, artifact digest
  `1997d7db`, archive digest `053ddcf2`. Activation changed only the three
  release pointers and retained rollback plist
  `com.nanoclaw.plist.rollback-da2869fbc655-2026-09-06T22-00-28-552Z`.
- Live verification: `/health`, launchd, and the single `:8088` listener agree
  on `663b63be` under Node 22.23.2; Gmail/Slack are connected; no container,
  queued group, or active/attention send exists. The exact deployed compiled
  validator reads the operational schedule and returns both
  `schedule_contradiction` and `price_contradiction` for the incident sentence,
  while accepting the corrected March/July 2027 and $4,796 statement. This
  read-only canary created no approval, Slack post, Gmail call, action, or send.

### NC-20260906-005 — Academy capacity status sync

- Program: `work:academy-capacity-simple-status-sync`, authorized at Company OS
  revision 218, claimed by this task at revision 219, and bound to ten open
  continuity commitments at revision 220.
- Contribution: prevent routine Academy oversales with a low-complexity
  committed-seat ledger and cached-site status synchronization while accepting
  the small residual simultaneous-sale risk chosen by the owner.
- Required controls: successful-sales and explicit invoice/manual commitments
  update internal inventory; capacity changes are versioned and reasoned;
  transfers remain atomic; a daily reconciliation and only public-state
  threshold transitions publish signed status to WordPress; only affected page
  URLs are purged and prewarmed.
- Hard boundary: no short-lived checkout reservation, synchronous checkout
  dependency on NanoClaw, per-visitor capacity API, automatic waitlist contact,
  refund/payment execution, or full roster/provider/assignment-authority
  cutover. Preserve both sold-out September pools, available MCS Thursday, and
  Rita's settled January Thursday assignment.

### NC-20260906-003

- Program: `work:academy-capacity-minion-operator-workflow`, authorized for
  Gate D at revision 213, claimed by this task at revision 214, and bound to
  nine open continuity commitments at revision 215.
- Contribution: give operators one narrow Capacity processor for exact
  inventory/enrollment reads, manual holds and releases, transfers,
  withdrawals, reconciliation, and FIFO waitlist staging while the host and
  PostgreSQL transactions remain the only policy and mutation boundary.
- Required controls: strict case/result schemas, immutable idempotency keys,
  expected-version checks, stable pool locking, last-seat refusal, bounded
  manual reason/source/TTL evidence, prompt-injection and ambiguity refusal,
  privacy-minimized receipts, no direct database/provider access from the
  minion, and human approval before any waitlist contact.
- Hard boundary: no checkout or Tandemweb/public change, provider/Sheet write,
  customer message, automatic waitlist promotion, refund, payment,
  certificate, or assignment/capacity authority cutover. Rita is settled in
  January Thursday and is not an exception or task input.

### NC-20260906-002

- Program: `work:academy-capacity-production-shadow-population`, authorized at
  Company OS revision 209, claimed at revision 210, and bound to seven open
  continuity commitments at revision 211.
- Owner authority: proceed with production shadow population while preserving
  the three remaining source exceptions. No runtime or authority activation.
- Production preflight: `mini-claw.local` is healthy on exact immutable release
  `886e258730729a2cade1baee70466e62e2bff59e`, Node 22.23.2, Gmail/Slack
  connected, zero active containers and queue depth zero. Migrations 142-143
  are absent. Required `business_v2` Party tables, append-only function, admin
  role, and PostgreSQL 16.13 exist. Of 40 unique accepted participants, 37
  resolve to one active Party, three have no exact email Party, and none match
  multiple Parties.
- Implementation boundary: add the exact active `acc-module-1` offer/bundle;
  use a mode-0600 private runtime manifest whose reviewed SHA-256 is required
  at apply; create Parties only for exact roster identities with no Party;
  reject multiple matches; populate deterministic orders, seats, agreements,
  enrollments, entitlements, assignments, roster projection receipts, capacity
  blocks/pools/offer mappings/events, history, and exactly three held
  exceptions. Repository artifacts persist aggregates/hashes only.
- Delivery boundary: build and extract a current-lineage immutable artifact for
  migration tooling, but do not activate the daemon or enable a consumer.
  Create and verify a custom-format production backup before migration; prove
  first apply and zero-change replay by exact aggregate readback.

### NC-20260906-001

- Program: `work:academy-capacity-reconciliation-resolution`, reauthorized by
  `.program/decisions/decision-academy-capacity-source-repairs-2026-09-06.json`,
  activated at revision 205, claimed at revision 206, and continuity-bound at
  revision 207. Revision 208 completes all six episode commitments, clears the
  claim, and leaves the item waiting on the three exact remaining facts.
- Owner authority: Rita Ramakrishnan's move from September Friday MCS to
  January 2027 Thursday is final. Do not reopen her origin or destination.
- Exact provider state: the Student Roster already placed Rita in January
  Thursday. Heartbeat incorrectly retained her in `MCS Practicum - September
2026`; the guarded removal left her active with `MCS - Standard path` and
  verified the September group absent.
- Source separation: exact Stripe metadata remains Thursday, so the payment
  adjustment stays on September Thursday while the owner-confirmed assignment
  origin remains Friday. The destination pair carries both fields. This
  resolves rather than reopens the apparent conflict.
- ACC source repair: the May 27 boundary row is assigned to `2026-06`; every
  September 7 assignment is now explicitly labeled `2026-09`. Exact readback
  is 21 active seats: 10 Module 1 routes and 11 Full Program routes, capacity
  12, oversold by 9, with zero post-boundary unlabeled rows.
- Offer/funding evidence: the 11 Full Program participants are all exact
  `$3,999` ACC Full enrollments; none is a `$7,499` Professional Coach
  enrollment. Seven bind through Payment Log/individual invoice evidence and
  four through three paid Plutio invoices, including one exact two-seat
  corporate invoice. Nine of ten Module 1 routes have an exact Module 1
  payment; one roster assignment remains a funding-source exception and is
  not inferred.
- Projection evidence: all 11 ACC Full participants have current Heartbeat
  Full Course access (10 exact-email matches plus one exact-name/company email
  alias). No participant belongs to the Professional Coach access group, so no
  PCC/ACTC/Professional projection write is warranted.
- MCS funding: Thursday is 5/matched/open after the historical payment-source
  adjustment. Three exact manual/legacy Payment Log bindings reconcile Friday
  funding to all 13 current assignments; Friday remains sold out and
  `needs_review` only because the 13 rows excluding Rita differ from the prior
  owner count of 12.
- Tandemweb delivery: pushed `56bb8f6ee`; the prevention commit rebased to
  `4bb852bb3` on current `origin/main`. Focused 13/13 and governed pre-push
  design gates pass. Claude's manual-funding validation finding is corrected.
- Toolbox support: committed `8d996dd` adds dry-run-default, exact-group/name,
  exact-user, confirmation-gated `heartbeat/remove-from-group` with post-write
  user/membership readback. Full toolbox tests pass 65/65 plus focused
  Heartbeat tests; Claude R1's readback-refusal and argument-test findings were
  corrected and independently verified against Heartbeat's official OpenAPI.
- Boundary: no refund, communication, certificate, payment, public website,
  production database, runtime/minion, migration, or authority-cutover action.

### NC-20260905-010

- Program: `work:academy-capacity-reconciliation-resolution`, authorized at
  revision 200, claimed at revision 201, with seven continuity commitments at
  revision 202.
- Owner fact: September 7 capacity is 12. The operational 21-seat population is
  oversold by 9; the held 22-seat upper boundary would be oversold by 10.
- Contribution: make that numeric boundary durable and remove the current
  lower-owner-override failure before any future automation or source repair.
- Boundary: local reversible source, tests, review, commit, and push only in
  this slice. All external, production, communication, deployment, migration,
  and authority changes remain separately gated.
- Result: Company OS records capacity 12, operational overage 9, and held upper
  overage 10. Tandemweb now computes the roster and adjusted-Stripe floors
  independently and takes the maximum; an owner assertion may raise but cannot
  lower occupancy. Owner variance stays `needs_review` even without Stripe.
- Review/verification: bounded Claude Sonnet/high returned
  `NO MATERIAL FINDINGS`; Tandemweb focused 10/10 and read-only live-shaped
  report prove 13 roster / owner 12 remains 13 occupied, over by 1, sold out,
  needs review. NanoClaw validator and focused 14/14, typecheck, continuity,
  and diff checks pass.
- Delivery: NanoClaw `23ad8b94` and Tandemweb `6509cef2` are pushed. Nothing
  was deployed and no external source was changed.

### NC-20260905-009

- Program: `work:academy-capacity-sales-evidence-reconstruction`, authorized at
  revision 196, claimed at revision 197, with seven continuity commitments at
  revision 198; all seven completed and the item closed at revision 199.
- Owner correction: count unique participants across ACC Module 1, the $3,999
  ACC Full Program, and the $7,499 Professional Coach Program, including
  check/email sales; the multi-tab combined-program projection consumes one
  shared ACC seat. Rita probably originated in Friday and must remain in
  January, with the exact January weekday still unresolved.
- Boundary: transient identity joins for read-only reconstruction only;
  committed artifacts remain aggregate/hash/exception evidence. No external or
  production write is authorized.
- Result: eight explicit September rows plus 13 cohort-blank rows from the
  June 3 next-entry boundary yield 21 unique seats; the earlier May 27 row is a
  held upper-bound candidate, producing 22 if later proven September. The
  operational 21 split into 10 Module 1 and 11 Full Program roster routes.
- Funding/projection: Payment Log exactly classifies 8 Module 1, 5 ACC Full,
  and 0 Professional Coach participants, leaving 8 without an exact offer.
  None of the 21 intersects PCC, ACTC, or the Professional Coach Heartbeat
  group, so `$7,499` count and fulfillment remain unresolved rather than
  double-counted.
- MCS correction: the current January Thursday assignment remains accepted;
  Friday origin is recorded only as owner recollection and needs historical
  evidence before transfer import.
- Review/verification: Claude Sonnet/high returned `NO MATERIAL FINDINGS` after
  13 load-bearing checks. Combined validator, correction 13/13 and combined
  focused 26/26, pinned typecheck/build, formatting, continuity/capability,
  privacy, and diff checks pass. Full root is 3,544 passed / 32 skipped with
  only the two exact predecessor CNPC-wrapper and date-sensitive Trafft
  failures.
- Delivery: correction/validator/evidence commit `3d900ac0` is pushed on
  `codex/academy-capacity-sales-reconstruction-20260905`; revision 199 keeps
  source resolution separately unauthorized.

### NC-20260905-008

- Program: `work:academy-capacity-readonly-reconciliation`, authorized at
  revision 192, claimed at revision 193, with seven continuity commitments at
  revision 194; all seven completed and the item closed at revision 195.
- Population: MCS 2026-09-24 Thursday, MCS 2026-09-25 Friday, the 2027-01-07
  Thursday and 2027-01-08 Friday transfer destinations, and ACC Module 1
  2026-09-07 with all three offers that consume that start.
- Window/privacy: exact matching records from 2026-01-01 through the current
  2026-09-05 observation; identity may be inspected transiently but only
  aggregates, hashes, and durable exceptions may be committed.
- Boundary: read-only provider/public inspection and local reversible report,
  validator, tests, review, commit, and push. No operational or production
  mutation is authorized.
- Result: five delivery blocks and seven exceptions are persisted without PII.
  Thursday MCS is 5/12 open; Friday is 13/12 and remains sold out rather than
  being reduced to the owner hypothesis of 12. The January assignment exists,
  but its origin weekday is unresolved. ACC September 7 has eight active
  assignments, unknown numeric capacity, and six collapsed funding/offer
  bindings; its owner-directed sold-out state remains fail-closed.
- Review: Claude Sonnet/high R1 found the missing MCS funding exception and
  non-applied JSON Schema. Both were fixed; R2 returned
  `NO MATERIAL FINDINGS`.
- Verification: validator pass, focused 20/20, pinned typecheck/build,
  formatting, documentation continuity/capability, privacy and diff checks.
  Full root is 3,538 passed / 32 skipped with only the two exact predecessor
  CNPC-wrapper and date-sensitive Trafft failures.
- Delivery: report/validator/evidence commit `c0779fcb` is pushed on
  `codex/academy-capacity-readonly-reconciliation-20260905`; revision 195
  registers the not-authorized source-resolution candidate.

### NC-20260905-007

- Program: `work:academy-capacity-disposable-schema-proof` authorized at
  revision 188, claimed at revision 189, and given six continuity commitments
  at revision 190; all six were completed and the item closed at revision 191.
- Lineage: capacity foundation `b7b95824` plus corrected enrollment disposable
  verifier `01351538`; conflicts were reconciled by preserving both task and
  architecture records.
- Contribution: prove migrations 142 and 143 as one synthetic PostgreSQL chain
  before real-data reconciliation can become eligible.
- Boundary: generated local databases only through the reviewed `/tmp:5432`,
  stripped-`PG*`, new-prefix-only runner. No production, `nanoclaw_business`,
  real student/provider/roster/payment/cohort/runtime/message/deployment access.
- Result: local PostgreSQL 16.15 proved 13 enrollment tables/one view/13
  sequences followed by seven capacity tables/one view/seven sequences. The
  synthetic shared pool projected `0|2|2|1|open` before assignment and
  `1|2|1|1|open` afterward; a consumed reservation was not double-counted.
- Rollback: five capacity constraint/refusal paths were reason-matched. The
  populated rollback retained evidence. Cleanup explicitly removed the class
  assignment coupled to the capacity delivery block, retained the enrollment
  order foundation, then proved empty rollback, reapply, and final cleanup.
- Review: Claude Sonnet/high R1 found the implicit cross-migration cascade and
  missing CLI residue self-check. Both were fixed; bounded R2 returned
  `NO MATERIAL FINDINGS`.
- Verification: focused 25/25, pinned typecheck/build, formatting,
  ...
  exact zero prefix residue pass. Full root is 3,531 passed / 32 skipped with
  only the two exact predecessor CNPC-wrapper and date-sensitive Trafft
  failures.
- Delivery: verifier/evidence commit `7091d120` is pushed on
  `codex/academy-capacity-disposable-proof-20260905`.

### NC-20260905-005

- Program alignment: `program:company-os` charter 1.0.0,
  `work:academy-capacity-extension`, authorized at revision 179, claimed by
  this task at revision 180, with seven commitments attached at revision 181.
- Exact base: pushed and independently reviewed enrollment dark-foundation tip
  `deac91a8`, plus the accepted capacity-architecture commits rebased onto it.
- Contribution: extend—not duplicate—the order/seat/enrollment/entitlement/
  assignment/finance/receipt/exception foundation with seat pools,
  reservations, waitlists, occupancy, and typed deterministic host commands.
- Hard boundary: source-only C2. Do not apply a migration, inspect/reconcile
  real students, change MCS Thursday or any live cohort state, wire providers,
  checkout, Bookkeeper, Student Roster, runtime, or a minion, send a message,
  deploy, or cut over authority.
- Implementation: migration/rollback 143, seven admin-only tables plus
  occupancy view, composite relational integrity, guarded rollback, and pure
  commands for registration/mapping, reservations, assignment commit,
  transfer/withdrawal, close/reopen/reconcile, and FIFO waitlist lifecycle.
- Review: one bounded Claude Sonnet/high round returned
  `NO MATERIAL FINDINGS` after 13 SQL/domain/concurrency/idempotency/waitlist/
  rollback checks. No second round is needed.
- Verification: typecheck/format, focused 54/54, continuity/capability, JSON,
  unwired, and diff checks pass. Full root is 3,523 passed / 32 skipped with
  only the two exact predecessor CNPC-wrapper and date-sensitive Trafft
  failures.

### NC-20260905-006

- Program: `work:student-enrollment-disposable-schema-proof` claimed at
  revision 185 with six continuity commitments attached at revision 186.
- Handoff: revision-182 checkpoint was correctly rejected as stale after
  capacity work advanced Company OS. Its semantic safety boundary is retained,
  but its colliding `NC-20260905-004` pickup ID is superseded operationally by
  this collision-free task ID.
- Exact source: reviewed/pushed migration-142 foundation at `deac91a8`.
- Authority: generated disposable PostgreSQL databases with a strict prefix,
  synthetic rows, verifier source/tests/review/commit/push only.
- Prohibited: connecting to or mutating `nanoclaw_business`; real student,
  roster, provider, payment, cohort, runtime, deployment, or communication
  access; role creation/alteration; retaining a disposable database.
- Implementation: credential-free verifier and safety tests; explicit local
  socket/port, stripped ambient `PG*`, server-address proof, generated-prefix
  new-target refusal, reason-matched failures, 13-table/view/sequence ownership
  and grant checks, synthetic chain, rollback refusal/retention, empty rollback,
  reapply, and `finally` cleanup.
- Live synthetic result: PostgreSQL 16.15 returned all proof flags true while
  deliberately poisoned parent `PG*` values were present; exact prefix residue
  was zero before and after.
- Review: Sonnet/high R1 found three target/reason/sequence gaps. All were
  corrected; R2 returned `NO MATERIAL FINDINGS`.
- Verification: focused 12/12, typecheck, and full 3,505 pass / 32 skip with
  the two exact unchanged CNPC-wrapper and date-sensitive Trafft failures.

### NC-20260905-003

- Program alignment: `program:company-os` charter 1.0.0,
  `work:student-enrollment-dark-foundation`, claimed at primary revision 175
  with eight continuity commitments attached at revision 176.
- Authority: accepted local C2 source, reversible migration design, synthetic
  tests, review, commit, and push only. No production/data/provider boundary.
- Base: exact pushed `codex/student-enrollment-foundation-20260905` tip
  `1d77ae5a`; dirty primary remains untouched outside program control.
- Hard exclusions: no migration apply, real student data inspection,
  reconciliation/backfill, adapter or projection activation, provider/Sheet/
  Heartbeat/Encharge/Plutio/runtime write, deployment, or communication.
- Implementation: conservative policy revision 1; contract revision 2; ordered
  migration/rollback 142; 13 tables plus aggregate view; pure unwired domain
  commands for source/evidence, incomplete-order correction, seats,
  participants, materialization, obligations, assignments, projections,
  exceptions, order states, and transfers.
- Review: Sonnet/high R1 found one P0, six P1, and two P2 implementation gaps.
  R2 verified those corrections and found two residual runtime-validation
  misses; lower-snake code validation and evidence-subject validation now have
  direct negative tests. No reviewed material finding remains unresolved.
- Verification: contract validation; five focused files / 61 tests;
  pinned-Node typecheck/build/format, documentation continuity/capability,
  JSON, and diff checks pass. Full root is 3,501 passed / 32 skipped with only
  the unchanged CNPC-wrapper and date-sensitive Trafft baseline failures.

### NC-20260905-004

- Owner decision: the reviewed Academy capacity architecture is accepted under
  `program:company-os`; the separate tandemweb program draft is retired. MCS
  Thursday 2026-09-24 explicitly remains open.
- Predecessor reconciliation: capacity extends the entitlement catalog and
  enrollment order/seat/student-enrollment/class-assignment/finance/projection/
  exception foundation. The earlier conceptual `academy_*` names do not create
  a competing model.
- Concurrent work: the first state update correctly failed on stale revision 173. Reorientation at revision 176 found active
  `work:student-enrollment-dark-foundation` under the existing
  `NC-20260905-003` claim. This task does not interrupt or duplicate it.
- Program result: accepted decision plus revision 177 adds six unauthorized
  dependent candidates for the capacity extension, read-only reconciliation,
  Bookkeeper contract, Capacity minion/operator workflow, Tandemweb cutover,
  and final authority cutover. None is eligible.
- Architecture: one delivery block per seat pool in v1; many offers may share
  it; occupancy is current capacity-bearing class assignments; checkout and
  reason/actor/source/TTL-bound manual holds use one reservation boundary;
  refunds do not imply withdrawals; transfers are atomic; waitlist selection
  is FIFO and outreach remains human-approved.
- Boundary: C1 authority/design records only. No migration, implementation,
  provider/student/roster/waitlist-message/cohort-state/runtime/deployment or
  production change occurred.

### NC-20260905-002

- Program alignment: `program:company-os` charter 1.0.0,
  `work:student-enrollment-multisource-ingress-reconciliation`, claimed by this
  task at primary revision 170 with six continuity commitments attached at
  revision 171. The accepted decision supersedes the narrower payer/student
  candidate and limits this task to foundation-only C1 work.
- Foundation: one order owns one or many independently assignable seats; an
  exact participant and frozen offer/bundle version materialize at most one
  current enrollment per seat. Financial obligations, entitlements, class
  assignments, provider projections, and exceptions remain separate.
- Review: Sonnet/high R1 found two P0, two P1, and one P2. R2 verified the
  corrections and found one remaining missing financial-terms gate assertion;
  Codex added it and its mutation test. No material review finding is open.
- Hard boundary: no historical student inspection, reconciliation, backfill,
  provider/database/runtime change, deployment, or communication.
- Verification: foundation contract validation, 22 focused tests, typecheck,
  build, formatting, continuity/capability checks, and diff/JSON checks pass.
  Full root is 3,462 passed / 32 skipped with only the two unchanged documented
  CNPC-wrapper and date-sensitive Trafft baseline failures.

### NC-20260905-001

- Program alignment: `program:company-os` charter 1.0.0,
  `work:student-lifecycle-entitlement-catalog`, claimed by this task at primary
  program revision 163 with continuity attached at revision 164. The expired
  `work:checkout-failure-customer-recovery` claim was recovered to `waiting`;
  its natural-outcome commitment remains open.
- Exact source: isolated branch `codex/student-entitlement-catalog-20260905`
  from live verified NanoClaw `886e25873072`. The dirty primary and active
  `NC-20260903-001` Student Roster/Contador implementation remain untouched;
  this task reads the Product Map but does not edit payment or roster code/data.
- Read-only inventory: seven complete-program offers, six bundle definitions,
  42 typed components, exact Stripe product/price identifiers, exact Heartbeat
  group/course identifiers, and the Product Map. No student membership or PII
  was requested or retained.
- Authority correction: Company OS owns cross-provider enrollment,
  entitlements, assignments, consumption, reconciliation, and receipts;
  providers retain native-fact authority. The older courses-repository Plutio
  source-of-truth plan is preserved as historical template/compliance evidence
  and corrected in an isolated courses worktree; Plutio is an operator-facing
  projection.
- Heartbeat boundary: existing paid/Course Access Groups remain constant.
  Future enrollment/class markers are hidden, admin-controlled, and
  zero-content; purchase scope never enters class-marker identity. Catalog
  revision 1 authorizes no group or membership creation.
- Evidence and conflicts:
  `docs/programs/company-os/evidence/NC-20260905-001-student-entitlement-audit.md`.
  Full-group attachment graphs, several ACC/PCC/ACTC service quantities,
  ambiguous September groups, roster bundle collapse, and the inactive PCC
  exam-group mismatch remain explicit rather than inferred.
- Current verification: catalog validator passes 42 components / 6 bundles / 7
  offers / 6 conflicts / 20 open questions; focused Vitest 9/9, TypeScript
  typecheck, documentation continuity, and diff checks pass. Sonnet/high R1's
  two material findings—unenforced typed vocabulary and an invalid marker on
  MCS individual mentoring—are corrected; narrow R2 returned no material
  findings. Full root regression is 3,449 passed / 32 skipped / 2 unchanged
  base failures (CNPC wrapper contract and date-sensitive Trafft fixture);
  build and format pass. Courses authority correction is pushed at `419981a1`.
  NanoClaw commits `64f48df8` and `8a0d3b10` and courses commit `419981a1`
  are pushed on their isolated branches. Deployment is not applicable; no
  provider or runtime write exists.

### NC-20260904-001

- Live ingress evidence: the exact Stripe destination showed two 403 attempts
  for one event while neighboring attempts to the same URL returned 200.
  Cloudflare Ray `a35f707ecd6b2d7c` attributes the block to managed OWASP rule
  `949110`, not a custom rule, n8n, or NanoClaw.
- Applied boundary: custom rule `44343c96ea2c43789a39319c1f68be11`
  matches only the Stripe n8n host/method/path and skips only managed WAF rules.
  It leaves custom rules, rate limits, logging, and downstream Stripe signature
  validation intact. The original event resend returned 200.
- Fulfillment evidence: case 55 moved from version-0
  `provider_delivery_missing` to version-1 `complete`; Payment Log,
  PostgreSQL, roster, and final receipts are verified. The exact MCS row exists
  with the payment date, but its cohort cell is blank.
- Cohort root cause: NC-20260817-001's useful cohort resolver/writer was never
  committed. Once immutable releases owned `tools/contador`, the operational
  files stopped being runtime authority and the feature disappeared.
- Local repair: current split metadata resolves to `September 2026 – Friday`;
  legacy slugs/text remain supported; partial/non-MCS evidence fails closed;
  existing Sheet/Postgres operator values are preserved; readback is required;
  and the release builder requires the tracked helper.
- Verification: focused 160/160, syntax, typecheck, build, and continuity pass
  under Node 22.23.2. Full root is 3,440 pass / 32 skip with only the two exact
  recorded base failures. Sonnet/high R1's one material non-MCS scope finding
  is corrected; fresh R2 returned `NO MATERIAL FINDINGS`. Commit/push,
  immutable release, deployment, and exact cohort repair remain.
- Rollback: disable/delete only the exact Cloudflare custom rule for ingress;
  source rollback to `a00edaeb` restores the cohort omission.

### NC-20260903-002

- Exact lineage: isolated branch `codex/support-routing-integrity-20260903`
  from post-live `58bfa985`; production was verified on immutable release
  `658b4730`. The dirty primary checkout is untouched.
- Trigger: overnight access/support mail produced two nonexistent
  `MrGru/student/support` classifications routed only to Chief, one visible
  Mailman escalation with no durable classification, and one correctly
  Sales-routed message contaminated by an expected inbound-send denial that
  later agents misread as a recipient/thread integrity failure.
- Root design defect: Mailman had two non-atomic completion paths—visible
  escalation and raw classification IPC—while only classification drove host
  routing. The live taxonomy, knowledge labels, and hard-coded prefix router
  disagreed; unknown labels were inserted and stamped routed. Chief retained a
  competing customer-draft workflow, and Gmail denial semantics could not
  distinguish raw inbound classification from real approved delivery.
- Accepted design: one typed `classify_email` call, exact stored-source binding,
  canonical policy plus migration 141, Sales ownership for support/refund,
  explicit classify-only dispositions, host-owned fan-out, and a bounded
  restart-safe reaper for both missing and `routed_at IS NULL` classifications.
  A routed host fallback blocks late model re-routing. Mailman run-kind proof
  keeps expected inbound denials out of Chief without weakening genuine
  approved-action alerts.
- Claude review: Sonnet/high R1 returned `GO WITH REQUIRED CHANGES`. It required
  run-kind proof before denial wording changes, recovery of durable unrouted
  rows as well as missing rows, and a late-classification race guard. All three
  are implemented with focused coverage.
- Safety: no customer email, approval, payment, provider write, relationship
  mutation, or broad Gmail grant is authorized. Existing exact-card, Action-ID,
  recipient/CC, Gmail execution, idempotency, and confirmed-receipt boundaries
  remain unchanged.
- Current evidence: pinned Node 22.23.2 typecheck and 169 focused tests pass.
  Documentation/capability/full/release gates, implementation review, commit,
  migration, activation, live no-send canaries, and exact stranded-work recovery
  remain pending.

### NC-20260903-001

- Trigger: a mapped Foundations payment hit a Student Roster timeout, rendered
  as unmapped, and remained `write_failed` after Gru manually reran the script
  outside the durable case boundary.
- Audit: production message/case/Sheet/Plutio readback over 2026-08-03 through
  2026-09-03 found 20 flagged Stripe objects among 96. Exact content-minimized
  dispositions are recorded in
  `docs/reports/NC-20260903-001-GRU-BOOKKEEPER-MONTH-AUDIT.md`.
- Applied production repair: two missing exact aliases added/read back; ten
  archived transactions replayed through host ownership to `complete`; eight
  stale in-window Sales rows cleared only after destination proof, including
  eight verified group-invoice participant rows. Current ledger has zero
  `needs_product` and zero `processing`.
- Local source: retry one safe transient Sheets GET; keep retryable stage
  failures in the existing five-attempt webhook queue; post retry outcome;
  render failed classification truthfully; restore non-student and renamed-tab
  controls; clear stale catch-all only after readback; hold Plutio invoices for
  participant identity; prohibit direct-script completion claims in the prompt.
- Verification: Node 22.23.2; focused 149/149; typecheck, build, root formatting,
  continuity, capability, and diff checks pass. Full root is 3,414 pass / 32
  skip / two exact base failures in the CNPC wrapper-literal and date-stale
  Trafft fixture tests. Bounded Sonnet/high R1B returned
  `NO MATERIAL FINDINGS`; commit/push,
  immutable release, deployment, and controlled transient-failure canary
  remain.
- External boundary: no payment, refund, accounting/QuickBooks action,
  customer communication, or manufactured purchase. Product Map/roster/Payment
  Log and fulfillment-case repair is C4 and already applied/read back.

### NC-20260902-001

- Incident: Lead #1311's Sales draft said Coaching Supervision Mastery was
  still in development with no schedule or public price. The exact live
  release-mounted KB contained those stale instructions, while the operational
  Mini checkout already said live/enrolling and carried a current schedule plus
  two learned corrections for the same error.
- Root cause: Procurement's release-owned procedure boundary was generalized to
  every packaged minion knowledge directory. That silently overrode configured
  operational mounts, omitted ignored `SCHEDULE.md`, and separated
  host-written `LEARNED.md` from the file the next minion invocation read.
- Implementation: restrict release-owned knowledge to Procurement; keep stable
  facts/catalogs and prompts immutable while Sales and other business KBs use
  their configured operational mount. Add an exact Coaching Supervision
  Mastery catalog/pack, stale-negative and checkout checks, deterministic
  all-KB injection, build validation, and an activation check against the
  effective operational target. The activator remains read-only; deployment
  performs an explicit backup and injection before activation.
- Verification: canonical injection/check 13/13; deterministic detector five
  domains / zero findings; focused TypeScript 92/92; Python 8/8; formatting,
  shell/Python syntax, typecheck, continuity, email-critical 748/748, and runner
  45/45 pass. Full root is 3,394 pass/32 skip with the exact two failures
  reproduced on untouched base `59fbdbea`. The negative target-root check
  correctly rejects the unsynchronized operational checkout. Claude
  Sonnet/high R1 returned `GO` with no material findings; release, deployment,
  and live replay remain pending.
- Scope boundary: no schema, provider, schedule, approval, Slack post, customer
  email, checkout, or service mutation. The linked draft remains unapproved.
- Reviewed commit: `8dcadfda4e01458a8827394b3cc52ff15bcd0db6`.
- Release/deployment: final documentation commit lineage produced immutable
  release `658b473061a3a684e837c409fa3737812fe3a8e9`, source tree
  ...
  Local fresh extraction and Mini installation both verify under Node 22.23.2.
- Operational transition: backup
  `/Users/xbohdpukc/nanoclaw-knowledge-backups/NC-20260902-001-20260902T1230Z/operational-knowledge-before.tar.gz`
  has SHA-256
  `95568754fcf24268a003cd47c9c65679303ce30357135ce6ac16cde2e653218f`.
  Target-release injection/check passed across 14 current KBs. Sales schedule
  hash `f5e1c1b0…93ae` and learned hash `5a3a3e2e…b6ed` were unchanged; the
  reviewed validation script is `3f8dcc7e…0c37` in both release and checkout.
- Activation: after Sales and Procurement drained naturally, the final preflight
  found zero active containers and zero pending approved/executing actions.
  Dry-run and apply changed only the three release pointers and retained
  rollback plist
  `com.nanoclaw.plist.rollback-8df61d98c9e4-2026-09-02T13-20-43-242Z`.
- Live verification: PID/listener/launchd agree on 30957; `/health` reports
  exact verified release `658b4730`, Node 22.23.2, connected Gmail/Slack, zero
  active/waiting work. The installed bundle re-verifies. The compiled live
  planner resolves Sales to `~/dev/NanoClaw/knowledge/agents/sales` and
  Procurement to the release procedure tree. Operational Sales has one exact
  AACS block, zero exact stale claims, unchanged Schedule/Learned hashes, and
  the exact-release detector reports five domains, provider products available,
  and zero findings.
- Outcome: Alex's in-thread request to reread the KB produced a corrected Lead
  #1311 card with both current cohort schedules and Coaching Supervision Mastery
  enrolling now. It remains unapproved and no customer email was sent. The card
  was generated during the drain before activation, so current-release proof is
  the post-activation mount plan plus exact consumer/detector readback rather
  than a claim about that container's release lineage.
- Non-regression note: the checkout-recovery `column reference "id" is
ambiguous` tick error occurred repeatedly on prior release `8df61d98` and
  continues unchanged; it is not introduced or repaired by this knowledge task.
...
- Boundary: providers stay absent and the workflow stays inactive until exact
  ...
  pass; the same four rows are restored with exact readback and registry receipt 4. Circle, legacy receivers, lifecycle consumers, and student/provider
