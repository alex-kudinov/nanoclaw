...
| `NC-20260823-006` | Make every host-admitted Stripe payment or refund finish with verified operational receipts or remain as a durable owned Contador exception | Codex | natural proof pushed on `codex/contador-natural-payment-proof-20260827` from current evidence `f13959a4`; implementation/live lineage `b131071c` is contained in exact live `6a978328` | `deployed_unverified` | C4 | The 07:54 CDT natural Foundations payment proves the happy path: case 12 completed in one attempt with exact aliases, six verified stage receipts, handled n8n binding, and final Payment Log/PostgreSQL/roster readback. Refund validation stays separate. The broader terminal-state invariant is not complete: two older cases remain `processing` with expired leases and only admission receipts; their source inbox rows dead-lettered after five attempts on `invalid_charge_alias`. A distinct 04:14 CDT same-course payment never reached webhook inbox/case/PostgreSQL. Current aggregate is six complete, two needs-product, one write-failed, one refund needs-review, and two stale-processing. No provider/event replay, missed-payment repair, database write, refund, customer communication, product/payer expansion, or runtime change occurred. | Obtain a bounded correction decision for charge-alias compatibility and host-only terminalization of the two expired cases without external replay; separately design Stripe-to-inbox parity/owned exception capture for missing provider deliveries. Product/student identity remains dependency-blocked; refund closure remains separate. | 2026-08-27T17:05Z |
