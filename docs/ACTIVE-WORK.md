...
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- |
| `NC-20260906-001` | Post one verified, image-bearing congratulations thread in Heartbeat's `Our Graduates` channel after every newly issued public certificate, from both the Foundation grading workflow and Gru Certifier | Codex + Claude reviewer | NanoClaw pushed `codex/graduate-announcements-20260906`; toolbox final `c252fd3`; Tandemweb main `583b240a0`; personal Heartbeat grading skill | `complete` | C3 | Shared guarded announcement/channel inspection, reviewed Gru follow-through, and validated grading-skill ledger are deployed. Owner-authorized Michelle Ambrose test reconciled one existing active public MCS Practicum credential, created one post, and proved duplicate suppression. Final browser QA shows an inline member mention, complete certificate image, and clickable verification link inside a compact Tandem-hosted card with no Heartbeat preview. Private and `already_issued` auto-announcements remain excluded. No certificate was issued/resent, private credential exposed, browser token extracted, or restart used. | None. Future newly issued public credentials follow the same path; announcement exceptions remain separate from issuance. | 2026-09-06T16:58Z |
| `NC-20260904-003` | Create complete, canonical Sertifier packages for the full MCS/AAMC Practicum graduation credential and its materially distinct partial-completion companion, then prepare duplicate-safe issuance without issuing a recipient | Codex + Claude reviewer | NanoClaw pushed `codex/mcs-sertifier-packages-20260904` @ `86d430d3`; toolbox local commit `a1697715`; live release remains `886e2587` | `complete` | C3 | Provider-read-back full package: approved `MCS Practicum` certificate, 71-hour/41-contact-hour Detail, branded delivery template, Tandem-only graduate badge, and empty public canonical campaign. Provider-read-back partial package: materially distinct certificate, Detail with neutral Mentor Coaching skill, branded email, three dynamic attributes, private empty canonical campaign, no AAMC logo, and no badge. Reviewed prompt/tool/config overlay is byte-identical on the Mini; both campaign fingerprints and `.invalid` dry runs pass live. Bare MCS is held as ambiguous. No recipient, issued credential, email delivery, Heartbeat mutation, historical credential mutation, or AAMC/MCS designation issuance. | None. | 2026-09-05T02:40Z |
| `NC-20260828-003` | Make Gru's canonical-campaign fast path understand the owner's natural multiline `issue coaching tools to` command and reuse a matching pending request | Codex + Claude reviewer | NanoClaw pushed `codex/sertifier-natural-campaign-command-20260828` @ `53d87824`; toolbox local commit `3f7e413`; production mounted files separately hash-bound on release `2773def5` | `complete` | C3 | The owner-authorized Coaching Tools certificate was preflighted against one exact Heartbeat identity, zero prior Sertifier credentials, and matching Pending #003, then issued once through canonical campaign `coaching-tools-mastery@v1`; provider search/get confirm an active public credential and email tracking 2, Pending #003 is archived, and Plutio activity is verified. The deployed parser accepts exact anchored `send` or `issue` with an alias and either `<email>` or `Name <email>`, preserves typed-name casing, uses Heartbeat's exact identity, rejects attachments/multiple recipients/mismatches, and tells Gru to reuse one exact pending script. Sonnet/high R1 found the multi-email and casing defects; both are fixed and covered. Production exact-command parse and Heartbeat preparation return authorized true; the multi-recipient canary fails closed, focused tests pass, channels are connected, and no second send occurred. | None. | 2026-08-29T02:39Z |
| `NC-20260828-002` | Replace Gru Certifier's recipient-named one-off campaigns with one versioned canonical campaign per preset while preserving safe per-recipient issuance | Codex + Claude reviewer | NanoClaw pushed `codex/sertifier-canonical-campaigns-20260828` @ `aa41a134`; toolbox local commit `44c6617`; production files separately hash-bound on live release `2773def5` | `complete` | C3 | Ten empty canonical campaigns were created and exact-fingerprint read back. Presets bind those IDs and unique exact-send aliases; issuance validates campaign state before duplicate search and `Campaign/AddCredentials`, never creates a campaign, never resends a duplicate, and durably holds uncertain reconciliation. Gru's exact `send ai for coaches to EMAIL` path resolves one exact Heartbeat identity and can issue in the same turn only for attribute-free presets. Three Sonnet/high rounds closed every material strategy and implementation finding. Production hashes, merged registry, focused suites, campaign readback 10/10, exact parser, fail-closed identity, canonical AI `.invalid` dry run, and connected Slack/Gmail are verified. No credential, recipient, certificate email, historical campaign edit/delete, Slack canary, Heartbeat write, or daemon restart occurred. | None. | 2026-08-29T01:52Z |
| `NC-20260828-001` | Create catalog-bound Sertifier Details and delivery templates for Coaching Tools Mastery and AI for Coaches, then register both as guarded Gru Certifier presets | Codex + Claude reviewer | NanoClaw `codex/sertifier-practitioner-presets-20260828` from exact live release `2773def5`; toolbox local commit `f01d183`; Practitioner receipt `32fbb49` | `complete` | C3 | Live Sertifier now has two provider-read-back Details and two branded email templates bound to the owner-created designs and Practitioner catalog revision 2. The owner corrected the AI design from 10 to 20 total hours; rendered readback is 20/6/14. Production prompt/preset/tool files hash-match Studio; component tests, registry validation, exact preset discovery, and both `.invalid` dry runs pass on the Mini with no campaign, recipient, credential, or email created. Two Sonnet/high rounds closed all material findings. | None. | 2026-08-29T00:27Z |
| `NC-20260827-004` | Restore the Plutio outbox reaper after an activation canary contaminated the immutable release, then prevent release-root runtime logging | Codex + Claude reviewer | `codex/release-integrity-log-contamination-20260827` from completed parity branch `359e8698`; live release `d11e949b` | `validating` | C4 | Preserved the one-line contaminant outside the release with checksum, restored full-bundle verification, and processed all six outage-created Plutio outbox rows successfully with zero retries/dead letters. The reviewed source disables only the implicit JSONL sink inside an inventory-bearing release and makes inventory errors truthful; operational logging and explicit outside routing remain intact. Claude Sonnet/high found one material runbook wording issue, independently verified and corrected. The dirty primary and unrelated live state remain preserved. | Record review/live evidence and commit/push the corrected source, build and verify a clean immutable release, deploy it through the exact activator, then prove one natural Campanero scheduler success, clean bundle inventory, zero actionable rows, healthy channels, and no release-root log recreation. | 2026-08-28T02:00Z |
...
- Trigger: the owner created an `Our Graduates` community channel and wants one
  short congratulations post with the issued certificate image whenever a
  certificate is newly issued, while retaining the direct recipient message.
- Feasibility evidence: a real reconciled Sertifier credential exposes a
  token-free `certificateImageLink`; the image downloaded as a valid 1584x1224
  PNG. Sertifier also generated a credential-specific PDF link. Heartbeat's
  documented `PUT /threads` API can create a POSTS-channel thread and supports
  iframe embeds, but its request schema has no native file/image attachment
  field. The implementation will use the documented embed field rather than an
  undocumented upload endpoint.
- Safety boundary: announce only `created:true`, active, public credentials
  with an exact certificate image, certificate number, verified recipient, and
  resolved Detail title. Never announce private partial records or
  `already_issued` reconciliations automatically. Resolve exactly one POSTS
  channel named `Our Graduates`, check recent threads for the same verified
  credential URL, and fail closed on drift.
- External boundary: build, provider/channel readback, and dry runs are
  authorized. Do not create a test graduate post or announce an existing
  recipient merely to canary the integration; the first natural authorized
  issuance is the customer-facing outcome proof.
- Review and correction: bounded Sonnet/high R1 found an unbounded duplicate
  URL match and inconsistent registrar-link escaping. Both were corrected. R2
  verified the corrections and requested an explicit unsafe-certificate-number
  regression, which now passes. Production preflight then found the Heartbeat
  component environment needed to be loaded independently of the Sertifier
  environment; the helper and fixture now mirror the live split.
- Deployment: NanoClaw commit `ece2ef6d` is pushed; toolbox commits `d87447f`
  and `07e7582` are local source authority. The reviewed prompt files, merged
  registry entries, and three scripts/tests are installed on `mini-claw.local`
  with byte-identical hashes. Live channel discovery and a real-credential dry
  run pass; the latter downloads a valid 2,138,729-byte PNG and returns
  `created:false`, `willPost:false`. Release `886e2587` remains verified on
  Node 22.23.2, Slack and Gmail are connected, no groups are waiting, and no
  daemon restart was required. Rollback snapshot:
  `.release-backups/NC-20260906-001-20260906T1535Z/`.
- Live outcome: the owner explicitly authorized Michelle Ambrose's existing
  public MCS Practicum credential as the test. Exact Heartbeat identity and one
  matching Sertifier credential were reconciled; the helper created one
  `Our Graduates` thread and read it back, and a second call returned
  `already_announced` without a duplicate. Browser inspection showed the raw
  storage PNG iframe was blocked by Brave, while a certificate-only `srcdoc`
  was stripped. The same post was corrected in place to the branded registrar
  verification-page iframe, which visibly rendered Michelle's certificate and
  verification details. Toolbox `02b1c10` deploys that live-proven embed target
  byte-identically on Studio and Mini; focused/framework tests and a no-post
  dry run with `verified_registrar_page_embed` pass. No certificate was issued
  or resent, no private credential was exposed, and no restart was required.
- Final refinement: the owner moved Michelle's member mention into the sentence
  and rejected both the registrar-page chrome and Heartbeat's external-link
  preview. The public API cannot access the web client's native file uploader,
  so no browser token was extracted. Tandemweb `583b240a0`, independently
  reviewed by Claude Sonnet/high with no material findings, deploys a minimal
  noindex/frame-restricted certificate-card route. The live post now renders
  `@Michelle Ambrose` inline and shows only the complete certificate plus a
  clickable verification link inside the card, with no separate preview.
  Toolbox `c252fd3` is installed hash-identically on Studio/Mini, generates that
  exact structure, and uses exact member mention plus credential title for
  current-format duplicate detection while retaining the legacy URL check.

### NC-20260904-003

- Trigger: the owner completed the MCS/AAMC Practicum graduation certificate
  and asked for the rest of the issue-ready certification package. The owner
  then clarified that the full MCS graduation package is primary and the
  partial-completion record should also be retained.
- Provider state: the full Detail, branded email template, Tandem-only badge,
  and empty public canonical campaign have exact readback IDs. The partial
  certificate, three dynamic attributes, Detail with neutral Mentor Coaching
  skill, branded email template, and empty private canonical campaign also have
  exact readback IDs; it deliberately has no AAMC logo or badge.
- Implementation: isolated toolbox source adds `mcs-practicum` and
  `mcs-practicum-partial`, exact aliases, three partial-record flags, required
  attribute validation, component/campaign fingerprints, and dry-run coverage.
  Gru's prompt lists Foundation, Practicum graduation, and Practicum partial
  separately; bare MCS is ambiguous in both ordinary and exact-send paths.
- Review: Sonnet/high R1 found the missing ordinary-flow mappings. R2 verified
  those mappings and found that the old Foundation fast-path aliases still
  accepted bare `mcs` and `mentor coaching specialization`; both aliases were
  removed and mechanically regression-tested. No third review round is needed
  for that exact alias deletion.
- Verification: toolbox component/canonical suites and registry pass; the
  toolbox framework suite is 65/65; NanoClaw focused prompt tests and typecheck
  pass. The full NanoClaw suite is 3,368 passing/31 skipped with the two
  unchanged CNPC wrapper and stale Trafft failures. No recipient, credential,
  email, Heartbeat write, historical credential mutation, or AAMC/MCS
  designation issuance occurred.
- Deployment: NanoClaw `86d430d3` is pushed and toolbox `a1697715` is committed
  locally. The reviewed Certifier prompt, preset registry, merged toolbox
  registry, issue tool, and focused tests are byte-identical on the Mini. The
  merge preserves the independent `update-credential` tool. Production
  campaign verifier 2/2, both `.invalid` dry runs, registry, and focused suites
  pass. Release `886e2587` remains verified on Node 22.23.2 with Slack/Gmail
  connected, zero active containers, and no waiting groups; no restart was
  required. Rollback snapshot:
  `.release-backups/NC-20260904-003-20260905T0235Z/`.

### NC-20260828-003

- Trigger: the owner typed `issue coaching tools to` followed by a recipient
  name and `<email>` on the next line. The deployed fast parser
  recognized only exact `send <alias> to <email>`, so Gru routed into ordinary
  review, found Pending #003, and later treated the owner's explicit execute
  instruction as an ambiguous cancellation instead of using the canonical
  campaign.
- Live issuance: Heartbeat returned one exact identity and Sertifier returned
  zero prior credentials. The reviewed current issue tool validated
  `coaching-tools-mastery@v1`, issued one credential, and reconciled email
  tracking 2. Search/get readback confirms the active public certificate;
  Pending #003 moved to `completed/`, and the Plutio Activity Log entry was
  created and read back. No retry or second credential occurred.
- Correction boundary: expand only the deterministic explicit grammar for one
  attr-free preset alias, one optional human-readable name, and one exact email;
  continue resolving the authoritative name from Heartbeat. Attachments,
  multiple emails, unknown aliases, required attributes, identity mismatch,
  ambiguous pending scripts, and uncertain provider results remain closed.
- Review: one bounded Sonnet/high round found that the initial bracketed-name
  regex could accept two email segments and captured names from a lowercased
  working string. The parser now counts structural delimiters, permits exactly
  one `@`, parses original-cased text, and rejects angle/email characters in
  the supplied name. Explicit assertion helpers prove both corrected cases.

### NC-20260828-002

- Trigger: the owner questioned Gru's 74 recipient-named MCS campaigns and
  directed design plus implementation of campaign-aware issuance. Live
  content-minimized audit confirms 74 MCS credentials across 74 campaigns,
  exactly one credential per campaign; the account already has 36
  multi-recipient campaigns, up to 58 recipients.
- Provider model: Sertifier's fixed-course guidance is one stored campaign ID
  per course and `Campaign/AddCredentials` for each graduate. The UI's
  `Send to New Recipients` button duplicates a campaign; same-campaign manual
  addition is `Edit Recipients` -> `Add Recipients`.
- Intended command: `send ai for coaches to EMAIL` resolves the full name by
  exact Heartbeat email when possible, then adds the recipient to the preset's
  canonical campaign. Missing/ambiguous identity fails closed. Eligibility,
  prior-issuance search, durable pending state, provider receipt reconciliation,
  and student notification rules remain unchanged.
- Safety boundary: campaign/component/config creation and readback plus
  non-sending `.invalid` dry runs only. No credential, recipient, email,
  historical campaign mutation/deletion, Slack canary, or Heartbeat write.
- Provider result: ten empty canonical campaigns exist and exact-fingerprint
  readback passes 10/10. Existing 74 MCS credentials/campaigns are untouched.
- Implementation result: toolbox syntax, focused canonical/component tests,
  registry, and full 65/65 framework suite pass. NanoClaw prompt contract 4/4,
  typecheck, continuity, and capability checks pass. Full root is 3,367
  passing/31 skipped with the unchanged CNPC wrapper and stale Trafft failures.
- Review result: strategy R1 found eight gaps and R2 closed all eight. Final
  implementation R3 verified all eleven invariants and identified only a
  missing `icf-competencies` ordinary-flow mapping, now fixed with a regression
  assertion. No unresolved material finding remains.

### NC-20260828-001

- Authority: Practitioner catalog revision 2 owns course identity and approved
  hours; Sertifier owns the live design, Detail, template, and issued-credential
  state. The new presets record the exact catalog revision and SHA-256.
- Live boundary: two Details and two email templates were created and read
  back; the owner corrected the AI design from 10 to 20 total hours before
  registration. No campaign, recipient, credential, certificate email, Slack
  canary, Heartbeat record, daemon release, or restart was created.
- Review: Sonnet/high R1 required the known AI design correction and exact
  `Five-Layer Trap` naming. Both were independently verified; R2 returned no
  unresolved material finding.
- Deployment: Studio and Mini prompt/preset/tool hashes match. Production
  component tests and registry validation pass; both `.invalid` preset dry
  runs resolve their exact component IDs with `willSend:false`.
- Future courses: design creation alone is insufficient. Each remaining
  Practitioner course requires its own catalog-bound Detail, branded delivery
  template, preset, prompt mapping, and destination readback under a separate
  task. Every real issuance remains separately gated by the existing exact
  recipient/email, prior-issuance search, pending script, review card, and
  distinct send approval.
...
- Completion: focused/full pinned-Node verification, independent bounded
  Sonnet/high review, exact commit/push/release/deployment, one natural
  Campanero scheduler success, clean full-bundle verification, healthy channels,
  and zero remaining actionable Plutio outbox rows.
