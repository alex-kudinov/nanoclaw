...
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- |
| `NC-20260827-001` | Stop the external watchdog from turning a healthy deployment restart into a self-sustaining stale-Slack-heartbeat restart loop | Codex + Claude reviewer | `codex/watchdog-heartbeat-grace-20260827` from exact live Sales lineage `f52f708f` / continuity `f6e64177` | `ready_for_deploy` | C3 | Repair only `scripts/nanoclaw-watchdog.sh` plus focused tests and continuity/operations docs. A newly started healthy, Slack-connected daemon must receive one bounded first-heartbeat grace window derived from `HEARTBEAT_INTERVAL_MS`; genuinely stale heartbeats after that window, disconnected Slack, unreachable health, frozen process, and container recovery behavior must remain fail-loud. Preserve the heavily dirty primary checkout and all business/provider/customer state. Focused Bash tests pass and bounded Claude Sonnet/high R1 returned `NO MATERIAL FINDINGS`. | Commit and push the reviewed change, deploy only the exact watchdog script after a hash/health preflight, and verify the daemon survives past its first 10-minute heartbeat with a fresh stored heartbeat, no further forced restart, and healthy channels/queues. | 2026-08-27T05:29Z |
...
- Exact defect: the stale-heartbeat check has no deployment-aware startup grace.
  Each stale observation adds two failure points, so two checks are reported as
  "Consecutive failures: 4" even though the daemon health endpoint, Slack,
  Gmail, and queue were healthy.
- Safety boundary: preserve every non-heartbeat recovery path and do not mutate
  provider, customer, database, schedule, credential, message, approval, or
  business state. Production change is limited to the independently reviewed
  external watchdog artifact; no NanoClaw daemon release is required.
- Review: focused Bash syntax/timing tests and diff checks pass. Claude
  Sonnet/high R1 returned `NO MATERIAL FINDINGS`, confirming malformed health
  and disconnected Slack fail closed, the startup grace is strictly narrower
  than the stale threshold, sourced tests do not alter direct execution, and a
  copy-only activation is sufficient for the per-tick launchd job.
...
- Deployment: commit `f52f708f5125fcbe888d1eaf7d72e4ea0ef17d93`
  is pushed and live as the exact immutable release. Its 1,004-file artifact
  ...
  fresh local and Mini extraction verification pass under Node 22.23.2.
  Activation retained rollback plist
  `~/Library/LaunchAgents/com.nanoclaw.plist.rollback-25b44f985ba1-2026-08-27T04-17-56-577Z`.
  WAL-safe SQLite, plist, and three Sales-prompt backups use suffix
  `rollback-NC-20260826-008-f52f708f-20260827T0418Z`; SQLite quick-check is
  `ok`.
- Live readback: release/code root match `f52f708f`; one listener, connected
  Gmail/Slack, zero active/waiting/outgoing work, zero pending send states,
  and exact release/operational prompt hashes pass. Relationship Context query
  remains disabled with zero grants; Trafft, source-enrichment, and client-
  projection workers are healthy. No email, approval, pipeline/database row,
  provider action, permission, schema, model, or query setting changed. A live
  synthetic operator canary was intentionally not created because the host
  cannot truthfully impersonate Alex/Cherie and Ari already has one confirmed
  customer send; the next real qualifying operator reply is the natural
  outcome gate.
