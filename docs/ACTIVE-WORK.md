...
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- |
| `NC-20260824-006` | Release the Community student-lifecycle control plane into four-action shadow capture without changing legacy receivers, Circle, or lifecycle consumers | Codex | `codex/student-lifecycle-community-shadow-20260824` from pushed lifecycle documentation `a06b0549` and exact live code `7364accd`; toolbox companion `0242661` | `validating` | C3 | Implemented exact four-action manifest/catalog, protected 18-registration baseline, provider/reconciliation CLIs, aggregate health, inactive relay/rendering, and guarded Heartbeat/n8n inventory/config/import/activation/rollback tools. Focused 83/83, production-shape integration 4/4, full 3,180/15 skip with unchanged CNPC failure, toolbox 14/14 + 9/9 + 65/65, continuity/build/typecheck/format/diff pass. Bounded Claude R1B returned no material findings. No provider/config/runtime mutation yet; progress API is held on current HTTP 401. | Commit/push the NanoClaw source and docs, build/verify the immutable release, then follow the runbook: backups, inactive n8n import/config, host deployment/config/catalog, transport canary, four exact registration writes/readback. Keep progress/14-day completion waiting and Circle untouched. | 2026-08-24T19:12Z |
...
- Owner authorization: Community shadow stages 2-4 only. Circle, legacy
  cutover, the other seven Heartbeat actions, lifecycle consumers, messages,
  certificates, Encharge, user/group/course mutations, and minion authority
  remain excluded.
- Initial provider manifest: main Community only; `USER_JOIN`, `USER_UPDATE`,
  `GROUP_JOIN`, and `COURSE_COMPLETED`. Existing receivers remain unchanged.
- Current gate: refresh live Community/n8n topology and implement/review all
  required deterministic controls before the first credential, provider, or
  runtime mutation.
- Implemented: exact manifest/catalog and same-key conflict controls, protected
  provider baseline/comparison, registry and reconciliation CLIs, aggregate
  health, inactive relay renderer, and guarded shared Heartbeat/n8n tools.
- Review: the first broad Sonnet/high session was stopped without a verdict at
  the bounded drift threshold. Fresh eight-file R1B completed in four model
  calls and returned `NO MATERIAL FINDINGS`.
- Live read-only findings: legacy 18-row registry remains exact; n8n is 2.9.4
  with 24 workflows / 23 active; the exact group membership snapshot contains
  143 members but no PII output. Cohort progress is fail-closed on HTTP 401.
- Verification: lifecycle 83/83; disposable production-shape PostgreSQL 4/4;
  full root 3,180 pass / 16 skip / unchanged CNPC failure; toolbox 14/14,
  9/9, and core 65/65; typecheck/build/format/continuity/capability/diff pass.
