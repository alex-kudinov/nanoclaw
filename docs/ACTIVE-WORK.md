...
| `NC-20260909-003` | Build the reusable Adyen payment foundation for the first MCS Foundations pilot | Codex + Claude reviewer | `codex/adyen-webhook-live-20260909`; live reviewed immutable release `75d5b6c1`; dirty primaries preserved | `blocked` | C5 | Filter deployed/live-verified; protected backup and rollback retained, exact health/channels/listener plus pure compiled foreign/invalid-HMAC proof pass. All TEST setup/key actions confirmed. New HMAC generated in unsaved disabled Adyen draft; no private install yet, n8n inactive. | Owner must run reviewed local/SSH hidden-input installer because native Terminal automation is denied and tool PTY cannot attach to app terminal. Then verify private config/reload exact release/save/enable TEST delivery and prove durable Tandem payment. No new approval needed for same TEST scope; no LIVE or migrations149-151. | 2026-09-10T12:40Z |
