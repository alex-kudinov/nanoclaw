...
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | --------------------- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- |
| `NC-20260909-003` | Admit Adyen TEST payment notifications into the durable webhook inbox with provider-native HMAC, exact environment/merchant/store/reference allowlists, and no fulfillment side effect | Codex + Claude reviewer | `codex/adyen-webhook-20260909` @ `51185a5db669` (isolated worktree; current dirty primary preserved) | `ready_for_review` | C5 | Dedicated TEST-only Standard webhook route, HMAC verification, minimized durable envelope, deterministic event identity, configuration contract, focused/full tests, architecture/security/continuity evidence, immutable receiver release and readback. No live Adyen, Stripe, payment/refund/payout, customer communication, enrollment/lifecycle projection, bank account, or primary-checkout edit. | Commit the reviewed source, build an exact immutable receiver release, deploy it unconfigured, verify inert health/503 behavior, then configure n8n and the Adyen TEST webhook for one correlated synthetic notification. | 2026-09-10T01:29Z |
...
- Source status: TEST-only HMAC admission, exact merchant/store/reference/event
  allowlists, minimized durable terminal receipt, health visibility, n8n relay
  source, and rollback runbook are implemented in the isolated worktree.
- Verification: Node 22.23.2 typecheck and build pass; focused receiver suite is
  46/46; full suite is 1,655/1,655 after repairing one incomplete logger mock;
  n8n workflow JSON parses. No provider, database, runtime, payment, message, or
  production state has changed.
- Security boundary: the HMAC secret remains host-only; the route rejects live,
  wrong merchant/store/reference/event, bad signatures, unavailable archive,
  and incomplete configuration before acknowledgement. It stores no HMAC or
  shopper fields and never dispatches an agent or fulfillment action.
...
- Next: commit, exact release preparation, deploy receiver inactive, then
  configure the n8n and Adyen TEST endpoints and read back one correlated
