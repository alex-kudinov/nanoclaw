...
| `NC-20260823-006` | Make every host-admitted Stripe payment or refund finish with verified operational receipts or remain as a durable owned Contador exception | Codex | resumed on `codex/contador-natural-payment-proof-20260827` from current evidence `f13959a4`; implementation/live lineage `b131071c` is contained in exact live `6a978328` | `in_progress` | C4 | The 07:54 CDT natural Foundations payment produced case 12 complete in one attempt with exact admission/source/Payment Log/PostgreSQL/roster/final verified receipts and one handled n8n webhook binding. A separate 04:14 CDT payment for the same course from a different customer has no webhook-inbox row, fulfillment case, or PostgreSQL payment row; it is an ingress gap, not a case-ledger failure or evidence to replay. Preserve the admitted-event boundary, keep refund validation separate, and do not manufacture/replay a provider event, repair the missed payment, communicate, or expand product/payer identity without separate authority. | Verify the successful case and missed-ingress distinction against current provider/host state; close the host-admitted case-ledger natural gate if evidence holds, register the missed-ingress payment as separate durable roadmap work, and leave refund closure separately blocked. | 2026-08-27T13:34Z |
