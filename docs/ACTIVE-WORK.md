...
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- |
| `NC-20260907-005` | Prepare the first bounded, unapplied student catalog publication rollout from the accepted NC-004 evidence | Astra strategy/acceptance owner + Codex Sol High worker + Claude reviewer | pushed `codex/catalog-rollout-prep-20260907` from exact clean base `0e6fb605`; claim `70e0bc6e`; closure commit is the commit carrying this row | `complete` | C1 | Reviewed source-only preparation selects inaugural+regular supervision for deterministic schema/generation and exact v1 roster identity parity, preserves active/inactive checkout states, and defines typed source authority, per-consumer coverage, non-atomic activation, and rollback. Alias activation remains held pending actual consumer identity proof; native price semantics and Heartbeat attachment/access remain unverified. Claude R1's one evidence-citation finding is corrected; R2 returned no material findings. Astra accepted the packet and exact holds. No canonical catalog/binding application, production code, provider/student/access/roster/payment/capacity/schedule/message write, migration, deployment, publication enforcement, primary `.program` mutation, or customer/learner/financial record population read. | None for repository preparation. A new implementation authorization may code from then-current production lineages. Astra separately owns primary `.program` reconciliation and handoff. | 2026-09-07T21:12Z |
| `NC-20260907-004` | Execute the reviewed read-only reconciliation of every current active student product and its typed provider, fulfillment, schedule, and catalog relationships | Astra strategy/acceptance owner + Codex Sol High worker + Claude reviewer | pushed `codex/student-catalog-reconciliation-20260907` @ evidence commit `7de990e3` from base `40f0d364` | `complete` | C1 | Sanitized evidence covers 56 source/disposition records: 47 checkout records, 8 accepted catalog offer keys, 9 source-only counterparts, and 0 canonical orders. All 37 active checkout entries have one treatment/disposition; price/account/party/schedule axes remain scoped; holds are proposed and unapplied. Astra independently accepted the full draft. Claude R1 findings were corrected and R2 returned no material findings. No canonical catalog, provider, student, roster, payment, schedule, capacity, migration, runtime, service, communication, primary checkout, or `.program` write. | None for repository evidence. Astra separately reconciles the primary program delta and saves the semantic handoff. Owner decisions and any catalog application, publication, rollout, or production pilot remain separately gated. | 2026-09-07T20:16Z |
| `NC-20260907-003` | Adopt durable Astra/Sol/Claude model routing for NanoClaw and assemble the owner-directed student lifecycle strategy plan | Astra strategy owner + Codex Sol High worker + Claude reviewer | pushed `codex/lifecycle-strategy-routing-20260907`; implementation `807808ae` from base `c9c65dfb`; primary install separately read back | `complete` | C1 | Project-local model-routing policy/config and both instruction bindings reviewed; owner-readable lifecycle strategy, phased plan, decision register, exact portfolio mapping, and first read-only reconciliation packet assembled. R1 and load-bearing R2 have no material findings. Astra applied the four candidate gaps at program r247 without activation and accepted the exact four-path primary install, which is applied/read-back. No runtime, migration, provider, student, payment, capacity, message, application, deployment, or production action. | None for this strategy/routing slice. Astra retains final `.program` continuity/handoff; the registered read-only source-reconciliation candidate is the next planned dependency but is not activated by this task. | 2026-09-07T15:43Z |
| `NC-20260907-001` | Repair supervision registration and replace name-dependent product identity with a catalog-bound bridge | Codex + Claude reviewer | `codex/product-identity-20260907`; deployed `138e43ab` | `complete` | C4 | Case 63 complete, CSS row 6 and accounting verified; supervision identity bridge deployed; two Sonnet reviews closed. Broader product coverage and supervision capacity are separate canonical follow-ups. | None for this bounded repair/bridge. See Company OS product-identity coverage and supervision-capacity candidates for remaining scope. | 2026-09-07T14:25Z |
...
- Worker routing: `/root/catalog_rollout_packet`, explicitly dispatched as
  `gpt-5.6-sol` with `high` reasoning and no recursive Codex delegation. Astra
  owns strategy, acceptance, primary `.program` state, and the final program
  handoff; this worker owns the isolated preparation branch.
- Authority and binding: owner authorization is recorded only in the dirty
  primary at
  `.program/decisions/decision-student-catalog-rollout-preparation-2026-09-07.json`.
  Astra claimed `work:student-product-identity-rollout-preparation` under this
  task and retained six continuity commitments. The worker reads that binding
  and does not edit `.program`.
- Base and overlap: exact clean base
  `0e6fb6050a304689561234d8fdbb0282674ae5bd` on worktree
  `/Users/xbohdpukc/dev/NanoClaw-catalog-rollout-prep-20260907` and branch
  `codex/catalog-rollout-prep-20260907`. The primary checkout is extremely dirty
  and excluded. No live-code fetch or wholesale merge enters this documentation
  branch.
- Accepted boundary: use the completed NC-004 audit to prepare a concrete,
  reviewed, unapplied publication contract and exact first application packet.
  Resolve only supported technical gaps through local authority or documented
  read-only operations, preserve scoped uncertainty, and do not repeat the full
  source inventory.
- First-slice direction: compile and validate the already verified supervision
  family into deterministic pinned consumer outputs while preserving existing
  offer activation state and processor behavior. Validate the configured
  `tandem` Stripe alias against its native account identity. Website checkout
  remains independent from a live NanoClaw lookup, and later implementation
  must start from then-current production lineage.
- Explicit exclusions: no production code, canonical facts/catalog/binding
  application, provider product/price/user/access change, roster/payment/
  capacity/schedule/message change, migration, deployment, publication
  enforcement, customer/learner/financial record population read, or primary
  `.program` mutation. MCS, locale, and service-policy holds do not block an
  independently proved supervision publication-tooling slice.
- Review and closure: one fresh bounded Claude Sonnet/high review through
  `work-with-claude`; fix verified material findings and rerun focused JSON,
  cross-link, coverage, privacy, diff, and pinned-Node continuity checks. Astra
  accepts the resulting packet before the final scoped source/continuity commit
  and push.
- Review result: R1 found one material omission: the first fixture's supervision
  Heartbeat object-existence claim lacked exact evidence provenance. The
  correction records the accepted NC-004 snapshot paths, recomputed digests,
  observation time, object IDs, existence-only statuses, attachment false, and a
  current-lineage refresh/acceptance gate. Narrow R2 returned
  `NO MATERIAL FINDINGS`.
- Verification: JSON, cross-link, privacy, no-`.program`, diff, exact source
  digest/object, current resolver 10/10, and pinned Node 22.23.2 continuity gates
  pass. No schema, fixture, generator, consumer output, runtime, or external
  state was created or activated.
- Acceptance: Astra independently recomputed the corrected snapshot hashes,
  checked exact native records and supervision fixture facts, confirmed the
  deployed-consumer alias remains unverified, read both review responses, and
  accepted the packet and activation holds. Astra authorized the final scoped
  source/continuity commit and push and retains primary `.program`/handoff
  closure.

### NC-20260907-004

- Worker routing: `/root/catalog_reconciliation`, explicitly dispatched as
  `gpt-5.6-sol` with `high` reasoning and no recursive Codex delegation. Astra
  owns strategy, acceptance, primary `.program` state, and the final program
  handoff; the worker owns this isolated repository evidence branch.
- Authority and binding: owner authorization is recorded in the primary at
  `.program/decisions/decision-student-catalog-reconciliation-2026-09-07.json`.
  Astra claimed `work:student-product-catalog-source-reconciliation` under this
  task and attached six open commitments; current primary revision 253 validates
  and retains the claim. The worker reads that binding and does not edit
  `.program`.
- Base and overlap: exact clean source base `40f0d364b3d7edcfcf87aca24af92984f866ba16`
  on the dedicated worktree and branch. The pre-existing untracked execution
  packet is Astra-owned authority and is preserved. The extremely dirty primary
  checkout and separately reviewed undeployed admission/store branch are not
  edit targets. Current Sales work is complete on release `1449812c1e71`; this
  task will verify relevant source lineage read-only.
- Accepted boundary: execute
  `docs/work-packets/NC-20260907-003-READONLY-CATALOG-RECONCILIATION.md` with the
  clarifications in `docs/work-packets/NC-20260907-004-EXECUTION.md`; refresh all
  active first-party checkout, locale, sale-mode, provider, curriculum, roster,
  and schedule populations; distinguish source verification, disposition,
  runtime migration coverage, and publication eligibility; retain ambiguity as
  an owned safe hold.
- Explicit exclusions: no canonical catalog or binding application, private
  Heartbeat route or browser export, provider/product/price/student/access/
  roster/payment/class/capacity/schedule/message write, migration, runtime or
  service release/restart, global/trust change, primary checkout edit, or
  `.program` mutation. Final evidence commit/push waits for Astra acceptance;
  this initial repository claim is authorized for immediate push.
- Current evidence: 56 source/disposition records remain distinct from eight
  accepted offer keys and zero canonical orders. The 37 active checkout records
  each have exactly one treatment and disposition. Forty-two checkout price
  declarations and eight unscoped offer price declarations are preserved; only
  three native account/product/default-price relationships are verified. All 56
  records preserve payer, purchaser, sponsor, participant, and student as
  distinct roles without reading identities or assuming sameness.
- Publishing/source boundary: a site-wide source scan found static page
  references for 33/37 active checkout slugs; four evaluation-training slugs
  were not found. French Foundations remains a source/native learning
  counterpart with no inferred sellable offer. Two native service-container IDs
  were not found in scanned course manifests; another allowance, procedure,
  schedule, assessment, or managed-manifest source may be appropriate.
- Review: bounded Sonnet/high R1 found missing payer/participant evidence in the
  review packet and one nine-family packet-boundary omission. Both were corrected
  and independently verified. Narrow R2 returned `NO MATERIAL FINDINGS`. All
  provider and publication holds remain proposals only; no grandfathering,
  allowlist, or enforcement was conferred.
- Repository closure: Astra independently accepted the full draft under
  `receipt:astra-independent-nc004-counts-and-scope-pass`. Evidence commit
  `7de990e3` is pushed on the task branch. Primary `.program` reconciliation and
  semantic handoff remain Astra-owned and were not performed by the worker.

### NC-20260907-003

- Worker routing: `/root/lifecycle_policy_plan`, explicitly dispatched as
  `gpt-5.6-sol` with `high` reasoning and `fork_turns: "none"`; no recursive
  Codex delegation. Astra owns strategy, the execution packet, the forthcoming
  strategy brief, and all primary `.program` state/decisions/handoff.
- Base and overlap: clean task-owned source base `c9c65dfb`; the pre-existing
  untracked `docs/work-packets/NC-20260907-003-EXECUTION.md` is Astra-owned and
  must remain unedited. Concurrent `NC-20260907-002` Sales work uses a separate
  older-base worktree and owns Sales files. The dirty primary checkout remains
  untouched until reviewed acceptance permits a narrow routing-policy install.
- Accepted boundary: adopt a project-local NanoClaw routing policy from the
  hash-verified CoachGrader proposal, set supported project defaults to Sol/high
  with a two-child ceiling, bind both client instruction surfaces, reconcile
  lifecycle source authority into an implementation-ready plan, and preserve
  Claude Sonnet/high review plus every existing safety/release gate.
- Explicit exclusions: no second `.program` claim, global configuration or
  trust change, fresh-task-default claim without observation, application code,
  migration, provider/student/payment/capacity write, communication, runtime
  deployment, restart, or production action.
- Drafted artifacts: project-local routing policy/config and both client
  bindings; lifecycle strategy with phases 0-7; open-decision register; three
  proposed candidate deltas without `.program` mutation; and one planned,
  unexecuted read-only catalog reconciliation packet. The accepted capacity
  simple-sync decision supersedes older checkout reservations/short-lived holds.
- Source reconciliation: current live product-identity lineage is `c9c65dfb`
  after deployed `138e43ab`; reviewed local enrollment admission is `8b1ffbb9`
  from common base `f5adc8cc`. Their application source paths do not overlap,
  while Active Work, Project Map, and changelog require semantic reconciliation.
- Independent review: bounded Claude Sonnet/high R1 read only the six allowed
  artifacts and returned `NO MATERIAL FINDINGS`. The earlier
  zero-model-call attempt was blocked by the repository 200-line CLAUDE hook;
  the canonical binding was compressed to exactly 200 lines before review.
  Successful usage: four model calls, 67,019 cache-create, 122,062 cache-read,
  14,995 output, and 67,021 maximum context tokens; no warning threshold fired.
- Astra then required one load-bearing correction: provider delivery must be
  locally ready before routing a paid event, and canonical plus projection
  activation/readback must occur as separate evidence milestones in the same
  authorized rollout before financial-family expansion. The plan now proposes a
  fourth projection-foundation candidate (21 total) and required narrow R2.
  R1's prose also overstated that Phase 3 excluded replay proof; the reviewed
  table correctly required disposable replay and the plan continues to do so.
- Narrow Claude Sonnet/high R2 verified the projection-readiness correction,
  acyclic foundation → pilot → live reconciliation graph, same-rollout separate
  evidence milestones, and pre-expansion end-to-end acceptance with
  `NO MATERIAL FINDINGS`. R2 usage: four calls, 52,991 cache-create, 126,367
  cache-read, 10,155 output, and 59,765 maximum context tokens; no warning.
- Primary install is applied after exact precondition match and Astra acceptance.
  All four post-install hashes match the proposal; policy/config are byte-identical
  and the primary's unrelated CLAUDE changes remain. Primary's old checkout lacks
  the pinned launcher/capability script and retains unrelated continuity debt;
  clean-worktree checks pass and no adjacent primary repair was attempted.
- Astra separately applied the reviewed four-candidate delta at Company OS r247.
  All four remain candidates, the graph is acyclic, and 21 candidates remain an
  honest pressure signal. Sol did not modify `.program`.
...
- Two Sonnet/high reviews closed; Codex independently fixed the unread-PI
  catch path. Release 138e43ab is live, healthy and its installed bindings and
  duplicate no-op were verified. CSS and Product Map rendered readably in
  Google Sheets at 100% zoom; native cell values read back. Broader coverage
  and supervision capacity retain separate program ownership.
...
- Predecessor reconciliation: capacity extends the entitlement catalog and
  ...
  a competing model.
- Concurrent work: the first state update correctly failed on stale revision
  173. Reorientation at revision 176 found active
