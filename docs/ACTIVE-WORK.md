...
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | --------------------- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- |
| `NC-20260907-003` | Make Gru-certifier complete every newly issued public credential through API-only Heartbeat DM and Our Graduates announcement, with durable idempotent receipts and no browser dependency | Codex Astra + Sol implementer + Claude reviewer | NanoClaw `codex/certifier-api-followthrough` @ `ff8e959b` (pushed); toolbox `codex/heartbeat-certificate-followthrough` @ `ce00285` (local repository has no remote); production prompt/tools/image deployed | `complete` | C4 | Shared Heartbeat direct-chat/message operation, certificate follow-through orchestration, Gru prompt/workflow packaging, authenticated repair ingress, jq-bearing agent image, focused tests, exact prompt/tool sync, certifier session reset, and live repair of the three owner-authorized Practicum credentials. LinkedIn is excluded. All unrelated dirty primary changes and existing issuance/duplicate/approval gates were preserved. | Observe the next naturally authorized new issuance confirm the already-tested combined wrapper in production; no implementation or repair work remains. | 2026-09-08T00:25Z |
...
- Outcome: Gru completes every newly issued public certificate with an
  API-verified Heartbeat DM and an API-verified `Our Graduates` announcement,
  without Browser or Computer Use.
- Trigger: three approved Practicum credentials were issued and archived by
  Gru, but the resumed root session never attempted either follow-through
  action. The direct-message capability did not exist; the community action
  existed only as a newly added prompt step that the old session had not read.
- Source boundaries: isolated NanoClaw and toolbox worktrees; exact operational
  Sertifier predecessor files imported as source, no secrets; LinkedIn paused
  and excluded; unrelated primary dirty changes preserved.
- Design: one Gru pending-script wrapper returns issuance plus two independent
  Heartbeat receipts. DMs resolve the exact recipient and Administrator sender,
  persist the registrar-URL intent before one send, read back the stored
  message, and refuse blind retry. Community announcements retain exact channel,
  credential, image, mention, duplicate, and thread-readback guards. Private
  and already-issued credentials do not auto-follow-through; explicit repair
  never reissues.
- Review and source: Claude Sonnet/high found two material identity/dedupe
  defects; both were fixed, regression-tested, and the final exact-match review
  reported no unresolved material finding. NanoClaw is pushed at `ff8e959b`;
  the local-only toolbox branch is at `ce00285`.
- Verification: all Heartbeat/Sertifier shell suites, strict registries, shell
  syntax, the five-case Nano prompt/container contract, typecheck, continuity,
  and diff checks pass. The unrelated `chaos-reconciler` full-suite import
  failure reproduces on untouched base `51185a5d`. Production hashes match the
  reviewed tools/prompts; the rebuilt `nanoclaw-agent:latest` reports `jq-1.6`.
- Live outcome: Gru processed all three owner-authorized Practicum repairs. Each
  exact DM body, Alex sender identity, registrar URL, and message ID were read
  back from Heartbeat; each exact `Our Graduates` thread was independently
  reconciled. No certificate was reissued and no duplicate announcement was
  created. Three initial empty provider messages exposed by the canary remain
  immutable in Heartbeat; their uncertain receipts were preserved under the
  dated `reconciled-empty` archive before the corrected messages were sent.
