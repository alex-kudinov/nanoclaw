...
| `NC-20260906-004` | Replace the planned real-time reservation cutover with simple committed-seat status synchronization for cached Academy pages | Codex + Claude reviewer | `codex/academy-capacity-simple-sync-20260906` from `0102dfb5`; Tandemweb `codex/academy-capacity-simple-sync-20260906` from `56bb8f6ee`; Company OS `work:academy-capacity-simple-status-sync` active r220 | `in_progress` | C5 | Owner-accepted simplified Gate E: no checkout reservations or live NanoClaw dependency; update internal inventory on successful web sales and explicit invoice/manual promises; add versioned capacity adjustment; preserve atomic transfer; reconcile daily; publish only available/sold-out threshold changes to a signed local WordPress status registry; purge/prewarm only affected pages. Preserve current ACC September 7 and MCS Friday sold out, MCS Thursday available, and Rita settled in January. No automatic waitlist contact, refund/payment execution, or full roster/provider/assignment authority cutover. | Run one bounded Claude Sonnet/high architecture round, record the converged implementation contract, then implement and verify both isolated worktrees before independent code review and established deployment/live cache verification. | 2026-09-06T22:12Z |
...
### NC-20260906-004 — Academy capacity status sync
