...
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- |
| `NC-20260821-005` | Reconcile Sales conversations, unsigned proposals, and overdue receivables into one read-only durable follow-up shadow and an operator-reviewable disposition backlog without generating customer work | Codex                              | `codex/nc-20260821-005-followup-shadow` @ `403513b2` (isolated worktree)                                       | `ready_for_deploy`    | C3    | Default-dry-run source/report candidate is committed across exact Sales entries, Plutio proposals/invoices, legacy proposal rows, approved-email actions, and the admin-only case store. It fails closed on source failure, duplicate entry, unbound thread, missing owner, and unreconciled payment; unchanged health is aggregate-only. Exact Node 22.23.2 runtime doctor is aligned and the 731-test email-critical plus 43-test runner gates pass. Keep the legacy task paused and proposal loop unchanged. No Slack card, draft, approval, pipeline/Plutio/payment mutation, schedule wiring, projection apply, or customer send.                                                                                                                                                                                                                             | Build and independently verify the immutable dark release, deploy the exact commit, then execute one full read-only Mini dry-run. Review the content-free snapshot before any separately confirmed projection.                                                                                                                                                              | 2026-08-21T21:53Z |
| `NC-20260821-004` | Make Node 22.23.2 the one executable contract for NanoClaw development, CI, container builds, releases, and production instead of repeatedly running project commands under ambient Node 20/26         | Codex                              | `codex/nc-20260821-004-runtime-contract` @ `68af34bf` (isolated worktree)                                      | `ready_for_review`    | C2    | Exact 22.23.2 now binds both package engines/engine-strict installs, five GitHub setup steps, CI push/PR, the exact agent image, release inputs, startup/health, and a repository launcher. Ambient `npm run runtime:doctor` hands Node 26.7.0 to Node 22.23.2/ABI 127 without changing global Node. Typecheck/build/continuity and 29 focused tests pass; runner is 43/43; full root is 2,919 pass with only the unchanged CNPC wrapper assertion failing. The official image registry exposes `22.23.2-slim`; the direct Docker CLI lookup timed out locally. The all-skill validator was interrupted after it reset the isolated uncommitted patch and exposed unrelated existing add-gmail/add-slack drift.                                                                                                                                                                                                                                      | Begin `NC-20260821-005` for the separate read-only revenue-follow-up source shadow. Merge/push and any agent-image activation remain separate release actions; no production restart or global Node change is required.                                                                                                                                                           | 2026-08-21T21:22Z |
...
- Current read evidence: the production Mini has 659 email interactions, 585
  thread IDs, and zero `pipeline_entry_id` bindings. The exact candidate query
  yields 164 entry rows across 143 parties; 42 active rows are ambiguous, 158
  carry a party-global thread, and zero are entry-bound. `party_relationships`
  has zero active rows; current party roles express buyer status, not assigned
  commercial ownership. Invoice arithmetic is intentionally not treated as a
  transaction receipt. Those facts block rather than authorize work.
- Local verification: exact Node 22.23.2 typecheck and 63 focused tests pass;
  ...
  toolbox Plutio read currently refuses on an unrelated environment-file shell
  syntax error, and the Studio has no production PostgreSQL route by design.
  The Mini aggregate database read succeeded. Commit `403513b2` also passes the
  731-test email-critical gate and all 43 independent runner tests; immutable
  release build and the full Mini dry-run are next.
...
- Scope/exclusions: repository/CI/container/runtime-contract files only. No
  production restart, agent-image build/activation, database, schedule,
  channel, customer action, or global Node mutation is authorized by this
  task.
