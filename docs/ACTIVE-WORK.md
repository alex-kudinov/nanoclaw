...
| `NC-20260823-006` | Make every host-admitted Stripe payment or refund finish with verified operational receipts or remain as a durable owned Contador exception | Codex + Claude reviewer | resumed/pushed `codex/contador-natural-payment-proof-20260827` from current evidence `f13959a4`; exact live `6a978328`; concurrent watchdog source remains separately deployed | `in_progress` | C4 | Owner-directed correction after natural proof: accept every legitimate Stripe charge-object alias shape supported by the provider contract; guarantee processor/finalization errors cannot strand expired `processing`; and terminalize exactly the two known stale cases host-side as owned exceptions without invoking Stripe, Sheets, roster, or payment processors. Migration/rollback, explicit-case guarded repair, privacy/idempotency, independent Sonnet/high review, exact-lineage release, backup, deployment, and live terminal-state proof are required. Preserve the successful natural case, two needs-product, one write-failed, one refund needs-review, and the separately recorded missed-ingress payment. No provider/event replay, missed-payment repair, refund, customer communication, product/payer redesign, accounting, or ingress-parity activation. | Record the bounded correction decision/program claim; merge the current external-watchdog source lineage without changing its installed runtime; implement migration 139 plus exact guarded terminalization; test/review/correct; then build/back up/deploy and verify zero processing cases with no external replay. | 2026-08-27T17:57Z |
