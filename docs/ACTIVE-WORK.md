...
| `NC-20260909-003` | Build the reusable Adyen payment foundation for the first MCS Foundations pilot | Codex + Claude reviewer | `codex/adyen-webhook-live-20260909`; live reviewed immutable release `75d5b6c1`; dirty primaries preserved | `blocked` | C5 | SSH provisioning mode reviewed and tested:10 Python fixtures, full3863pass/32skip/same3 baseline failures. Studio/Mini SSH verified; native Terminal control is unnecessary. Existing TEST authorization is complete. Adyen session expired and browser clipboard is empty; no key installed. | Owner signs into Adyen TEST only; then generate/copy the approved key, install privately through reviewed SSH stdin mode without key output/arguments, reload exact receiver, and finish provider/n8n TEST round trip. Do not ask owner to paste keys or run terminal commands. | 2026-09-10T14:38Z |
