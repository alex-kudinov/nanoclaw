...
| `NC-20260823-006` | Make every host-admitted Stripe payment or refund finish with verified operational receipts or remain as a durable owned Contador exception | Codex + Claude reviewer | pushed `codex/contador-natural-payment-proof-20260827`; exact live base `6a978328`; merged watchdog source `3423ca7a`; reviewed correction pending commit | `ready_for_deploy` | C4 | Migration/rollback 139 adds typed `ch_`/`py_` charge aliases; processor/contract failures return handled durable `write_failed`; exact terminal-held cases never reopen or launch external processing; and the compiled default-dry-run terminalizer requires exact case/version/attempt plus expired lease, preserves source time, and has no external client. It may terminalize only the two known stale cases after backup/migration/release. Disposable PostgreSQL, focused 125/125, format/typecheck/build/continuity, full 3,347 pass/31 skip with the sole unchanged CNPC assertion, and runner 45/45 pass. Claude R1 found terminalized-case replay; corrected store/host short circuits and regressions received R2 `NO MATERIAL FINDINGS`. Preserve six complete, two needs-product, one existing write-failed, one refund needs-review, and the missed-ingress candidate. No provider/event replay, missed-payment repair, refund, communication, product/payer redesign, accounting, or ingress-parity activation. | Commit/push, build/verify immutable exact-lineage release, back up PostgreSQL/SQLite/plists/release and case fingerprints, apply migration 139, activate, dry-run/apply only exact stale specs, then prove zero processing, receipt increments, replay no-op, channels/queues/watchdog, and non-interference. | 2026-08-27T18:27Z |
