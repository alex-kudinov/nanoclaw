...
| P2.12 functional closure             | partial        | Procurement, CNPC, grader, Sales, and Contador now have stronger closed-loop pieces. `NC-20260820-002` locally defines program-facts completion as one exact clean-detector receipt, with immediate owner-review blocking and explicit recurrence; it is not yet deployed. Every remaining process still needs an explicit completion definition, receipt, exception path, and outcome check.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
R4 detector-to-remediation checkpoint: `NC-20260820-002` locally implements
...
loop can therefore pick the source exception up on their next complete tick.
Unchanged findings are deduplicated, changed evidence re-alerts, a clean rerun
is the only completion receipt, and recurrence reopens the same item. Migration
125 is unapplied, active mode defaults off, and the installed job retains the
legacy notify-only entrypoint until a separate promotion/deployment milestone.
No automatic fact, knowledge, product, website, email, or source correction is
authorized.
...
30. `OPS-003` — convert deterministic detector findings from notify-only
    ...
    Slack text. `NC-20260820-002` implements the first local default-off
    `program_facts_drift` slice with replay, changed-finding, clean-close, and
    recurrence semantics. Migration 125, runtime activation, a live natural
    finding, owner resolution, clean-rerun closure, and broader detector
    coverage remain separately gated.
