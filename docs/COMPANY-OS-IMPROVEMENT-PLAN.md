...
| P1.4 three service indicators        | partial        | Exact release `a02abaca` under `NC-20260820-005` live-proves the standalone aggregate-only Sales email baseline from exact `accepted` and `outcome_validated` ledger events. Its first 30-day read is 15 accepted, 13 completed, two incomplete, and 86.67% completion; accepted-to-outcome p50 is 29m01.725s, p95 is 6h16m18.994s, and maximum is 9h25m12.618s across 13 samples. Protected fingerprints are unchanged. `NC-20260820-006` now defines a dark, admin-only, append-only outcome-quality receipt bound to each exact `external_acknowledged` delivery and makes the third rate coverage-aware: it remains unavailable until every customer-visible outcome has one current clean/defect/reversal assessment. Revisions supersede rather than rewrite history. Internal exceptions remain ineligible, and no producer, classifier, message, remediation, threshold/SLO, or agent authority is added. Migration/deployment, natural assessment coverage, and a statistically useful time series remain open before setting an objective or calling all three indicators operational. |
