...
| P0.1 runtime contract                | deployed_unverified | `NC-20260821-004` closes the declaration and host-runtime drift on exact Node 22.23.2: root and runner engine contracts, engine-strict installs, every GitHub Node setup, the agent-image base, repository-owned command handoff, release manifests, startup refusal, production launchd, and health now agree. The replacement minion image built from the exact release after retaining the prior Node-22.22.0 image for rollback; one natural minion launch must still report 22.23.2 before full implementation credit. The workstation may retain global Node 26 for other projects; it is not a NanoClaw execution path.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
- `NC-20260821-004` closes the known NanoClaw Node drift on exact 22.23.2.
  Package engines, engine-strict installation, all GitHub Node setup, the
  agent image, release manifests, production startup, and health now agree.
- The workstation's ambient shell can remain on Node 26 for other projects;
  repository npm scripts hand off through the exact installed pin instead of
  executing NanoClaw or its native modules under that ambient runtime.
- CI runs on pull requests and pushes to `main`; the continuity gate rejects
  future package/workflow/image drift from `.nvmrc`.
...
**Resolved problem:** NanoClaw previously mixed a Node 22 repository pin, Node
20 workflow jobs, an ambient Node 26 authoring shell, and an earlier Node 25
production process. Native bindings failed under that drift.
...
**Implementation checkpoint (`NC-20260821-004`):** exact Node 22.23.2 is now
the enforced project contract across root and runner packages, engine-strict
dependency installation, every GitHub `setup-node` step, exact agent image,
repository-owned local command handoff, release build/manifest verification,
production startup refusal, launchd, and health. `npm run runtime:doctor`
reports the non-secret runtime and ABI contract. The global workstation Node
is deliberately not changed. The exact replacement minion image has been built
and the prior Node-22.22.0 image retained for rollback; the remaining evidence
gate is one natural launch reporting Node 22.23.2 from the replacement image.
...
unwired. `NC-20260821-005` deploys and live-verifies the next default-dry-run
source/report milestone: exact source failures, duplicate entries,
...
aggregate unchanged health. The full content-free Mini scan completed with
zero source errors across 189 observations: zero ready, 165 blocked, 12
waiting, and 12 terminal. The source adapter also failed closed on and then
removed an N+1 Plutio identity-read defect before the successful rerun. No
projection or customer work was generated. Exact thread lineage, assigned
commercial ownership, and payment receipts are now the evidence-backed repair
