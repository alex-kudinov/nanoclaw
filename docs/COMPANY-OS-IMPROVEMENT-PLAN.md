...
| P1.7 ingestion loss windows          | partial        | Webhook inbox/reaper patterns are durable. `NC-20260817-003` implements the generic source-watermark contract: monotonic CAS advance, complete accepted/rejected accounting, gap freeze, exact-gap reconciliation, append-only history, and zero agent grants pass disposable PostgreSQL rehearsal. `NC-20260817-004` applies migration 122 and deploys it dark with zero source/event/state rows and no runtime import. `NC-20260817-005` adds the proposal-only inbound-Gmail full-snapshot proof. `NC-20260817-006` adds an unwired exact read-only Google wrapper plus unapplied migration 123's resumable content-free shadow ledger; disposable PostgreSQL completes 10,001 synthetic candidates over 21 pages with stable replay, guarded rollback, and admin-only grants. `NC-20260817-007` deploys exact release `de815e1d` with those bytes installed but still unwired and migration 123 absent. `NC-20260817-008/009` add, deploy, and naturally prove append-only content-free receipts for every current Gmail terminal path plus cursor holdback. `NC-20260817-010` locally adds a default-off retained-host coverage auditor that double-reads read-only SQLite/PostgreSQL evidence, separates terminal, recoverable, and unresolved retained IDs, emits aggregates/fingerprints only, and explicitly does not claim Gmail mailbox completeness; its production dry run is pending. Source/bootstrap, migration 123, live shadow proof, Gmail-only unknown coverage, watermark-age alerts, natural-404 recovery, and label-poll expiry remain open. |
...
R3 retained-host coverage checkpoint: `NC-20260817-010` locally adds a
...
remain outside this gate. Production aggregate proof is still pending, and no
receipt, cursor, source, shadow, task, message, or action authority changes.
