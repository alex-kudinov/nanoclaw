...
An alert is not dispatch. `NC-20260820-003` locally closes that process gap for
...
or work-state authority. Deployment and outcome validation remain separate.
...
28. `UX-001` — build the smallest ledger-backed exception brief/work panel for
    ...
    `NC-20260820-003` locally adds source-bound Chief work packets and exact
    Sales-email hydration without Gmail search; deployment and live proof remain
    pending. Later source resolution, dead-letter and wider-process coverage,
