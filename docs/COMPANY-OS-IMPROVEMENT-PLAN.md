...
| P1.7 ingestion loss windows          | partial        | Webhook inbox/reaper patterns are durable. `NC-20260817-003` implements the generic source-watermark contract: monotonic CAS advance, complete accepted/rejected accounting, gap freeze, exact-gap reconciliation, append-only history, and zero agent grants pass disposable PostgreSQL rehearsal. `NC-20260817-004` applies migration 122 and deploys it dark with zero source/event/state rows and no runtime import. `NC-20260817-005` adds the proposal-only inbound-Gmail full-snapshot proof. `NC-20260817-006` adds an unwired exact read-only Google wrapper plus unapplied migration 123's resumable content-free shadow ledger; disposable PostgreSQL completes 10,001 synthetic candidates over 21 pages with stable replay, guarded rollback, and admin-only grants. `NC-20260817-007` deploys exact release `de815e1d` with those bytes installed but still unwired and migration 123 absent. `NC-20260817-008` locally adds append-only content-free receipts for every current Gmail terminal path, ordinary-row plus routed-marker retry convergence, history-cursor hold on any unaccounted candidate, and non-terminal page-20 refusal. `NC-20260817-009` finds 21 routed and 36 unresolved legacy staging rows, isolates the unresolved candidates in cursorless scans, creates a WAL-safe production backup, and deploys exact release `263ac7c4` with the additive receipt table and two append-only triggers live. Service/channel/queue and non-interference checks pass, but one successful safety poll returned zero Gmail candidates, so no natural receipt exists and producer behavior remains `deployed_unverified`. Historical unknown accounting, first natural receipt proof, source/bootstrap and live shadow proof, watermark-age alerts, natural-404 recovery, and label-poll expiry remain open. |
...
receipt was naturally created and the checkpoint remains
`deployed_unverified`. The current 404 reset, migration 123, reconciliation
source/runtime, tasks, messages, and action authority remain unchanged.
