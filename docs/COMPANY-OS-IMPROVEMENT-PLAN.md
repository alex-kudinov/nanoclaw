...
| P1.7 ingestion loss windows          | partial        | Webhook inbox/reaper patterns are durable. `NC-20260817-003` implements the generic source-watermark contract: monotonic CAS advance, complete accepted/rejected accounting, gap freeze, exact-gap reconciliation, append-only history, and zero agent grants pass disposable PostgreSQL rehearsal. `NC-20260817-004` applies migration 122 and deploys it dark with zero source/event/state rows and no runtime import. `NC-20260817-005` adds the proposal-only inbound-Gmail full-snapshot proof. `NC-20260817-006` adds an unwired exact read-only Google wrapper plus migration 123's resumable content-free shadow ledger; disposable PostgreSQL completes 10,001 synthetic candidates over 21 pages with stable replay, guarded rollback, and admin-only grants. `NC-20260817-007` deploys exact release `de815e1d` with those bytes installed but still unwired and migration 123 absent. `NC-20260817-008/009` add, deploy, and naturally prove append-only content-free receipts for every current Gmail terminal path plus cursor holdback. `NC-20260817-010` completes the default-off retained-host audit: production double-read of read-only SQLite/PostgreSQL evidence accounts for 3,041 IDs as 23 terminal, 1,675 recoverable, and 1,343 unknown with identical pre/post fingerprints and no Gmail call; mailbox completeness is still not claimed. `NC-20260817-013` applies migration 123 dark after a verified backup; all three live tables are empty/admin-only and the runtime remains unwired. `NC-20260818-001` adds the default-refuse one-source bootstrap CLI and live-proves exactly one source/event/version-1 current state after a complete affected-schema backup; replay is duplicate-only, SQLite stays query-only, and shadow rows remain empty. `NC-20260818-002` applies migration 124 after a complete backup and live-proves a separate stable-terminal profile/unfiltered-ID audit over 85,076 IDs: 67 accepted, 39 rejected, and 84,970 unknown, with no retained token or protected source/cursor/work/email/service change. Watermark-age alerts, natural-404 recovery with exact accepted/rejected evidence, and label-poll expiry remain open. |
...
| P2.10 exception inbox                | partial        | `NC-20260816-014/015` implement and live-prove the smallest SELECT-only ledger exception report; NC-017 adds the second workflow. Exact release `a2e6d35` and live migration 120 activate NC-018's separate durable reason cases and deduplicated Chief delivery for one owner-confirmed operator while retaining zero workflow authority. The first natural brief, named acknowledgment, and threaded receipt are verified; later source resolution, routing, dead-letter/credential/policy coverage, and any volume-justified work panel remain open.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
24. `REL-003` — define one normalized trigger contract for time, Gmail,
    ...
    Live shadow/runtime proof, label-poll recovery, recurring definitions,
...
28. `UX-001` — build the smallest ledger-backed exception brief/work panel for
    ...
    acknowledgment, and threaded receipt are durably verified; later source resolution,
