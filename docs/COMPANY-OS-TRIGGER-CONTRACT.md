...
`NC-20260818-002` adds an unapplied migration-124 audit target and a separately
invoked profile/unfiltered-ID-list CLI for honest accepted/rejected/unknown
coverage without manufacturing a gap. Local and disposable-PostgreSQL proof
pass; production migration and live read remain pending.
...
NC-20260818-002 separately implements the gap-independent mailbox-audit
boundary in migration 124. It binds the exact current source/version/cursor
digest, permits accepted/rejected/unknown accounting, and cannot record a
watermark event or recovery proposal. Its production migration and first live
read remain a separate promotion gate.
