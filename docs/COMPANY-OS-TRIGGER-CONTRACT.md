...
tables empty/admin-only. `NC-20260818-001` adds a default-refuse, one-source
Gmail bootstrap CLI with cursor-fingerprint, freshness, drift, atomicity, and
exact-replay gates; its production application remains separately gated. Live
Google reads, shadow evidence, cursor/runtime wiring, and the separate
