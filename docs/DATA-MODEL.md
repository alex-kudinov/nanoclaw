...
`NC-20260820-003` deploys SQLite `pending_sends` with one content-free
...
no search, send, reply, approval, or ledger-transition authority. Exact release
`bab154cb`, the additive column/index, prompt, service, channels, queues, and
protected aggregates are verified; a natural work packet, Chief pickup, and
email-source hydration remain outcome-unverified.
...
`NC-20260817-006` adds migration 123 for the inbound-Gmail
...
rollback. `NC-20260817-013` applies that schema dark after a verified backup;
