...
- Implementation: a strict schema and source-reference manifest, allowlisted
  physical source reader, deterministic generator, byte-exact v1 compatibility
  artifact loaded by the existing resolver, and static Tandemweb validation
  artifact. NanoClaw release build/fresh-archive verification rejects stale
  generated bytes. Tandemweb validates one pinned candidate commit before its
  deployment reset; unrelated product records do not enter the selected hash.
- Source state: current deployed/main evidence supersedes the stale NC-005
  branch assumption. Inaugural and regular are both active: inaugural allows
  only the October 7, 2026 cohort; regular excludes that date and serves later
  cohorts. Prices remain USD 399,600 / 479,600 cents with four monthly
  obligations of 99,900 / 119,900 cents. Both checkout direct-price fields
  remain empty; their verified native default-price IDs are the installment
  subscription price IDs. Both project to `CSS / Coaching Supervision Mastery`.
- Account evidence: a read-only probe through the actual deployed consumer
  configuration reader and `STRIPE_SECRET_KEY_ALT` slot returned native account
  `acct_1G1wKzA7hTBWpVVq`, matching `stripe:alt`. No credential value or
...
- Rita: Student Roster already read back January 2027 Thursday. One stale
  ...
  `MCS - Standard path` access remain. Tandemweb now stores the owner-confirmed
  assignment origin as Friday separately from the historical Thursday Stripe
  payment source; the paired January destination carries both facts.
...
- MCS funding: two additional invoice-description Product cells were corrected
  to MCS Practicum after exact paid invoice readback. Together with one exact
  legacy MCS product row, these three bindings reconcile Friday's 13 roster
  assignments to 13 funded seats. Thursday remains 5/matched/open by subtracting
  the transferred payment from its historical payment source; Friday remains
  `needs_review`/sold out only for the distinct 13-versus-owner-12 variance.
...
- Verification: aggregate/hash-only source receipt validates as the third
  Academy reconciliation artifact; focused NanoClaw tests 22/22, pinned
  ...
  date-sensitive Trafft status fixture. Tandemweb focused tests pass 13/13 and
  its generated-source/design-system push gates pass.
- Review: three bounded Claude Sonnet/high rounds across Toolbox, the aggregate
  receipt, and Tandemweb found and corrected only enforceable guard/test gaps. The Toolbox
  readback-refusal and missing-argument paths and the NanoClaw arithmetic-drift
  and missing-provider-readback paths are now directly exercised. Tandemweb now
  rejects manual-funding adjustments unless seat delta is zero, funded count is
  positive and matches the reconciliation delta, source reference is present,
  and evidence is a lowercase 64-character SHA-256. No material finding remains.
...
- Correction: a release-bound code-node source infers exactly one of the four
  mutually exclusive minimal payload shapes and refuses ambiguity or explicit
  mismatch. The host no longer requires privacy-discarded `USER_JOIN.name` or
  `COURSE_COMPLETED.courseName`. Cross-boundary tests execute every relayed
  payload through the host parser. Exact current live `61b12648` is the base,
  preserving the concurrent production checkout-send boundary.
