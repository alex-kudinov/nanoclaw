...
- Incident evidence: the observed Mailman-to-Sales handoff entered host
  processing in under one second and produced its review card roughly 134
  seconds later. The wait was inside the running Sales agent rather than a
  container-slot queue. Source inspection found that Sales instructions assumed
...
- Verification: under pinned Node 22.23.2, focused routing tests pass 23/23,
  ...
  pending.
- Deployment/migration: not deployed. The startup write is a one-row,
  additive JSON configuration convergence that preserves existing Sales model,
  timeout, and other container overrides.
...
- Follow-ups: inspect the production Sales `model` field and the completed run's
  usage log when the production host is reachable; do not create a synthetic
  lead or customer email solely for outcome verification.
