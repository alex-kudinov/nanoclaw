...
| Relationship Context | `src/relationship-context*.ts`, `src/identity-join.ts`, `src/relationship-owner.ts`, migrations/rollbacks 137-138, `docs/RELATIONSHIP-CONTEXT-{CONTROL-PLANE,IMPLEMENTATION-PLAN,PRODUCTION-ROLLOUT,TRAFFT-EXACT-IDENTITY,PROVIDER-RECONCILIATION,STRIPE-CONTACT-CHAOS}.md`, `docs/RELATIONSHIP-OWNER-AUTHORITY.md` | exact live `1a381e48` has provider-neutral Plutio/Encharge/Trafft reconciliation: 1,365 current Plutio refs, 1,242 Encharge refs/consent projections, 159/173 Trafft customers and 358/424 appointment records exact, 14 customers/66 appointment records terminal legacy, and zero provider conflicts. Local NC-20260826-005 adds default-off, consumer-disabled adapters for two distinct Stripe scopes, immutable exact contact submissions, and only agreeing verified Chaos links, with terminal legacy and PII-minimized facts; review/release/deployment/live reconciliation remain pending. Global query remains off/zero grants; provider writes, guessed conflicts, Party merges, broad minion access, scheduled consumers, and outcomes remain excluded. |
...
| `docs/RELATIONSHIP-CONTEXT-CONTROL-PLANE.md` | canonical Party/source-ref, provider-adapter, projection, scoped query, Plutio dry-run/backfill, privacy, rollout, and rollback authority | migration 137 and exact Plutio/Encharge/Trafft refs are live under `1a381e48`; the local dual-Stripe/contact/Chaos extension remains unreleased while group/minion grants, provider writes, and outcomes stay separate |
| `docs/RELATIONSHIP-CONTEXT-STRIPE-CONTACT-CHAOS.md` | exact dual-Stripe, immutable contact-submission, and verified-Chaos source tiers, privacy contract, health, release, and rollback | local NC-20260826-005 source only until independent review, exact release, deployment, replay, and live aggregate readback |
