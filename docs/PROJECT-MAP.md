...
`NC-20260909-004` is the local production-pilot candidate over exact live
`00d66184`, not a live-state claim. It restores reviewed migrations 146-148 and
the admission/store/projection lineage, then adds a release/epoch/preflight-
bound selector in the shared Stripe host path, a native direct-PaymentIntent
or applicable Checkout Session evidence reload, exact `nanoclaw_business`
production facade, atomic one-event/legacy writer claims, post-activation CLI
writer enforcement, accounting-only Contador mode after enrollment ownership
or uncertainty, and immediate
assignment-version Student Roster plus Heartbeat delivery with exact readback
and receipt-owned compensation. Health exposes only bounded pilot state. The
switch defaults off; no production migration, configuration, backup, release,
Sheet/group/membership, writer, payment, student, or provider mutation follows
from source presence. Fresh P01-P10 receipts remain mandatory.
