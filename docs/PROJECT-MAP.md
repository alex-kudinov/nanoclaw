...
| Relationship Context | `src/relationship-context*.ts`, `src/identity-join.ts`, `src/relationship-owner.ts`, migrations/rollbacks 137-138, `docs/RELATIONSHIP-CONTEXT-{CONTROL-PLANE,IMPLEMENTATION-PLAN,PRODUCTION-ROLLOUT,TRAFFT-EXACT-IDENTITY,PROVIDER-RECONCILIATION,STRIPE-CONTACT-CHAOS,CLIENT-PROJECTION,PLUTIO-ENGAGEMENT}.md`, `docs/RELATIONSHIP-OWNER-AUTHORITY.md` | current exact live merge `55efd52f` preserves the source graph and enabled NC-006 all-Party projection: 1,437/1,437 active Parties, 62 paid customers, five active subscribers, nine unproven client labels, 96 summary-unknown, zero-change replay, query off/zero grants. NC-008 locally adds a separately default-off native Plutio engagement adapter. Complete read-only discovery found 117 projects, 183 contracts, eight coaching field definitions, and 59 qualified nonempty projects (11 In progress, 41 Completed, five Canceled, two New) with 52 person/eight company links. Only existing exact person refs may map; company/missing refs hold. In progress is freshness-bounded current engagement, Completed historical, New/Canceled non-authorizing. The shared Plutio auth repair parses only declared keys and copies no credentials. It is not deployed until NC-008 evidence says so. Provider writes, Party merges/role/ref rewrites, broad minion access, and actions remain excluded. |
...
| `docs/RELATIONSHIP-CONTEXT-PLUTIO-ENGAGEMENT.md` | exact Plutio coaching-project/contract evidence tier, safe toolbox dependency, freshness, health, release, and rollback | NC-20260826-008 is local/default-off until independent review, exact toolbox/host release, deployment, replay, and live aggregate readback; contact/project title alone never proves engagement |
