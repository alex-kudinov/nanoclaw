...
| Relationship Context | `src/relationship-context*.ts`, migration/rollback 137, `docs/RELATIONSHIP-CONTEXT-{CONTROL-PLANE,IMPLEMENTATION-PLAN,PRODUCTION-ROLLOUT}.md` | reviewed provider-neutral Party context foundation plus NC-004's production-bound credential-free Trafft host-ledger shadow. The shadow stores minimized appointment observations against exact source references and holds identity rather than inheriting email-based Party links; query capability remains default-off and ungranted. Production migration/deployment and live shadow verification are current gated work; provider writes, Plutio writes, guessed identity, broad minion access, and outcomes remain excluded. |
...
| `docs/RELATIONSHIP-CONTEXT-CONTROL-PLANE.md` | canonical Party/source-ref, provider-adapter, projection, scoped query, Plutio dry-run/backfill, privacy, rollout, and rollback authority | migration 137 and host/container mechanics are local dark source only; real adapters, provider reads/writes, minion grants, production migration, deployment, and outcomes remain separate |
