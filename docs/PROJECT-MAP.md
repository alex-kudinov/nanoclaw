...
`NC-20260817-010` adds a local, default-off historical coverage auditor over
...
production aggregate dry run remains pending; migration 123, source/bootstrap,
live shadow, 404 behavior, and cursor authority remain unchanged.
...
| `docs/COMPANY-OS-GMAIL-RECONCILIATION.md` | inbound Gmail history-expiry source, full-snapshot, accounting, refusal, and promotion contract | NC-005/006 are deployed but unwired; NC-008/009 receipts are live; NC-010's retained-host auditor is local and not mailbox-complete; production cursors/404 behavior remain unchanged |
