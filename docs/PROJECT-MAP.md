...
| Student lifecycle    | `src/student-lifecycle*.ts`, `src/webhook-server.ts`, `src/webhook-inbox-reaper.ts`, migration 134, `facts/catalogs/student-lifecycle-community-*.json`, `facts/catalogs/student-entitlements-v1.json`, `setup/n8n/student-lifecycle-community-*-workflow.json`, `docs/STUDENT-LIFECYCLE-SHADOW-RUNBOOK.md`, `docs/STUDENT-ENTITLEMENT-CATALOG.md` | exact live `8e475e036ad6` restores the Community-only four-action shadow after NC-010 corrected real minimal-payload action inference and discarded-field host drift. `NC-20260905-001` adds the design-only entitlement authority: 42 typed components, six bundle versions, seven active/future offers, explicit access-group versus hidden-marker projection rules, and unresolved provider/quantity conflicts. Revision 1 creates no Heartbeat group, membership, provider action, lifecycle consumer, or runtime behavior. Progress remains held on the documented-interface boundary while Circle, legacy cutover, consumers, and outcome completion remain excluded. |
