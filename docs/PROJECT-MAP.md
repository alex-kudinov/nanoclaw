...
| Student lifecycle    | `src/student-lifecycle*.ts`, `src/student-enrollment-foundation.ts`, `src/webhook-server.ts`, `src/webhook-inbox-reaper.ts`, migration 134 and source-only 142, `scripts/verify-student-enrollment-disposable.mjs`, `facts/catalogs/student-lifecycle-community-*.json`, `facts/catalogs/student-entitlements-v1.json`, `facts/catalogs/student-enrollment-foundation-v1*.json`, `facts/catalogs/student-enrollment-policy-v1.json`, `setup/n8n/student-lifecycle-community-*-workflow.json`, `docs/STUDENT-LIFECYCLE-SHADOW-RUNBOOK.md`, `docs/STUDENT-ENTITLEMENT-CATALOG.md`, `docs/STUDENT-ENROLLMENT-FOUNDATION.md`, `docs/STUDENT-ENROLLMENT-DARK-IMPLEMENTATION.md` | exact live `8e475e036ad6` preserves the Community-only four-action shadow. `NC-20260905-001/002` define entitlement and multi-source enrollment authority. `NC-20260905-003` adds the unwired local migration-142 foundation. `NC-20260905-006` proves that migration on local disposable PostgreSQL 16.15 with pinned target, stripped ambient `PG*`, reason-matched constraints/rollback, 13 table/sequence ownership and zero non-admin grants, reapply, and zero residue. Migration 142 remains unapplied to production; no real student inspection, reconciliation, backfill, provider/Sheet/runtime change, adapter/projection activation, deployment, or communication occurred. Progress remains held on the documented-interface boundary while Circle, legacy cutover, consumers, and outcome completion remain excluded. |
