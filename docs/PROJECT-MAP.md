...
| Company work ledger      | `src/company-work-ledger.ts`, `src/company-work-shadow.ts`, `src/company-job-work-shadow.ts`, `src/company-job-work-shadow-cli.ts`, `src/program-facts-company-work.ts`, `src/company-work-report.ts`, `src/company-work-exception-loop.ts`, `src/company-work-source-context.ts`, migrations 118-120 and 125, `docs/COMPANY-OS-WORK-LEDGER.md`, `docs/COMPANY-OS-JOB-LEDGER.md`                                        | Mailman/Sales and host-job projections are live under their recorded boundaries. NC-20260820-002 live-routes program-facts drift into the same exception projection. NC-20260820-003 locally adds source-bound Chief work packets: the PostgreSQL projection remains content-free while the host resolves a Sales item through its opaque action ID to SQLite and the exact Slack source root. Deployment and natural packet proof remain separate.                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
- `pending_sends` — durable approved-email actions: host action ID, approval
  ...
  failure state. NC-20260820-003 locally adds only the exact inbound source
...
- a durable Sales fallback that re-authorizes an exact thread or address
  ...
  work;
- an NC-20260820-003 local Chief fallback that re-authorizes only
...
NC-20260820-003 is the local work-ledger slice that generalizes one durable
exact-message grant without broadening it; it is not deployed or live-proven.
