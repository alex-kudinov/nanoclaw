...
| Tandem Identity D0-D3 | `src/identity-control-plane/*.ts`, migration/rollback 167, `scripts/verify-tandem-identity-d1-disposable.mjs`, `scripts/validate-tandem-identity-d{2,3}-production.mjs`, `scripts/set-tandem-identity-d2-shadow.mjs`, `docs/IDENTITY-CONTROL-PLANE-D{0,1,2,3}.md` | D0 is the pure deterministic replay core. D1 adds 12 admin-only persistence relations and disposable PostgreSQL contract proof. D2 is live in exact release `d99ca258`: 379 student-lifecycle rows remain 379 unverified/held receipts and observations plus 156 non-materializable candidates, with zero gaps, promotions, attempts or Party/ref drift. D3 adds a reviewed one-shot import for two stable, privacy-minimized Heartbeat `main` censuses and a webhook fingerprint. It can record only aggregate snapshots and one blocked simulation; exact replay writes zero. Because the artifact has no per-user identity graph, canonical resolution, absence decisions, access repair and all provider writes remain blocked. |
