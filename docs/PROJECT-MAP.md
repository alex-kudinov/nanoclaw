...
| Tandem Identity D0/D1 | `src/identity-control-plane/*.ts`, migration/rollback 167, `scripts/verify-tandem-identity-d1-disposable.mjs`, `docs/IDENTITY-CONTROL-PLANE-D{0,1}.md` | `NC-20260913-001` is the pure, unregistered replay core on exact live-base lineage `c190b957`: 12 proof and 27 failure fixtures cover scoped identity, candidate-versus-Party separation, exact resolution/merge lineage, shadow commands, complete/fresh snapshot absence, TEST/LIVE segregation, payer/learner roles, tombstones, split freeze, derived health and canonical hashing. `NC-20260913-002` adds source-only migration 167 over the existing relationship-context tables. Its 12 admin-only relations enforce manifest/effective-scope receipts, append-only versioned candidate lifecycle and materialization refusal, exact result/basis/Party/candidate coupling, monotonic auth/projection versions, a shadow outbox with provider attempts structurally prohibited, immutable readbacks, DB-recomputed complete snapshots and fresh-snapshot absence drift. A generated local PostgreSQL 16.15 rehearsal proves migration failure rollback, direct reapply, 18 negative contracts, backup/restore, populated rollback refusal, empty rollback/reapply and zero residue. Migration 167 is not release-packaged or production-applied; there is no runtime import, provider/network/credential capability, Party/ref/access mutation, deployment or production outcome. |
