...
| Tandem Identity D0-D3, dark Google claim and disposable BFF proposal | `src/identity-control-plane/*.ts`, migration/rollback 167, `scripts/verify-tandem-identity-d1-disposable.mjs`, `scripts/validate-tandem-identity-d{2,3}-production.mjs`, `scripts/set-tandem-identity-d2-shadow.mjs`, `docs/IDENTITY-CONTROL-PLANE-D{0,1,2,3}.md`, `docs/IDENTITY-CONTROL-PLANE-ACCOUNT-CLAIM-DARK.md`, `docs/IDENTITY-CONTROL-PLANE-CLAIM-PROPOSAL-DISPOSABLE.md` | Exact live release `1509c38e` contains D0-D3. D1 provides 12 admin-only persistence relations. D2 remains 379 unverified/held receipts and observations plus 156 non-materializable candidates. D3 stores the stable privacy-minimized Heartbeat census and blocked aggregate simulation. Separate source-only work proves the internal Google claim and one participant-only BFF proposal consumed by an independent Company OS schema. Company OS derives the sole verified candidate/open conflict inside a serializable generated transaction and reuses migration 167; exact replay writes zero. Both additions are unwired and disposable-only with no transport authentication, DDL, provider call or production write. Live canonical resolution, access repair and all provider writes remain blocked. |
