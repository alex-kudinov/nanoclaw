...
| Relationship Context | `src/relationship-context*.ts`, `src/relationship-owner.ts`, migrations/rollbacks 137-138, `docs/RELATIONSHIP-CONTEXT-{CONTROL-PLANE,IMPLEMENTATION-PLAN,PRODUCTION-ROLLOUT}.md`, `docs/RELATIONSHIP-OWNER-AUTHORITY.md` | exact live `416384fc` preserves the healthy credential-free Trafft shadow (417 exact-reference observations/holds, zero projections/queries/grants) and adds one admin-only `team:tandem` principal plus three decision-bound lane assignments with no action authority. A live release-owned 205-observation dry run resolved only `team:tandem`, returned no source errors, and wrote zero follow-up cases/events. Provider/Plutio writes, guessed identity, broad minion access, and actions remain excluded. |
