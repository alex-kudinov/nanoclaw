...
| Tandem Identity D0-D2 | `src/identity-control-plane/*.ts`, migration/rollback 167, `scripts/verify-tandem-identity-d1-disposable.mjs`, `scripts/validate-tandem-identity-d2-production.mjs`, `scripts/set-tandem-identity-d2-shadow.mjs`, `docs/IDENTITY-CONTROL-PLANE-D{0,1,2}.md` | `NC-20260913-001` is the pure replay core: 12 proof/27 failure fixtures cover scoped identity, exact resolution, safe holds, shadow commands, complete/fresh snapshots, TEST/LIVE, payer/learner, merge/split/tombstone and canonical hashing. `NC-20260913-002` adds migration 167: 12 admin-only relations enforce manifest/effective-scope receipts, versioned candidates, exact resolution coupling, monotonic auth/projections, prohibited provider attempts, immutable readbacks and fresh snapshot drift; disposable PostgreSQL proves failed migration, reapply, 18 negative contracts, backup/restore, concurrency, populated refusal and zero residue. `NC-20260913-003` is live in exact release `d99ca258`: the host-only no-provider-client mirror writes the immutable student-lifecycle ledger only as unverified/held receipts, Party-null observations and non-materializable candidates. Production is 379/379 receipts, 379 held observations, 156 candidates, zero gaps/promotions/provider attempts and unchanged Party/ref state. Health is intentionally blocked on provider authenticity; exact replay writes zero. D3 snapshots, canonical resolution, projections, repair and provider writes remain unauthorized. |
