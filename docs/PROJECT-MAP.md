...
| Tandem Identity D0  | `src/identity-control-plane/*.ts`, `docs/IDENTITY-CONTROL-PLANE-D0.md` | `NC-20260913-001` is a pure, unregistered replay core on exact live-base lineage `c190b957`. Strict contracts and semantic checks cover scoped provider identity, candidate-versus-Party separation, exact resolution/merge lineage, shadow-only simulated commands, complete/fresh snapshot absence, TEST/LIVE segregation, payer/learner roles, tombstones, split freeze, derived health, and canonical replay hashing. The 12 proof and 16 failure fixtures contain no person data. No runtime import, database/migration, network/provider dependency, credential, Party/ref/access write, consumer, deployment, or production outcome exists. |
