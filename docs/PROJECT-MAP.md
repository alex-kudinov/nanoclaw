...
| Student lifecycle | `src/student-lifecycle*.ts`, `src/webhook-server.ts`, `src/webhook-inbox-reaper.ts`, migration 134, `facts/catalogs/student-lifecycle-community-*.json`, `setup/n8n/student-lifecycle-community-*-workflow.json`, `docs/STUDENT-LIFECYCLE-SHADOW-RUNBOOK.md` | live Community-only four-action shadow on exact release `cf05bca358c9`: protected legacy 18 plus four empty-filter provider rows, active no-retention relay, exact cohort catalog, aggregate health, duplicate/identity-hold transport, and completed catalog/registry/143-member membership receipts; progress is held on `AUTH_ERROR`, and Circle, legacy cutover, all lifecycle consumers, and outcome completion remain excluded |
