...
| Follow-up        | `src/proposal-followup*.ts`, `src/followup-drop*.ts`, `src/followup-policy.ts`, `src/followup-case-store.ts`, migrations 113 and 130                                     | legacy approval-gated proposal/Sales mechanisms plus NC-20260821-002's live-dark, unwired policy-led exact-case foundation across Sales, proposal signatures, and receivables; the broken daily Sales task is paused, migration 130 is live/empty/admin-only, and no replacement source/draft/send path is active |
...
| `docs/SALES-FOLLOWUP-OPERATING-MODEL.md`  | unified Sales/proposal/receivables ownership, eligibility, cadence, receipts, and rollout gates | NC-20260821-002 pauses the broken Sales task and live-deploys an unwired pure policy plus empty/admin-only migration 130/store; no source adapter, presentation, draft, approval, or customer action |
