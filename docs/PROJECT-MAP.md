...
| Follow-up        | `src/proposal-followup*.ts`, `src/followup-drop*.ts`, `src/followup-policy.ts`, `src/followup-case-store.ts`, migrations 113 and 130                                     | legacy approval-gated proposal/Sales mechanisms plus NC-20260821-002's unwired policy-led exact-case candidate across Sales, proposal signatures, and receivables; the broken daily Sales task is paused, migration 130 is not yet live, and no replacement source/draft/send path is active                |
...
- candidate migration-130 admin-only Company OS follow-up current cases and
  append-only changed-evidence events. Under NC-20260821-002 they are not live
  or wired; repository presence grants no source, scheduler, presentation,
...
| `docs/SALES-FOLLOWUP-OPERATING-MODEL.md`  | unified Sales/proposal/receivables ownership, eligibility, cadence, receipts, and rollout gates | NC-20260821-002 pauses the broken Sales task and adds an unwired pure policy plus candidate migration 130/store; no source adapter, draft, migration, deploy, or activation yet                    |
