...
| Follow-up        | `src/proposal-followup*.ts`, `src/followup-drop*.ts`, `src/followup-policy.ts`, `src/followup-case-store.ts`, migrations 113, 130, and 131                            | legacy approval-gated proposal/Sales mechanisms plus NC-20260821-002's live-dark, unwired exact-case foundation; the broken task is paused, migration 130 is live/empty/admin-only, and the local migration-131/policy-v2 correction makes exact Sales rejection terminal without yet adding a source, presentation, pipeline mutation, draft, or send path |
