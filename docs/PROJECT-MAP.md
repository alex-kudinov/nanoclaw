...
The first successful safety poll returned zero candidates, so the table stayed
empty and natural producer behavior remains `deployed_unverified`.
...
| Gmail gap reconciliation | `src/company-gmail-reconciliation.ts`, `src/company-gmail-reconciliation-shadow.ts`, `src/company-gmail-reconciliation-shadow-store.ts`, `src/gmail-inbound-disposition.ts`, migration 123, `docs/COMPANY-OS-GMAIL-RECONCILIATION.md`                   | NC-005/006 provide the installed-but-unwired proposal/wrapper/shadow. NC-008/009 add and deploy real-ingestion terminal receipts and cursor holdback in exact release `263ac7c4`; live schema/service proof passes, but the first natural receipt is still unobserved. Migration 123, live shadow proof, source registration/bootstrap, 404 recovery wiring, historical unknown coverage, and label-poll recovery remain absent. |
...
- `gmail_inbound_disposition_receipts` — NC-008/009's live append-only,
  ...
  WAL-safe backup; it remained empty during the bounded no-traffic observation,
  so natural producer behavior is not yet verified;
