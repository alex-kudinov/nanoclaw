...
`NC-20260817-009` preflights that producer against production without changing
it. Aggregate-only evidence finds 57 legacy direct-route staging rows: 21 have
one exact routed marker and 36 correctly remain unresolved. Cursorless label
and thread scans now hold only the unresolved candidate and continue unrelated
mail; push history still retains the whole batch cursor. The additive receipt
schema, immutable deployment, and natural non-interference proof remain
pending.
...
- `gmail_inbound_disposition_receipts` — NC-008's local, not-yet-deployed
  ...
  NC-009 has preflighted its production compatibility but has not yet created
  the table;
