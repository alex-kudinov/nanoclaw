...
| Relationship Context | `src/relationship-context*.ts`, `src/identity-join.ts`, `src/relationship-owner.ts`, migrations/rollbacks 137-138, `docs/RELATIONSHIP-CONTEXT-{CONTROL-PLANE,IMPLEMENTATION-PLAN,PRODUCTION-ROLLOUT,TRAFFT-EXACT-IDENTITY}.md`, `docs/RELATIONSHIP-OWNER-AUTHORITY.md` | exact live `416384fc` has the admin-only provider-neutral schema, owner authority, and healthy complete credential-free Trafft host-ledger shadow: 420 minimized null-Party observations/holds, zero projections/queries/grants/Trafft refs before NC-20260826-003. The new local candidate binds only unique post-registration Trafft-created customer/appointment refs, uses exact-first future Booking resolution, appends exact projections, and exposes one host-only minimized canary; legacy/shared/mismatched identities stay held. Provider/Plutio writes, guessed identity, broad minion access, scheduled context consumers, and outcomes remain excluded. |
...
| `docs/RELATIONSHIP-CONTEXT-CONTROL-PLANE.md` | canonical Party/source-ref, provider-adapter, projection, scoped query, Plutio dry-run/backfill, privacy, rollout, and rollback authority | migration 137 and the credential-free null-Party Trafft shadow are live; NC-20260826-003 locally adds strict source-created exact refs and a one-shot host canary, pending deployment/readback; legacy identity, group/minion grants, provider writes, and outcomes remain separate |
