...
| External-write control | `src/action-safety.ts`, `src/action-safety-drill-exec.ts`, `docs/ACTION-SAFETY-CONTROL.md` | deployed/default-off common action envelope and dynamic global/per-system brakes; exact release `d32fda08` live-verifies Gmail send/reply, Slack, Courses SMTP projection, Plutio, Stripe, and Hive/Firestore; `NC-20260816-009` locally adds Things denial before fetch pending immutable deployment; in-flight interruption, standalone scripts, remaining integrations, ceilings, and demotion remain open |
...
| `docs/ACTION-SAFETY-CONTROL.md`         | host action envelope, safety precedence, covered boundaries, and activation/drill transaction | six-system boundary through Hive/Firestore is live-proven under `NC-20260816-008`; the seventh Things boundary is locally verified under `NC-20260816-009` pending immutable deployment; controls remain default-off and residuals explicit |
