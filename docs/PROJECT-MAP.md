...
| Student lifecycle    | `src/student-lifecycle*.ts`, `src/webhook-server.ts`, `src/webhook-inbox-reaper.ts`, migration 134, `facts/catalogs/student-lifecycle-community-*.json`, `facts/catalogs/student-entitlements-v1.json`, `facts/catalogs/student-enrollment-foundation-v1*.json`, `setup/n8n/student-lifecycle-community-*-workflow.json`, `docs/STUDENT-LIFECYCLE-SHADOW-RUNBOOK.md`, `docs/STUDENT-ENTITLEMENT-CATALOG.md`, `docs/STUDENT-ENROLLMENT-FOUNDATION.md`, `docs/ACADEMY-CAPACITY-CONTROL-PLANE.md` | exact live `8e475e036ad6` restores the Community-only four-action shadow after NC-010 corrected real minimal-payload action inference and discarded-field host drift. `NC-20260905-001` adds the design-only entitlement authority. `NC-20260905-002` adds the design-only multi-source enrollment foundation: order -> independently assignable seats -> exact student enrollments, separately versioned obligations, source-reference aliases, class assignments, projection receipts, and durable exceptions. `NC-20260905-004` adds the accepted capacity extension architecture without creating parallel enrollment, entitlement, finance, participant, or schedule records. These design revisions create no Heartbeat group, membership, provider action, production database/runtime change, lifecycle consumer, reconciliation, or deployment. Progress remains held on the documented-interface boundary while Circle, legacy cutover, consumers, and outcome completion remain excluded. |
