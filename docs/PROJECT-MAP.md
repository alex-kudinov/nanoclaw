...
| Student lifecycle    | `src/student-lifecycle*.ts`, `src/student-enrollment-foundation.ts`, `src/academy-capacity.ts`, `src/webhook-server.ts`, `src/webhook-inbox-reaper.ts`, migrations 134 and source-only 142-143, `facts/catalogs/student-lifecycle-community-*.json`, `facts/catalogs/student-entitlements-v1.json`, `facts/catalogs/student-enrollment-foundation-v1*.json`, `facts/catalogs/student-enrollment-policy-v1.json`, `setup/n8n/student-lifecycle-community-*-workflow.json`, `docs/STUDENT-LIFECYCLE-SHADOW-RUNBOOK.md`, `docs/STUDENT-ENTITLEMENT-CATALOG.md`, `docs/STUDENT-ENROLLMENT-FOUNDATION.md`, `docs/STUDENT-ENROLLMENT-DARK-IMPLEMENTATION.md`, `docs/ACADEMY-CAPACITY-CONTROL-PLANE.md`, `docs/ACADEMY-CAPACITY-DARK-IMPLEMENTATION.md` | exact live `8e475e036ad6` preserves the Community-only four-action shadow. `NC-20260905-001/002` define entitlement and multi-source enrollment authority. `NC-20260905-003` adds an unwired local dark enrollment implementation. `NC-20260905-004` accepts the capacity architecture, and `NC-20260905-005` adds its unwired migration/rollback 143 plus pure deterministic capacity command engine without creating parallel enrollment, entitlement, finance, participant, or assignment records. Migrations 142-143 remain unapplied; no real student inspection, reconciliation, backfill, provider/Sheet/runtime change, adapter/projection/minion activation, deployment, waitlist outreach, or live cohort-state change occurred. Progress remains held on the documented-interface boundary while Circle, legacy cutover, consumers, and outcome completion remain excluded. |
