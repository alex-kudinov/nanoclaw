...
| Student lifecycle | `src/student-lifecycle*.ts`, `src/webhook-server.ts`, `src/webhook-inbox-reaper.ts`, migration 134, inactive `setup/n8n/student-lifecycle-community-dark-workflow.json` | local Community-only dark intake, normalized facts, identity/catalog resolution, multi-axis projection, exceptions, replay, and fixtures-only reconciliation; default off, no Circle, provider, deployment, action, message, certificate, or minion authority |
