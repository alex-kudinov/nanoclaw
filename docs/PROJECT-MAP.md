...
`NC-20260818-002` locally adds a separate migration-124 mailbox audit rather
...
proposal. Focused/typecheck and disposable PostgreSQL tests pass. Migration
124 is unapplied and no live Gmail read, daemon import, cursor/recovery, work,
or action change has occurred.
...
| Gmail gap reconciliation | `src/company-gmail-reconciliation.ts`, `src/company-gmail-reconciliation-shadow.ts`, `src/company-gmail-reconciliation-shadow-store.ts`, `src/company-gmail-source-bootstrap*.ts`, `src/company-gmail-mailbox-audit*.ts`, `src/gmail-inbound-disposition.ts`, `src/gmail-historical-coverage*.ts`, migrations 123-124, `docs/COMPANY-OS-GMAIL-RECONCILIATION.md` | NC-005/006 provide the installed-but-unwired proposal/wrapper/gap shadow. NC-008/009 deploy and naturally prove current-ingestion receipts/cursor holdback in exact release `263ac7c4`. NC-010 completes default-off retained-host coverage with 3,041 IDs split into 23 terminal, 1,675 recoverable, and 1,343 unknown; it does not claim mailbox completeness. NC-013 applies migration 123 dark. NC-20260818-001 live-proves one source/bootstrap/version-1 current state. NC-20260818-002 locally implements a separate migration-124 gap-independent mailbox audit with profile/unfiltered-ID-only Gmail scope, query-only receipt accounting, honest unknowns, and disposable-PostgreSQL source-drift/terminal proof. Migration 124 and its live audit are pending; 404 recovery wiring and label-poll recovery remain absent. |
...
- `NC-20260818-002` adds unapplied migration 124 and a separately invoked,
  default-refuse gap-independent mailbox audit. It lists only unfiltered Gmail
  IDs plus profile head, reads SQLite terminal receipts query-only, stores
  accepted/rejected/unknown evidence under admin-only tables, and rechecks the
  exact current source before page or completion writes. Disposable PostgreSQL
  proves terminal accounting and source-drift refusal. No daemon import,
  production migration/read, cursor event, recovery, work, or action change has
  occurred yet.
