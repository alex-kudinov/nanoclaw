...
| Relationship Context | `src/relationship-context*.ts`, migration/rollback 137, `docs/RELATIONSHIP-CONTEXT-CONTROL-PLANE.md`, `docs/RELATIONSHIP-CONTEXT-IMPLEMENTATION-PLAN.md` | local dark provider-neutral Party context foundation: scoped refs and ambiguity-safe identity, bounded observations/projections/query receipts, tracked adapter/fact registry with fixture-only LMS proof, exact one-shot host grants, default-off IPC, and dry-run-only Plutio plans. No production migration, real adapter/provider access, minion grant, Plutio write/backfill, deployment, or live outcome |
