...
| Tandem Identity D0-D3 and dark Google account claim | `src/identity-control-plane/*.ts`, migration/rollback 167, `scripts/verify-tandem-identity-d1-disposable.mjs`, `scripts/validate-tandem-identity-d{2,3}-production.mjs`, `scripts/set-tandem-identity-d2-shadow.mjs`, `docs/IDENTITY-CONTROL-PLANE-D{0,1,2,3}.md`, `docs/IDENTITY-CONTROL-PLANE-ACCOUNT-CLAIM-DARK.md` | Exact live release `1509c38e` contains D0-D3. D1 provides 12 admin-only persistence relations. D2 remains 379 unverified/held receipts and observations plus 156 non-materializable candidates, with zero gaps, promotions, attempts or Party/ref drift. D3 live-imports two stable, privacy-minimized Heartbeat `main` censuses and a webhook fingerprint: 1,694 users, 79 groups, 4,202 edges, 22 webhooks, 82 normalized items, one blocked simulation and one unavailable readback. Exact replay writes zero. A separate source-only dark claim proves that a recent explicit Google subject can bind one already-known Heartbeat user to one existing Party only after all ambiguity/conflict/role guards close; it is unwired, disposable-only and adds no DDL, provider call or production write. Live canonical resolution, access repair and all provider writes remain blocked. |
