...
| Relationship Context | `src/relationship-context*.ts`, `src/identity-join.ts`, `src/relationship-owner.ts`, migrations/rollbacks 137-138, `docs/RELATIONSHIP-CONTEXT-{CONTROL-PLANE,IMPLEMENTATION-PLAN,PRODUCTION-ROLLOUT,TRAFFT-EXACT-IDENTITY,PROVIDER-RECONCILIATION,STRIPE-CONTACT-CHAOS,CLIENT-PROJECTION}.md`, `docs/RELATIONSHIP-OWNER-AUTHORITY.md` | current exact live merge `55efd52f` has NC-006 release `f8595966` as a direct parent, preserving the Plutio/Encharge/Trafft/Stripe/contact/Chaos graph and the enabled, provider-free, consumer-disabled all-Party relationship projection. All 1,437 active Parties have one projection: 62 defensible paid customers (five actively subscribed), nine separately unproven client labels, two recorded student labels, 1,317 recorded prospect labels, and 96 summary-unknown Parties with controlled overlaps. All remain version 1 after zero-change replay; active coaching engagement is explicitly unknown. Existing source totals/conflicts remain unchanged. Global query stays off/zero grants; provider writes, Party merges/role rewrites, broad minion access, and actions remain excluded. |
