...
| Tandem Identity D0-D3, dark Google claim, disposable transport and first tools authorization path | `src/identity-control-plane/*.ts`, migration/rollback 167, private `alex-kudinov/tandem-identity` source, `docs/IDENTITY-CONTROL-PLANE-D{0,1,2,3}.md`, `docs/IDENTITY-CONTROL-PLANE-ACCOUNT-CLAIM-DARK.md`, `docs/IDENTITY-CONTROL-PLANE-CLAIM-PROPOSAL-DISPOSABLE.md`, `docs/IDENTITY-CONTROL-PLANE-SERVICE-TOPOLOGY-DISPOSABLE.md` | Exact live release `1509c38e` contains D0-D3; later claim and transport work remains source-only. Company OS derives Party/conflict context and reuses migration 167; exact replay writes zero. `NC-20260915-002` proves a future Google-signed service-account caller contract without runtime wiring. `NC-20260915-003` adds the first customer-facing authorization path in the private Tandem Identity source: one revocation-checked session and exact `(project, UID)` binding can open `/tools` only through a fresh active `coaching_tools.plus` projection. All uncertain states deny and gateway failure is unavailable. The deployed server remains unconfigured, so no live user journey, gateway, binding, grant or deployment is claimed. Live completion waits for an owner-named pilot identity and separately authorized gateway path; provider writes and broad migration remain blocked. |
...
`NC-20260914-002` and `NC-20260918-001` add optional
`submission_language=it|pl` to this same privileged request. It is part of
idempotency identity and is bound in memory to the exact Slack root before
persistence. Only that root's verified run context receives it; course locale
and feedback language remain registry-owned, and chat text or an English
translation cannot supply the attestation.
