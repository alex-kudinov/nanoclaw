...
| Checkout recovery | `src/checkout-recovery*.ts`, `src/webhook-server.ts`, migrations 135-136, Tandemweb signed producer, fixed-account Stripe extractor | separate pre-payment cases, exact alias/current-and-sibling purchase precedence, Tandem-only 45-minute timeout, Heartbeat event-only failure/expiry evidence, policy-v2 locale/safe-return context, privacy-minimal Inbox/report projection, and independently gated prospective per-touch Encharge handoff with append-only receipts |
...
| Relationship Context | `src/relationship-context*.ts`, `src/identity-join.ts`, `src/relationship-owner.ts`, migrations/rollbacks 137-138, `docs/RELATIONSHIP-CONTEXT-{CONTROL-PLANE,IMPLEMENTATION-PLAN,PRODUCTION-ROLLOUT,TRAFFT-EXACT-IDENTITY,PROVIDER-RECONCILIATION,STRIPE-CONTACT-CHAOS,CLIENT-PROJECTION}.md`, `docs/RELATIONSHIP-OWNER-AUTHORITY.md` | exact live `d5375964` preserves Plutio/Encharge/Trafft reconciliation and adds consumer-disabled read-only adapters for both Stripe accounts, immutable contact submissions, and agreeing verified Chaos links. Live totals are 75 Stripe customers plus 157 native payment/subscription refs, 185 contact submissions, 1,328 Chaos visitors, 722/6/40 terminal legacy identities, 1,314 held Stripe native facts, and zero conflicts/prohibited values. Trafft remains 159/358 exact and 14/66 legacy with 1,365 Plutio refs. NC-006 locally adds a separately default-off, provider-free all-Party relationship projection: 62 current Parties have defensible paid-customer evidence; nine active client role rows remain separately visible but unproven because they lack source/decision provenance. Active coaching engagement remains unknown pending authoritative contract evidence. It is not deployed until NC-006 release evidence says so. Global query remains off/zero grants; provider writes, Party merges/role rewrites, broad minion access, and actions remain excluded. |
...
| `docs/RELATIONSHIP-CONTEXT-CLIENT-PROJECTION.md` | deterministic all-Party client/customer evidence policy, privacy-minimized projection, health, release, and rollback | NC-20260826-006 is local/default-off until independent review, exact release, deployment, replay, and live aggregate readback; payment does not prove active engagement |
