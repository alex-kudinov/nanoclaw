...
identity, conflicting offers, managed unknown companion signals, and projection
disagreement hold. The Tandemweb v2 artifact validates source-owned checkout
price, installment arithmetic, enrollment scope, and cohort-context role;
...
| Stripe               | `src/stripe-payment-host.ts`, `src/stripe-payment-source.ts`, `src/contador-payment-fulfillment-store.ts`, `src/contador-name-reaper.ts`, `tools/contador/process-payment.cjs`, `tools/contador/lib/cohort.cjs`, migrations 116 and 133                                                                                                                                                           | dual-account payment/refund ingestion, canonical transaction identity, durable operational complete/exception cases, exact payment/roster/cohort readback, and name recovery; refunds remain review-held and Bizmgr retains accounting. NC-20260904-001 restored the release-required cohort dependency and the current split checkout metadata contract after an untracked operational implementation disappeared from immutable releases.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Checkout recovery    | `src/checkout-recovery*.ts`, `src/checkout-customer-identity.ts`, `src/webhook-server.ts`, migrations 135-136 and 140, Tandemweb signed producer, fixed-account Stripe extractor, exact-node n8n patch spec, tracked Encharge assets under `docs/programs/company-os/provider-assets/checkout-recovery/`                                                | separate pre-payment cases, exact alias/current-and-sibling purchase precedence, Tandem-only 45-minute timeout, one six-key safe decline policy, a persistent localized website remediation step, and the existing gated two-touch Encharge handoff enriched with localized failure-specific copy and human support. NC-20260829-001 live-deploys recovery plus capture-first identity: synchronously resolve/create one canonical Party, retain unpaid prospect semantics, create/reuse one Party-idempotent Stripe Customer, durably bind the exact ref, and block payment until both identities exist. Structural proof passes; the next natural checkout remains outcome proof.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| Tandem Identity D0-D3, dark Google claim, disposable BFF proposal and service-identity verifier | `src/identity-control-plane/*.ts`, migration/rollback 167, `scripts/verify-tandem-identity-d1-disposable.mjs`, `scripts/validate-tandem-identity-d{2,3}-production.mjs`, `scripts/set-tandem-identity-d2-shadow.mjs`, `docs/IDENTITY-CONTROL-PLANE-D{0,1,2,3}.md`, `docs/IDENTITY-CONTROL-PLANE-ACCOUNT-CLAIM-DARK.md`, `docs/IDENTITY-CONTROL-PLANE-CLAIM-PROPOSAL-DISPOSABLE.md`, `docs/IDENTITY-CONTROL-PLANE-SERVICE-TOPOLOGY-DISPOSABLE.md` | Exact live release `1509c38e` contains D0-D3. D1 provides 12 admin-only persistence relations. D2 remains 379 unverified/held receipts and observations plus 156 non-materializable candidates. D3 stores the stable privacy-minimized Heartbeat census and blocked aggregate simulation. Separate source-only work proves the internal Google claim and one participant-only BFF proposal consumed by an independent Company OS schema. Company OS derives the sole verified candidate/open conflict inside a serializable generated transaction and reuses migration 167; exact replay writes zero. `NC-20260915-002` adds an unwired pure verifier for a future Google-signed Cloud Run service-account ID token with exact injected issuer, HTTPS audience, verified principal and immutable subject. An ephemeral synthetic signature plus the exact existing BFF artifact proves the contract and zero-write replay only; it does not prove Google interoperability, IAM, certificate refresh, HTTPS routing or production availability. No endpoint, network client, credential, runtime import, new DDL, provider call or production write exists. Live canonical resolution, access repair and all provider writes remain blocked. |
| Student lifecycle    | `src/student-lifecycle*.ts`, `src/student-enrollment-foundation.ts`, `src/academy-capacity.ts`, `src/webhook-server.ts`, `src/webhook-inbox-reaper.ts`, migrations 134 and 142-145, `scripts/verify-student-enrollment-disposable.mjs`, `scripts/verify-academy-capacity-disposable.mjs`, `scripts/validate-academy-capacity-reconciliation.mjs`, `facts/catalogs/student-lifecycle-community-*.json`, `facts/catalogs/student-entitlements-v1.json`, `facts/catalogs/student-enrollment-foundation-v1*.json`, `facts/catalogs/student-enrollment-policy-v1.json`, `facts/catalogs/academy-capacity-reconciliation-*.schema.json`, `setup/n8n/student-lifecycle-community-*-workflow.json`, `docs/STUDENT-LIFECYCLE-SHADOW-RUNBOOK.md`, `docs/STUDENT-ENTITLEMENT-CATALOG.md`, `docs/STUDENT-ENROLLMENT-FOUNDATION.md`, `docs/STUDENT-ENROLLMENT-DARK-IMPLEMENTATION.md`, `docs/ACADEMY-CAPACITY-CONTROL-PLANE.md`, `docs/ACADEMY-CAPACITY-DARK-IMPLEMENTATION.md` | exact live `8e475e036ad6` preserves the Community-only four-action shadow. `NC-20260905-001/002` define entitlement and multi-source enrollment authority. `NC-20260905-003` adds the unwired local migration-142 foundation; `NC-20260905-006` proves it on local disposable PostgreSQL 16.15 with a pinned target, stripped ambient `PG*`, reason-matched failures, table/sequence ownership, reapply, and zero residue. `NC-20260905-004/005` accept and implement the separate unwired migration-143 capacity extension without parallel enrollment records. `NC-20260905-007` proves the ordered 142-to-143 chain on the same generated local-only boundary, including shared-pool occupancy, reservation/waitlist integrity, populated refusal, explicit cross-migration cleanup, empty rollback/reapply, and zero residue. `NC-20260905-008` records a privacy-minimized read-only MCS/ACC reconciliation: Thursday is 5/12 open; Friday is fail-closed sold out with 13 roster assignments against capacity 12; its labeled-only ACC count is preserved as historical evidence. `NC-20260905-009` corrects ACC September 7 to 21 operational unique seats from 8 explicit plus 13 post-boundary unlabeled rows, with one May 27 candidate making the held upper boundary 22. The 21 split into 10 Module 1 and 11 Full Program roster routes; current exact evidence identifies 8 `acc-module-1`, 5 `acc-full`, 0 `acc-pcc-full`, and 8 unclassified. No candidate intersects PCC, ACTC, or the Professional Coach Heartbeat group, so combined-program offer/projection gaps remain exceptions and add no capacity. `NC-20260905-010` records owner-confirmed capacity 12: operational overage is 9, the held upper-bound overage is 10, availability is zero, and the numeric-capacity exception is resolved without authorizing source repair. NC-20260906-002 then applied migrations 142-143 and populated the exact five-block/40-assignment production shadow without daemon activation. NC-20260906-003 adds the default-off migration-144 operator ledger, host transaction adapter, strict Capacity IPC/capability/group contract, and disposable last-seat/idempotency/waitlist/transfer proof. Capacity has no direct database, provider, message, network, checkout, payment, refund, certificate, or public-site authority; Gate E/F, Circle, legacy lifecycle cutover, customer communication, and automatic waitlist promotion remain excluded. |
...
| Knowledge drift      | `facts/programs.yaml`, `facts/catalogs/*`, `tools/sync-program-facts.py`, `src/program-facts-drift.ts`, `src/program-facts-drift-job.ts`, `src/program-facts-company-work.ts`, `src/sales-fact-consistency.ts`, `src/lesson-conflict.ts`, `src/learn-ipc-handler.ts`, migration 125                                                                                                      | deterministic factual controls and exact generated KB blocks plus the live detector-to-Company-Work adapter; Practitioner facts, the verified English/French/Japanese/Spanish Foundations availability set, and Coaching Supervision Mastery's live AACS/enrollment/checkout facts are hash-bound and injected into every tracked minion KB, while provider/public evidence remains higher authority; the detector checks the operational Sales KB actually mounted at runtime. `NC-20260906-004` additionally validates schedule-dependent Sales output against the current operational schedule and immutable supervision catalog at pre-approval, approval-arm, rescue, and Gmail execution boundaries, so a correct internal analysis cannot be followed by a contradictory customer draft. |
...
`NC-20260906-001` supersedes the student-lifecycle table's historical unresolved
capacity snapshot with exact source repair: Rita's transfer is final from MCS
September Friday to January Thursday; stale September Heartbeat access was
...
`NC-20260906-002` completed the production shadow boundary. The reviewed
...
`NC-20260906-003` is the separately authorized Gate D operator pilot. The
Capacity minion can submit only exact typed host commands through an enforced,
zero-credential/no-network capability. The host records a case and requested
receipt, locks affected pools, invokes the pure engine, writes only its
compare-and-swap delta, reloads database state, and records a final hash-bound
receipt. Migration 144 stores privacy-minimized cases/receipts only. Mutation
IPC defaults off until the exact reviewed release, migration, capability
allowlist, and private operator group are verified. Waitlist staging cannot
contact anyone or convert an enrollment. Gate E Tandemweb integration and Gate
F assignment/capacity authority cutover remain separately unauthorized.

`NC-20260906-005` supersedes the real-time reservation Gate E candidate with
the owner-approved simple sync. Migration 145 adds durable, one-seat
commitments and separates their count from temporary holds; successful website
sales enter after verified Contador fulfillment, while invoice/check/sponsor
promises enter through Capacity. New versioned commands change pool capacity,
move a commitment atomically, and reconcile it only to an exact existing
assignment. A retryable host outbox publishes signed `available|sold_out`
state to a monotonic WordPress option, purges only affected LiteSpeed and
Cloudflare URLs, and prewarms them. Checkout reads the local projection but
never calls NanoClaw or creates a new short-lived hold. Gate F, automatic
waitlist contact, refunds/payments, and provider authority remain excluded.
...
The accepted terminal-state correction adds migration 139's typed `ch_`/`py_`
...
Payment foundation (NC-20260909-003): `src/payment-domain.ts` and
`docs/PAYMENT-DOMAIN.md` add unwired quote/attempt/operation guards and deterministic
one-time financial-evidence projection for the MCS Foundations pilot. No runtime
consumer, database migration, provider request or enrollment effect is enabled.
The existing TEST receiver remains a separate, inert admission boundary.

The source-only `src/payment-store.ts`/`src/payment-payload-vault.ts` layer and
migration 149 add durable encrypted session-operation storage, DB-clock fenced
leases and append-only state receipts. The store takes an injected transaction
function; it opens no production connection and is not daemon-wired. Its
disposable Postgres proof exercises contention, lease loss, atomic receipts and
...
atomic nonce replay rejection and encrypted attempt-scoped status capabilities
with expiry/revocation. It is not exposed publicly: route-owned command binding,
response minimization and rate limits remain mandatory integration work.
...
command/operation binding, process-local rate/concurrency limit, capability-checked
resume/status and minimized no-store responses. See `docs/PAYMENT-API-CONTRACT.md`.
...
Crossed-reference races share provider/attempt locks, and reconciliation conflicts
remain visible on later status reads. Successful capture/refund acknowledgments,
later failures/reversals and chargebacks remain distinct; no settlement is inferred.
The pure readiness gate implements the owner-approved ACH provisional-access
policy and keeps certificate financial clearance blocked without authenticated
received-funds evidence. It emits no action and is not certificate/enrollment
wiring. The private `/returns` route never accepts a browser Session ID or method.
The public gateway still needs the separate pre-n8n filter and actual TEST proof.

`adyen-test-edge-ingress.ts` now provides the reviewed factory-only pre-workflow
TEST filter. It reuses native HMAC verification, discards foreign traffic locally
with aggregate counters, forwards only minimized owned native envelopes retaining
their signatures, and requires an exact bounded durable upstream ACK. It has no
default listener, environment/database access or external activation. Local HTTP
...
trusted signed-event projection into one canonical enrollment transaction. Order
identity is scope plus quote, while the scoped native PSP receives one writer
claim. The trusted event boundary rejects authorization amount/currency mismatch
before projection persistence. Exact replay preserves the original consumption
digest while separately surfacing later adverse facts. It writes one seat,
locale component, paid-in-full agreement and pending-receipt obligation, but
...
| Adyen TEST admission     | `src/adyen-webhook.ts`, `docs/ADYEN-TEST-WEBHOOK.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | HMAC and signed merchant/reference/event allowlists, defense-in-depth reported store, minimized durable terminal receipt, no fulfillment dispatch                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| External daemon watchdog | `scripts/nanoclaw-watchdog.sh`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | checks process/health/channel/container evidence and performs deterministic recovery; Slack-heartbeat age is interval-derived and receives a bounded live-uptime startup grace only while Slack is connected, preventing deployment restarts from resetting the first heartbeat forever                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| Healer                   | `src/healer/*`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | collection, diagnosis, trust, approval, remediation, incident reporting, plus NC-20260822-015's standalone read-only resolution catalog; the catalog deduplicates by incident fingerprint and exposes bounded proposed solutions/pending-decision state without reading raw context, commands, diffs, Slack identity, or transcript paths; it is not daemon/scheduler/Company-Work wired and grants no action authority                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
`NC-20260822-015` adds a separately invoked, read-only resolution catalog over
the existing incident store. One stable `healer:<fingerprint>` item represents
the current open incarnation or latest terminal history. The pure classifier
distinguishes monitoring, `verified_fixed`, named `proposal_rejected`, and
pending decisions, including automatically assigned `wont_fix`, unrouted or
stale lifecycle states, recurrence, low-trust/manual fixes, and unverified
terminal states. The source query selects only bounded solution/diagnosis and
trust metadata; output omits raw context, executable content, Slack/thread
...
- deployed `NC-20260821-005` source-shadow/report code reads those cases plus
  ...
  evidence. It is default-dry-run and absent from daemon/scheduler/IPC.
  Deployed `NC-20260821-006` exact transaction evidence advanced eight overdue
  invoices to internal review in a zero-error 191-observation Mini scan while
  leaving 12 future invoices waiting; exact historical thread bindings and
  assigned relationship ownership remain explicit blocks rather than inferred
  permission. Local candidate `NC-20260821-007` can render only those exact
  internal reviews into a bounded, source-bound, non-posting packet; it has no
  projection or decision authority;
...
| `docs/RELATIONSHIP-CONTEXT-CONTROL-PLANE.md`        | canonical Party/source-ref, provider-adapter, projection, scoped query, Plutio dry-run/backfill, privacy, rollout, and rollback authority | migration 137 and exact Plutio/Encharge/Trafft/Stripe/contact/Chaos evidence are live through `d5375964`; NC-006's all-Party relationship projection remains local/default-off while group/minion grants, provider writes, and outcomes stay separate                                                                  |
| `docs/RELATIONSHIP-CONTEXT-STRIPE-CONTACT-CHAOS.md` | exact dual-Stripe, immutable contact-submission, and verified-Chaos source tiers, privacy contract, health, release, and rollback         | NC-20260826-005 is live/replayed/privacy-verified under exact release `d5375964`; query/minion consumers remain disabled                                                                                                                                                                                               |
| `docs/RELATIONSHIP-CONTEXT-CLIENT-PROJECTION.md`    | deterministic all-Party client/customer evidence policy, privacy-minimized projection, health, release, and rollback                      | NC-20260826-006 is live/replayed/privacy-verified in current exact descendant merge `55efd52f`; payment does not prove active engagement and query/minion consumers remain disabled                                                                                                                                    |
| `docs/RELATIONSHIP-CONTEXT-PLUTIO-ENGAGEMENT.md`    | exact Plutio coaching-project/contract evidence tier, safe toolbox dependency, freshness, health, release, and rollback                   | NC-20260826-009 is live/replayed/privacy-verified under exact release `6a978328`; only two exact person links map while 50 person/eight company links remain held; contact/project title alone never proves engagement                                                                                                 |
| `docs/RELATIONSHIP-CONTEXT-IMPLEMENTATION-PLAN.md`  | exact NC-20260825-003 local dark schema/source/test/review boundary                                                                       | implementation evidence only; it grants no provider, production, minion, Plutio, communication, or deployment authority                                                                                                                                                                                                |
| `docs/STUDENT-ENTITLEMENT-CATALOG.md`               | versioned full-program bundle/component, Heartbeat access-versus-marker, scheduling, consumption, evidence, and implementation-gate contract | revision 1 is source/design only; several ACC/PCC/ACTC quantities and full-group attachments remain provisional, and no provider group, membership, student record, message, flow, migration, consumer, or deployment is authorized |
| `docs/STUDENT-ENROLLMENT-FOUNDATION.md`              | provider-neutral enrollment order/seat/participant, source alias, obligation, assignment, projection-receipt, exception, operator-intake, and rollout-gate contract | revision 2 aligns the reviewed local dark command contract; migration 142 remains unapplied and no historical inspection, reconciliation, backfill, provider/data/runtime write, deployment, or communication is authorized |
| `docs/STUDENT-ENROLLMENT-DARK-IMPLEMENTATION.md`     | conservative policy defaults, migration-142 schema, pure command engine, test matrix, rollback, and promotion gates | local/unwired source only; migration apply, real-data reconciliation, adapters, projections, backfill, deployment, and communication remain unauthorized |
| `docs/ACADEMY-CAPACITY-CONTROL-PLANE.md`             | accepted seat-pool, reservation, assignment, transfer, waitlist, Tandemweb projection, and Capacity-minion architecture extending the entitlement and enrollment foundations | migration/rollback 143 and the pure capacity command engine are local/unwired; migration apply, reconciliation, operator/minion activation, website cutover, messages, deployment, and authority cutover remain separately gated |
| `docs/ACADEMY-CAPACITY-DARK-IMPLEMENTATION.md`       | exact migration-143 relations, enrollment-extension boundary, reservation channels, pure commands, tests, rollback, and promotion gates | local source only; no migration apply, real-data/provider/runtime wiring, minion activation, communication, deployment, or live cohort-state change |
| `docs/RELATIONSHIP-OWNER-AUTHORITY.md`              | Tandem OS generic organizational owner, exact assignment/provenance, fail-closed resolution, and no-action boundary                       | `team:tandem` is accountability/routing only; it never grants sender, approval, provider-write, minion, or follow-up authority                                                                                                                                                                                         |
