...
| Relationship Context | `src/relationship-context*.ts`, `src/relationship-owner.ts`, migrations/rollbacks 137-138, `docs/RELATIONSHIP-CONTEXT-{CONTROL-PLANE,IMPLEMENTATION-PLAN,PRODUCTION-ROLLOUT}.md`, `docs/RELATIONSHIP-OWNER-AUTHORITY.md` | exact live `e97c9f87` has the admin-only provider-neutral context schema plus healthy complete credential-free Trafft host-ledger shadow: 414 minimized exact appointment-reference observations with null Party identity/414 holds, zero projections, zero queries/grants, and 414/414 replay duplicates. NC-20260826-001 adds the owner-approved `team:tandem` organizational principal and exact lane assignments with no action authority; its migration/release state is recorded separately. Provider/Plutio writes, guessed identity, broad minion access, and outcomes remain excluded. |
...
| `docs/RELATIONSHIP-CONTEXT-CONTROL-PLANE.md` | canonical Party/source-ref, provider-adapter, projection, scoped query, Plutio dry-run/backfill, privacy, rollout, and rollback authority | migration 137 and the credential-free null-Party Trafft shadow are live; exact Party authority, real providers, minion grants, provider writes, and outcomes remain separate |
