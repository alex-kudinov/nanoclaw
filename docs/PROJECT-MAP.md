...
| Checkout recovery | `src/checkout-recovery*.ts`, `src/checkout-customer-identity.ts`, `src/webhook-server.ts`, migrations 135-136 and 140, Tandemweb signed producer, fixed-account Stripe extractor, exact-node n8n patch spec | separate pre-payment cases, exact alias/current-and-sibling purchase precedence, Tandem-only 45-minute timeout, safe decline mapping, and prospective per-touch Encharge handoff. NC-20260829-001 live-deploys the recovery correction. Its local capture-first extension synchronously resolves/creates one canonical Party, keeps unpaid identity as prospect, creates/reuses one Party-idempotent Stripe Customer, durably binds the exact ref, and blocks payment until both identities exist; ambiguity fails closed. Extension deployment and the next natural checkout remain pending. |
