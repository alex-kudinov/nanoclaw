...
| Checkout recovery | `src/checkout-recovery*.ts`, `src/webhook-server.ts`, migrations 135-136 and proposed 140, Tandemweb signed producer, fixed-account Stripe extractor, exact-node n8n patch spec | separate pre-payment cases, exact alias/current-and-sibling purchase precedence, Tandem-only 45-minute timeout, Heartbeat event-only failure/expiry evidence, affirmative v2/v3 locale/safe-return context, customer-safe decline mapping, and prospective per-touch Encharge handoff. NC-20260829-001 locally corrects exact-byte website HMAC, ID-only Stripe forwarding, Party/failure context, one atomic fixed-window human-first incident, and historical-queue no-replay cutover; it is not deployed yet. |
