...
| Gmail gap reconciliation | `src/company-gmail-reconciliation.ts`, `src/company-gmail-reconciliation-shadow.ts`, `src/company-gmail-reconciliation-shadow-store.ts`, `src/gmail-inbound-disposition.ts`, `src/gmail-historical-coverage*.ts`, migration 123, `docs/COMPANY-OS-GMAIL-RECONCILIATION.md` | NC-005/006 provide the installed-but-unwired proposal/wrapper/shadow. NC-008/009 deploy and naturally prove current-ingestion receipts/cursor holdback in exact release `263ac7c4`. NC-010 completes default-off, content-free retained-host coverage with 3,041 IDs split into 23 terminal, 1,675 recoverable, and 1,343 unknown, plus identical production pre/post state fingerprints; it explicitly does not claim mailbox completeness. Migration 123, live shadow proof, source registration/bootstrap, 404 recovery wiring, Gmail-only unknown coverage, and label-poll recovery remain absent. |
