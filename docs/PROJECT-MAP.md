...
| Company work ledger      | `src/company-work-ledger.ts`, `src/company-work-shadow.ts`, `src/company-job-work-shadow.ts`, `src/company-job-work-shadow-cli.ts`, `src/program-facts-company-work.ts`, `src/company-work-report.ts`, `src/company-work-exception-loop.ts`, `src/company-work-source-context.ts`, migrations 118-120 and 125, `docs/COMPANY-OS-WORK-LEDGER.md`, `docs/COMPANY-OS-JOB-LEDGER.md`                                        | Mailman/Sales and host-job projections are live under their recorded boundaries. NC-20260820-002 live-routes program-facts drift into the same exception projection. Exact release `bab154cb` deploys NC-20260820-003's source-bound Chief work packets: the PostgreSQL projection remains content-free while the host resolves a Sales item through its opaque action ID to SQLite and the exact Slack source root. Runtime/schema/prompt state is verified; the unchanged daily brief deduplicated at startup, so natural packet and Chief-pickup proof remain pending.                                                                                                                                                                                                                                                                                                                                                                                          |
