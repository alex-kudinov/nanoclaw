...
| Relationship Context | `src/relationship-context*.ts`, migration/rollback 137, `docs/RELATIONSHIP-CONTEXT-{CONTROL-PLANE,IMPLEMENTATION-PLAN,PRODUCTION-ROLLOUT}.md` | exact live `e97c9f87`: admin-only provider-neutral context schema plus healthy complete credential-free Trafft host-ledger shadow. It stores 414 minimized exact appointment-reference observations with null Party identity/414 holds, zero projections, zero queries/grants, and 414/414 replay duplicates. Query capability is off. Provider/Plutio writes, guessed identity, broad minion access, and outcomes remain excluded. |
