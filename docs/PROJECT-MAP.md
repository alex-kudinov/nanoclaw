...
| Tandem Identity D0-D2 | `src/identity-control-plane/*.ts`, migration/rollback 167, `scripts/verify-tandem-identity-d1-disposable.mjs`, `scripts/validate-tandem-identity-d2-production.mjs`, `scripts/set-tandem-identity-d2-shadow.mjs`, `docs/IDENTITY-CONTROL-PLANE-D{0,1,2}.md` | `NC-20260913-001` is the pure replay core: 12 proof/27 failure fixtures cover scoped identity, exact resolution, safe holds, shadow commands, complete/fresh snapshots, TEST/LIVE, payer/learner, merge/split/tombstone and canonical hashing. `NC-20260913-002` adds source-only migration 167: 12 admin-only relations enforce manifest/effective-scope receipts, append-only versioned candidates, exact resolution coupling, monotonic auth/projections, structurally prohibited provider attempts, immutable readbacks and fresh complete-snapshot drift. Local PostgreSQL 16.15 proves failed migration, reapply, 18 negative contracts, backup/restore, concurrency, populated refusal and zero residue. `NC-20260913-003` is the separately authorized D2 candidate: a default-off, host-only, no-provider-client mirror from the existing immutable student-lifecycle ledger into unverified/held receipts, Party-null observations and non-materializable candidates. The frozen production prefix is 379 rows/156 users/71 current Party-linked source rows, all provider-unreconciled, so caught-up health must remain blocked. Disposable 2+2+no-op replay and unexpected-authenticity rollback pass locally. Production migration, release, enablement and live outcome are not yet claimed. |
