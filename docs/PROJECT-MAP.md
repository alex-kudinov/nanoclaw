...
`NC-20260820-002` adds the first local `business_condition` composition over
that contract. The versioned program-facts detector now derives canonical
...
only completion receipt; recurrence reopens the same item. Migration 125 and
`PROGRAM_FACTS_COMPANY_WORK_MODE=active` remain local, unapplied, and off, and
the installed job still uses the legacy notify-only path. No detector or
healer component receives authority to edit facts, knowledge, products,
...
| Knowledge drift  | `src/program-facts-drift.ts`, `src/program-facts-drift-job.ts`, `src/program-facts-company-work.ts`, `src/lesson-conflict.ts`, `src/learn-ipc-handler.ts`, migration 125 | deterministic factual controls plus a local default-off detector-to-Company-Work adapter; migration/runtime activation remain unpromoted |
...
| Company work ledger      | `src/company-work-ledger.ts`, `src/company-work-shadow.ts`, `src/company-job-work-shadow.ts`, `src/company-job-work-shadow-cli.ts`, `src/program-facts-company-work.ts`, `src/company-work-report.ts`, migrations 118-119 and 125, `docs/COMPANY-OS-WORK-LEDGER.md`, `docs/COMPANY-OS-JOB-LEDGER.md` | Mailman/Sales and host-job projections are live under their recorded boundaries. NC-20260820-002 adds a local default-off condition-work candidate that routes program-facts drift into the existing exception projection; migration 125/runtime activation remain unpromoted and no component edits fact authority. |
