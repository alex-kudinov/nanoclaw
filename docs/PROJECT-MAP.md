...
| Company work ledger      | `src/company-work-ledger.ts`, `src/company-work-shadow.ts`, `src/company-job-work-shadow.ts`, `src/company-job-work-shadow-cli.ts`, `src/program-facts-company-work.ts`, `src/company-work-report.ts`, `src/company-work-indicators.ts`, `src/company-work-outcome-quality-assessment*.ts`, `src/company-work-exception-loop.ts`, `src/company-work-source-context.ts`, migrations 118-120, 125, and 126, `docs/COMPANY-OS-WORK-LEDGER.md`, `docs/COMPANY-OS-JOB-LEDGER.md`                 | Mailman/Sales and host-job projections are live under their recorded boundaries. NC-20260820-002 live-routes program-facts drift into the same exception projection. Exact release `bab154cb` deploys NC-20260820-003's source-bound Chief work packets; natural packet and Chief pickup proof remain pending. Exact release `09bc2408` preserves the 15 accepted/13 completed Sales baseline and live-deploys NC-20260820-006's dark, admin-only, append-only outcome-quality receipt plus coverage-aware third indicator. The live table has zero receipts; 0/13 visible outcomes are assessed, so the adverse rate remains unavailable. NC-20260820-007 adds a candidate standalone, default-dry-run, exact-release-gated operator receipt producer with no daemon, agent, content-read, bulk/default, message, or remediation path; repository presence is not deployment or receipt evidence. |
