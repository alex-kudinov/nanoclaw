...
| Relationship Context | `src/relationship-context*.ts`, `src/identity-join.ts`, `src/relationship-owner.ts`, migrations/rollbacks 137-138, `docs/RELATIONSHIP-CONTEXT-{CONTROL-PLANE,IMPLEMENTATION-PLAN,PRODUCTION-ROLLOUT,TRAFFT-EXACT-IDENTITY,PROVIDER-RECONCILIATION}.md`, `docs/RELATIONSHIP-OWNER-AUTHORITY.md` | exact live `adc0c5d8` has 2 source-created Trafft customers/4 appointments and global query off. NC-20260826-004 locally adds provider-neutral Plutio-ledger and private Encharge-snapshot adapters, explicit corroboration tiers, and terminal legacy classification. Aggregate preflight projects 1,374 Plutio refs, 1,242 Encharge refs, 159/173 Trafft customers and 358/424 appointment records connected, with 14 customers plus 66 appointment records legacy (42 identified, 24 missing customer ID); deployment/readback remain pending. Provider writes, guessed conflicts, Party merges, broad minion access, scheduled consumers, and outcomes remain excluded. |
...
| `docs/RELATIONSHIP-CONTEXT-CONTROL-PLANE.md` | canonical Party/source-ref, provider-adapter, projection, scoped query, Plutio dry-run/backfill, privacy, rollout, and rollback authority | migration 137 and strict source-created Trafft exact refs are live under `adc0c5d8`; one host-only canary is delivered while legacy identity, group/minion grants, provider writes, and outcomes remain separate |
