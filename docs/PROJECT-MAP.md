...
| Relationship Context | `src/relationship-context*.ts`, `src/identity-join.ts`, `src/relationship-owner.ts`, migrations/rollbacks 137-138, `docs/RELATIONSHIP-CONTEXT-{CONTROL-PLANE,IMPLEMENTATION-PLAN,PRODUCTION-ROLLOUT,TRAFFT-EXACT-IDENTITY}.md`, `docs/RELATIONSHIP-OWNER-AUTHORITY.md` | exact live `adc0c5d8` has the admin-only provider-neutral schema/owner authority plus healthy complete Trafft reconciliation over 422 natural rows: 2 exact customers, 4 exact appointments, 4 current projections, 418 current-row holds, zero conflicts, and stable 422/422 replay. One host-only minimized appointments canary delivered receipt 1; global query remains off with zero grants/consumer. Legacy/shared/mismatched identities remain held; provider/Plutio writes, guessed identity, broad minion access, scheduled consumers, and outcomes remain excluded. |
