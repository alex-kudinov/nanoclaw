...
Status: proposed and unapplied under `NC-20260907-005`
...
A future implementation may add one coordination manifest at
`facts/catalogs/student-catalog-publication-v1.json`. That file would contain
references, versions, digests, scopes, evidence states, and consumer intents. It
would not restate prices, curriculum, people, orders, or provider payloads. This
task creates no such canonical file.
...
These tests are implementation prerequisites. This proposed contract runs none
of them against production and changes no present publication behavior.
