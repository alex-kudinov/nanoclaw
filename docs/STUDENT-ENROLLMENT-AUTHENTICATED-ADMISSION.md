...
## NC-20260909-004 production-pilot integration candidate

The production-pilot branch adds a separately named facade without weakening
the disposable guard. It starts only in a verified immutable release whose
commit, activation epoch, exact preflight receipt, three distinct host-only
issuer keys, and Heartbeat marker UUID are bound in configuration. The facade
requires the exact `nanoclaw_business` / `nanoclaw_admin` identity and migration
146-148 shape before loading or persisting canonical state.

Both real Stripe ingress paths still converge on `handleStripePayment`. The
host reloads the exact Payment Intent, charge and canonical Party plus the
Checkout Session/line item/price/product when that native route supplied them.
The current Tandem Payment Element route is a direct Payment Intent: its
source-stamped offer, cohort, Party, terms, standard-pricing and participant
metadata replace any invented Checkout Session or price identity. The selector
accepts only the owner-authorized future Tandem event and rejects old, missing,
discounted, installment/invoice, refunded/disputed, wrong-product, wrong-class,
multi-seat or ambiguous identity evidence before a claim. The writer claim,
one-event policy budget, authenticated receipts, canonical materialization and
two assignment-version target identities share one SERIALIZABLE transaction.
An exact lost-commit acknowledgement is reconciled only by replaying the same
immutable certificate.

After `writer=enrollment`, Contador runs in accounting-only mode: Payment Log,
`public.payments`, and the durable fulfillment case still require exact
readback, while the legacy roster path is skipped. Provider delivery starts
only after those accounting receipts verify. Source presence is still local
candidate evidence: production migrations, configuration, activation, and any
provider/student effect remain blocked until fresh P01-P10 receipts pass.

Once the activation epoch is live, the Contador CLI also reads the persisted
canonical Payment Intent writer before any in-scope supervision roster effect.
Only `writer=legacy` permits that old path; `writer=enrollment`, no claim, a
claim-read failure, or a direct invocation without a valid event time becomes
accounting-only. Refused but source-valid pilot events receive the legacy claim
before the host continues the established route. Pre-activation and unrelated
offers retain their existing behavior.
