...
## NC-20260909-004 production driver candidate

The pilot branch promotes only the two required targets behind the exact
release/preflight/event gate. The canonical transaction supersedes the earlier
enrollment-preview roster request and persists one current assignment-version
delivery identity for Student Roster and one for Heartbeat. The production
worker claims only that exact assignment, not an arbitrary queued projection.
Already-verified target versions are read as complete after a host restart.

The Student Roster driver requires the accepted A:M header, resolves one unique
email/projection-key row, writes only C and F:M plus B when blank/Unknown, and
reads back the normalized projection/version payload. The Heartbeat driver
adds only the exact existing course-access UUID and accepted marker UUID with
sibling removal disabled, then reloads the exact user's group UUIDs. Both
drivers write a mode-0600 private preimage receipt before the first provider
effect. Ambiguous acceptance becomes a durable hold with no blind retry.
Compensation restores only an unchanged receipt-owned roster postimage and
removes only memberships absent in the recorded preimage; it never deletes a
group or removes pre-existing course access.

Preimage writes fsync the mode-0600 file and its private directory before the
first provider effect. An appended roster row gets a second durable `prepared`
checkpoint with the provider-assigned row number before it can become applied;
if the append response itself was lost, compensation re-resolves the unique
email/projection key, persists that locator, verifies the full postimage, and
then clears only A:M for the receipt-owned appended row. Existing-row apply and
compensation never rewrite A, D, or E.

These adapters are installed source, not current provider readiness. CSS header
write/readback and Heartbeat full invariant/create/exact-user/add/remove proof
remain mandatory P07/P08 gates before activation.
