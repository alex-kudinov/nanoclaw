...
Live status: lifecycle release `cf05bca358c9` and the four-action provider
shadow are deployed/live-verified; exact current merge release `7a36d79ca787`
directly includes those bytes and preserves the boundary. Catalog, registry,
and 143-member membership receipts are complete. Progress remains fail-closed
on `AUTH_ERROR`, so the 14-day/two-complete-progress-scan completion gate is
still open.
