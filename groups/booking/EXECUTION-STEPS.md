...
**Step 2b — Log the interaction (dedup by the host event identity):**

Set `INTERACTION_SOURCE_ID` before the query:

- `canceled`: `appt:${appointment_id}:canceled`
- `rescheduled`: `appt:${appointment_id}:rescheduled:${new_start}` where
  `new_start` is the same value parsed from `bookingStart`,
  `appointmentStart`, `start_date_time`, or `appointmentStartDateTime`, in that
  priority order
- all other appointment events: `${appointment_id}`

The canceled/rescheduled values must byte-match the host's archived `event_id`.
Do not invent or normalize a different date representation.
...
`fn_log_interaction_dedup` is idempotent — the last two args (`'trafft'`,
`'${INTERACTION_SOURCE_ID}'`) form the dedup key. Re-delivery of the same
lifecycle webhook produces no duplicate row, while a canceled or rescheduled
event no longer collides with the original booked interaction.
...
### Step 2c — Host-owned Plutio boundary (canceled/rescheduled only)

Do not call Plutio. This container has no Plutio credentials, scripts, or
mounts. After your run returns successfully, the host re-loads the archived
Trafft event and refuses to enqueue unless it finds the exact Booking
interaction from Step 2b. The durable outbox reaper performs the external write
with a remote replay marker. A host enqueue failure keeps the webhook retryable;
do not claim Plutio completion in your Slack notice.
...
Why:    {reason}
Source: {source}
— party {party_id} · interaction {interaction_id}
