...
`--campaign-id`; campaign selection, issuance, direct-message delivery, public
announcement, and provider readback belong to `issue-and-followthrough.sh`.
...
6. Branch on the structured receipt:
   - `issued_followthrough_complete` AND `verified:true`: archive to
     `completed/`, then report recipient, preset, canonical campaign key/ID,
     credential URLs, confirmed certificate email, verified direct-message
     receipt, and verified `Our Graduates` receipt.
   - `issued_private_no_followthrough`: archive to `completed/`; report the
     private issuance and both follow-through actions as not applicable.
   - `already_issued`: archive to `completed/`; report duplicate-safe no-op,
     no add, and no resend. Show the existing credential receipt.
   - `issued_followthrough_incomplete`, `issuance_pending_reconciliation`,
     malformed output, or ambiguous/API failure: move the script to
...
4. Branch on the receipt:
   - `issued_followthrough_complete` AND `verified:true`: archive, report the
     issuance plus both verified Heartbeat outcomes, and log Plutio.
   - `issued_private_no_followthrough`: archive, report the private issuance
     and both Heartbeat outcomes as not applicable, and log Plutio.
   - `already_issued`: archive, report no-op/no resend, do not log issuance.
   - incomplete follow-through, pending reconciliation, malformed output, or
...
`issue-and-followthrough.sh` owns this step inside the pending script. Gru does
not issue and then remember separate actions. For every newly issued public
credential the wrapper must return one combined receipt containing:

1. `issuance.status=issued`, `created:true`, and `emailConfirmed:true`;
2. `followthrough.directMessage.verified=true` with `sent`,
   `sent_reconciled`, or `already_sent`;
3. `followthrough.graduateAnnouncement.verified=true` with `announced` or
   `already_announced`;
4. overall `status=issued_followthrough_complete` and `verified=true`.

The direct message uses Heartbeat's documented `directChats` and
`directMessages` API endpoints. The community post uses the documented threads
API. Neither action uses Browser or Computer Use. Each action has its own
...
SERTIFIER_ENV_ROOT=/workspace/extra/sertifier \
HEARTBEAT_ROOT=/workspace/extra/heartbeat \
HEARTBEAT_ENV_ROOT=/workspace/extra/heartbeat \
CERTIFIER_FOLLOWTHROUGH_STATE_DIR=/workspace/group/followthrough \
  bash /workspace/extra/sertifier/tools/sertifier/certificate-followthrough.sh \
  --id "CREDENTIAL_ID" \
  --mode repair-existing \
  --state-dir /workspace/group/followthrough \
  --execute \
  --confirm REPAIR-EXISTING-CREDENTIAL-FOLLOWTHROUGH
```

Accept only `followthrough_complete` with both outcomes independently verified.
`followthrough_incomplete` is a hold. Never delete its durable receipt, reissue
the certificate, or retry a provider write blindly.
