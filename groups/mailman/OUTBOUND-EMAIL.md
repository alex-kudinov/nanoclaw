...
3. Use the exact `To`, optional `Cc`, `Thread-ID`, `Party ID`, `Entry ID`, and
   ...
   An approval card may name at most ten visible CC recipients. The exact
   action-bound approved card authorizes its CC list even when an address is
   outside the Party or absent from Gmail's latest external message. Never
   supply BCC.
