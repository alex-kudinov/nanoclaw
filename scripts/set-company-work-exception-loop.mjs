...
function usage(message) {
  ...
  node ${path.join(scriptDir, 'set-company-work-exception-loop.mjs')} --env-file <absolute-path> --expected-release <full-commit> --mode <off|on> [options]
...
const { restoreCompanyWorkExceptionConfig, setCompanyWorkExceptionConfig } =
  await import(
    new URL('../dist/company-work-exception-config-file.js', import.meta.url)
  );
...
if (restore) {
  ...
  result = setCompanyWorkExceptionConfig({
    envFile,
    mode,
