...
import {
  COMPANY_WORK_OUTCOME_REVIEW_ENV_KEYS,
...
export interface CompanyWorkOutcomeReviewConfigFileOptions {
  envFile: string;
  mode: 'off' | 'on';
...
export interface CompanyWorkOutcomeReviewPublicConfig {
  enabled: boolean;
  active: boolean;
  valid: boolean;
...
export interface CompanyWorkOutcomeReviewConfigFileResult {
  mode: 'dry-run' | 'applied' | 'unchanged';
  current: CompanyWorkOutcomeReviewPublicConfig;
  target: CompanyWorkOutcomeReviewPublicConfig;
  backupPath: string | null;
}

export interface RestoreCompanyWorkOutcomeReviewConfigOptions {
  envFile: string;
  backupFile: string;
  confirmHost: string;
}
...
function publicConfig(contents: string): CompanyWorkOutcomeReviewPublicConfig {
  const resolved = resolveCompanyWorkOutcomeReviewConfig(
    Object.fromEntries(
...
function requireSingleOperatorUid(value: string): string {
  const values = value
    .split(',')
    .map((entry) => entry.trim())
    .filter(Boolean);
  if (values.length !== 1 || !SLACK_UID_PATTERN.test(values[0])) {
    throw new Error('operator source must contain exactly one valid Slack UID');
  }
  return values[0];
}
...
function renderTarget(
  contents: string,
  options: CompanyWorkOutcomeReviewConfigFileOptions,
): string {
  if (options.mode === 'off') {
  ...
  if (sourceFile) {
    ...
    if (!sourceKey || !ALLOWED_OPERATOR_SOURCE_KEYS.has(sourceKey)) {
      throw new Error(
        'on mode requires the approved Company Work operator source key',
      );
    }
    ...
    operatorUids = requireSingleOperatorUid(existingOperatorUids);
  }
  let rendered = contents;
  const values: Record<string, string> = {
    COMPANY_WORK_OUTCOME_REVIEW_ENABLED: '1',
    COMPANY_WORK_OUTCOME_REVIEW_OPERATOR_UIDS: operatorUids,
    COMPANY_WORK_OUTCOME_REVIEW_INTERVAL_MS: String(
      positiveInteger(options.intervalMs, 86_400_000, 'intervalMs'),
    ),
    COMPANY_WORK_OUTCOME_REVIEW_WINDOW_DAYS: String(
      positiveInteger(options.windowDays, 30, 'windowDays'),
    ),
    COMPANY_WORK_OUTCOME_REVIEW_CANDIDATE_LIMIT: String(
      positiveInteger(options.candidateLimit, 25, 'candidateLimit'),
    ),
  };
...
function backupName(envFile: string, now: Date): string {
  ...
  return `${envFile}.rollback-company-work-outcome-review-${stamp}`;
}
...
export function setCompanyWorkOutcomeReviewConfig(
  options: CompanyWorkOutcomeReviewConfigFileOptions,
): CompanyWorkOutcomeReviewConfigFileResult {
  ...
  if (!target.valid) {
    throw new Error(
      `target outcome-review configuration is invalid: ${target.configurationError}`,
    );
  }
  ...
  try {
    ...
    if (fs.readFileSync(options.envFile, 'utf8') !== rendered) {
      throw new Error('outcome-review environment write verification failed');
    }
  } catch (error) {
...
export function restoreCompanyWorkOutcomeReviewConfig(
  options: RestoreCompanyWorkOutcomeReviewConfigOptions,
): CompanyWorkOutcomeReviewConfigFileResult {
  ...
  const expectedPrefix = `${options.envFile}.rollback-company-work-outcome-review-`;
  if (!options.backupFile.startsWith(expectedPrefix)) {
    throw new Error('backup is not an outcome-review backup for this file');
  }
  if (options.confirmHost !== os.hostname()) {
  ...
  if (!restored.valid) {
    throw new Error('backup outcome-review configuration is invalid');
  }
  ...
  if (fs.readFileSync(options.envFile, 'utf8') !== backupContents) {
    throw new Error('outcome-review environment restore verification failed');
  }
  return {
    mode: 'applied',
    current,
