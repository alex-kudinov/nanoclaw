...
const catalog = JSON.parse(
  fs.readFileSync(
    'facts/catalogs/student-enrollment-projection-targets-v1.json',
    'utf8',
  ),
);
...
describe('student enrollment projection target catalog', () => {
  ...
  it('names all targets exactly once and requires roster plus Heartbeat', () => {
    expect(catalog.targets.map((target: any) => target.target).sort()).toEqual([
      'encharge',
      'heartbeat',
      'plutio',
      'student_roster',
    ...
    expect(
      catalog.targets
        .filter((target: any) => target.disposition === 'required')
        .map((target: any) => target.target)
        .sort(),
    ).toEqual(['heartbeat', 'student_roster']);
  });
  it('keeps current rollout readiness false and the Heartbeat layers distinct', () => {
    expect(
      catalog.targets.every(
        (target: any) => !target.permission_ready && !target.activation_ready,
      ),
    ).toBe(true);
    const heartbeat = catalog.targets.find(
      (target: any) => target.target === 'heartbeat',
    );
    expect(heartbeat.destination.course_access_group_id).toBe(
  ...
  it('keeps Encharge and Plutio explicitly not applicable without destinations', () => {
    for (const target of catalog.targets.filter((target: any) =>
      ['encharge', 'plutio'].includes(target.target),
    )) {
