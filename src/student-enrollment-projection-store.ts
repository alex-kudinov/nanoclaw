...
export type ProjectionDatabaseGuard = (client: PoolClient) => Promise<void>;
...
export async function queueProjection(
  ...
  return transaction(
    pool,
    async (client) => {
      if (!['assignment', 'enrollment'].includes(envelope.subjectType))
        throw new Error('unsupported_projection_subject');
      const subjectTable =
        envelope.subjectType === 'assignment'
      ...
      const current = await client.query(
        `SELECT version FROM business_v2.${subjectTable}
       WHERE ${subjectKey}=$1 FOR UPDATE`,
        [envelope.subjectKey],
      );
      if (current.rowCount !== 1)
        throw new Error('projection_subject_not_found');
  ...
  return transaction(
    ...
    async (client) => {
      await client.query(
       ...
       WHERE o.target=$1 AND (
         (o.subject_type='assignment' AND EXISTS (
           SELECT 1 FROM business_v2.student_class_assignments a
           WHERE a.assignment_key=o.subject_key AND a.version<>o.subject_version
         )) OR
         (o.subject_type='enrollment' AND EXISTS (
           SELECT 1 FROM business_v2.student_enrollments_v2 e
           WHERE e.enrollment_key=o.subject_key AND e.version<>o.subject_version
         )))
...
export async function claimProjection(
  ...
  return transaction(
    ...
    async (client) => {
      const selected = await client.query(
        `SELECT * FROM business_v2.student_projection_outbox
       WHERE target=$1 AND target_idempotency_key=$2
         AND ((uncertain_acceptance=false AND
               (state IN ('queued','failed') OR
                (state='claimed' AND lease_expires_at <= $3))) OR
              ($4::boolean AND state='held' AND uncertain_acceptance=true))
       FOR UPDATE`,
        [envelope.target, envelope.idempotencyKey, now, reconcileUncertain],
      );
      if (!selected.rowCount) return null;
      ...
      const updated = await client.query(
        `UPDATE business_v2.student_projection_outbox
       SET state='claimed',lease_token=$2,lease_expires_at=$3,
           uncertain_acceptance=false,attempt_count=attempt_count+1,
      ...
      return {
        envelope: {
          ...
          payloadSha256: row.payload_sha256,
          expectedReadback: row.payload_json,
          expectedReadbackSha256: row.expected_readback_sha256,
        },
        projectionVersion: row.version,
        leaseToken,
...
export async function readProjectionState(
  pool: Pick<Pool, 'connect'>,
  envelope: ProjectionEnvelope,
  databaseGuard: ProjectionDatabaseGuard = guardEnrollmentStore,
): Promise<{
  state: 'queued' | 'claimed' | 'verified' | 'failed' | 'held' | 'superseded';
  ...
  return transaction(
    ...
    async (client) => {
      const result = await client.query<{
        ...
        `SELECT state,last_error_code FROM business_v2.student_projection_outbox
       WHERE target=$1 AND target_idempotency_key=$2`,
        [envelope.target, envelope.idempotencyKey],
      );
...
export class PgProjectionDeliveryLedger implements ProjectionDeliveryLedger {
  constructor(
    ...
    private readonly databaseGuard: ProjectionDatabaseGuard = guardEnrollmentStore,
  ) {}

  private async row(client: PoolClient, envelope: ProjectionEnvelope) {
    const result = await client.query(
      `SELECT o.*,
        CASE WHEN o.subject_type='assignment' THEN a.version
             WHEN o.subject_type='enrollment' THEN e.version END
          AS current_subject_version
       FROM business_v2.student_projection_outbox o
       LEFT JOIN business_v2.student_class_assignments a
         ON o.subject_type='assignment' AND a.assignment_key=o.subject_key
       LEFT JOIN business_v2.student_enrollments_v2 e
         ON o.subject_type='enrollment' AND e.enrollment_key=o.subject_key
       WHERE o.target=$1 AND o.target_idempotency_key=$2 FOR UPDATE OF o`,
      [envelope.target, envelope.idempotencyKey],
    );
    if (result.rowCount !== 1) throw new Error('projection_not_found');
  ...
  async recordVerified(
    ...
    await transaction(
      ...
      async (client) => {
        await client.query(
          `UPDATE business_v2.student_enrollment_exceptions_v2
         SET state='resolved',resolved_at=$2,resolution_sha256=$3,
             last_seen_at=$2,version=version+1,updated_by=$4
         WHERE exception_key=$1 AND state IN ('open','acknowledged')`,
          [
            `exception:${envelope.idempotencyKey}:provider_acceptance_uncertain`,
            now,
            readbackSha256,
            this.actor,
          ],
