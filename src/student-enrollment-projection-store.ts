...
export async function claimExactProjection(
  pool: Pick<Pool, 'connect'>,
  target: StudentProjectionTarget,
  subjectKey: string,
  now: string,
  ...
  return transaction(
    ...
    async (client) => {
      const selected = await client.query(
        `SELECT o.* FROM business_v2.student_projection_outbox o
         JOIN business_v2.student_class_assignments a
           ON a.assignment_key=o.subject_key
          AND a.version=o.subject_version
         WHERE o.target=$1 AND o.subject_type='assignment' AND o.subject_key=$2
           AND o.uncertain_acceptance=false
           AND (o.state IN ('queued','failed') OR
                (o.state='claimed' AND o.lease_expires_at <= $3))
         ORDER BY o.subject_version DESC,o.id
         FOR UPDATE OF o SKIP LOCKED LIMIT 1`,
        [target, subjectKey, now],
      );
      if (!selected.rowCount) return null;
      const leaseToken = randomUUID();
      const updated = await client.query(
...
export async function readExactProjectionState(
  pool: Pick<Pool, 'connect'>,
  target: StudentProjectionTarget,
  subjectKey: string,
  guard: EnrollmentStoreGuard = guardEnrollmentStore,
  ...
  return transaction(
    ...
    async (client) => {
      const result = await client.query(
        `SELECT state FROM business_v2.student_projection_outbox
          WHERE target=$1 AND subject_type='assignment' AND subject_key=$2
          ORDER BY subject_version DESC,id DESC LIMIT 1`,
        [target, subjectKey],
      );
      return result.rowCount === 1 ? String(result.rows[0].state) : null;
    },
    guard,
  );
}
...
export class PgProjectionDeliveryLedger implements ProjectionDeliveryLedger {
  constructor(
    ...
    private readonly guard: EnrollmentStoreGuard = guardEnrollmentStore,
  ) {}
