...
export function buildStudentRosterProjection(
  ...
  const projectionKey = `student_projection:student_roster:${subject.assignmentKey}:v${subject.assignmentVersion}`;
  const row = {
    projection_key: projectionKey,
    projection_version: subject.assignmentVersion,
...
export function normalizeProviderReadback(
  ...
  if (target === 'student_roster')
    return {
      projection_key: v.projection_key,
