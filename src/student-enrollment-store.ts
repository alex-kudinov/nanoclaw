...
export async function persistEnrollmentDecision(
  ...
  return persistEnrollmentDecisionWithGuard(pool, decide, guardEnrollmentStore);
}

/** Shared transaction mechanics for the separately guarded production facade.
 * Runtime composition must import the production facade, never this helper. */
export async function persistEnrollmentDecisionWithGuard(
  pool: Pick<Pool, 'connect'>,
  decide: (
    client: PoolClient,
    state: BookkeeperEnrollmentState,
  ) => Promise<EnrollmentIngressResult>,
  guard: EnrollmentStoreGuard,
  afterPersist?: (
    client: PoolClient,
  ...
  try {
    ...
    const before = await loadEnrollmentStore(client, guard);
    const result = await decide(client, before.state);
    await persistEnrollmentStore(client, before, result, guard);
