...
def validate_exports(web_path: Path, pack_path: Path) -> list[str]:
    ...
    marker = re.search(r"program-facts: practitioner-series revision=(\d+) sha256=([a-f0-9]{64})", pack)
    if not marker:
...
def inject_local() -> list[str]:
    ...
    for path in knowledge_paths():
        path.write_text(inject(path.read_text(encoding="utf-8"), pack), encoding="utf-8")
    return []
...
def check(source: Path | None = None) -> list[str]:
    ...
    for path in knowledge_paths():
        ...
        if block not in text:
            errors.append(f"missing/stale canonical block: {path.relative_to(ROOT)}")
        legacy = re.search(r"For the \*\*3 approved\*\* courses|courses still in review", text)
        if legacy:
            errors.append(f"legacy Practitioner approval prose remains: {path.relative_to(ROOT)}")
    return errors
...
def main() -> int:
    ...
    if errors:
        for error in errors:
            print(f"ERROR: {error}")
        return 1
    print(f"VALID: Practitioner facts pinned and present in {len(knowledge_paths())} knowledge files")
    return 0
